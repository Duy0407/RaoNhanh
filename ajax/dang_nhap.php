<?
include("config.php");
$user = getValue('taikhoan', 'str', 'POST', '');
$pass = getValue('matkhau', 'str', 'POST', '');
...
if ($user != "" && $pass != "") {
    ...
    if (preg_match($regex_sdt, $user)) {
        $db_qr = new db_query("SELECT * FROM `user` WHERE `usc_phone` = '" . $user . "' AND `usc_pass` = '" . $pass_md5 . "' AND (`usc_type` = 1 OR `usc_type` = 5) ");
    } else if (preg_match($regex_email, $user)) {
        $db_qr = new db_query("SELECT * FROM `user` WHERE `usc_email` = '" . $user . "' AND `usc_pass` = '" . $pass_md5 . "' AND (`usc_type` = 1 OR `usc_type` = 5) ");
    } else {
        $db_qr = new db_query("SELECT * FROM user WHERE `usc_account` = '" . $user . "' AND `usc_pass` = '" . $pass_md5 . "' AND AND (`usc_type` = 1 OR `usc_type` = 5) ");
    };

    if (mysql_num_rows($db_qr->result) > 0) {
        $row_u = mysql_fetch_assoc($db_qr->result);
        $id_user = $row_u['usc_id'];
        $us_type = $row_u['usc_type'];
        ...
        setcookie('UT', $us_type, time() + 24 * 3600, '/');
        ...
        $data = [
            ...
            'data' => null,
            'error' => [
                'message' => "Số điện thoại (email hoặc tên đăng nhập) hoặc mật khẩu không đúng",
            ]
        ];
    }
} else {
