...
$loai_do_uong = getValue('loai_do_uong', 'int', 'POST', '');
    ...
    if ($cou_file > 0 || $cou_dang > 0) {
        ...
        if (mysql_num_rows($check_tkhoan->result) > 0) {
            ...
            if (mysql_num_rows($check_tde->result) > 0) {
                ...
                $inser_des = new db_query("INSERT INTO `new_description`(`new_des_id`, `new_id`, `new_description`,`loai_chung`, `han_su_dung`,`canhan_moigioi`)
                                    VALUES ('','$id_dt','$mo_ta','$loai_do_uong','$han_sdung','$canhan_moigioi')");
