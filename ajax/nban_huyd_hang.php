...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT'])) {
    ...
    if ($id_dh != 0 ) {
        ...
        if(mysql_num_rows($donhang -> result) > 0){
            ...
            $nguoi_gui = $row_dh['id_nguoi_ban'];
            $nguoi_nhan = $row_dh['id_nguoi_dh'];
            ...
            $updat_dh = new db_query("UPDATE `dat_hang` SET `trang_thai` = '$trang_thai', `tgian_huydhang` = '$tgian_huy' WHERE `dh_id` = $id_dh ");

            $inser_nof = new db_query("INSERT INTO `notify`(`id`, `notify_from`, `new_id`, `notify_to`, `type`, `create_time`, `notify_content`)
                                VALUES ('','$nguoi_gui','$new_id','$nguoi_nhan','12','$tgian_huy','$noi_dung')");
