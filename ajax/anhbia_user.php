...
$filename = '/pictures/avt_dangtin/'.time() . '_' . $_FILES['files']['name'];
...
if($filename != ''){
    ...
    $up_img = new db_query("UPDATE `user` SET `usc_anhbia` = '$filename' WHERE `usc_id` = '$user_id' AND `usc_type` = '$user_type'");
}else{
    echo "Ảnh Lỗi";
}
