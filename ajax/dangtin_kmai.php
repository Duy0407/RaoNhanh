...
if (isset($_COOKIE['UID'])) {
    ...
    if ($xac_thuc == 1) {
        ...
        $so_luong_kho = getValue('so_luong_kho', 'str', 'POST', 0);
        ...
        if ($cou_file > 0 || $cou_dang > 0) {
            ...
            if (mysql_num_rows($query_check->result) > 0) {
                ...
                $query_dt = new db_query("INSERT INTO `new`(`new_title`,`link_title`,`new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,
                                                ...
                                                `chotang_mphi`, `quan_huyen`, `phuong_xa`, `new_sonha`, `dia_chi`, `new_video`, `new_ctiet_dmuc`, `new_buy_sell`,
                                                `new_update_time`,`thoigian_kmbd`,`thoigian_kmkt`,`soluong_min`,`soluong_max`) 
                                                VALUES ('$tieu_de','$tieu_de','$td_gia_spham','$id_dm','$tinh_thanh','$user_id','$file_anh',
                                                '$ngay_dang','$user_type',1,'$dvi_tien','$user_name','$sdt_lhe','$email_lhe',
                                                '" . $dia_chi . "','$td_lienhe_ngban',0,0,'','" . $dia_chi . "','$file_video','$ctiet_dmuc',
                ...
                $query_decription = new db_query("INSERT INTO `new_description`(`new_id`, `new_description`,`canhan_moigioi`) VALUES ('$id','$mo_ta','$canhan_moigioi')");
                if ($xac_thuc == 1) {
                    $query_ttbh = new db_query("INSERT INTO `thongtin_banhang` (`id_new`,`nhom_phan_loai`,`phan_loai`,`loai`,`so_luong_kho`,`loai_khuyenmai`,
                                        `giatri_khuyenmai`,`van_chuyen`,`phi_van_chuyen`,`gia_sanpham_xt`) 
                                        VALUES ('$id','$nhom_phan_loai','$phan_loai','$loai_xt','$so_luong_kho','$loai_khuyenmai','$giatri_khuyenmai','$van_chuyen',
                                        '$phi_van_chuyen','$gia_sanpham_xt')");
                }
            }
        } else {
