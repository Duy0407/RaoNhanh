...
if(isset($_FILES) and $_SERVER['REQUEST_METHOD'] == "POST"){
   $userid = $_COOKIE["UID"];
   $userid = (int)$userid;
   $db_qrcheck = new db_query("SELECT usc_id,usc_time,usc_anhbia FROM user WHERE usc_id = '".$userid."' LIMIT 1");
   if(mysql_num_rows($db_qrcheck->result) > 0)
   {
      ...
      $db_exx = new db_execute("UPDATE user SET usc_anhbia = '".$dir.$name."' WHERE usc_id = ".$userid);
   }
