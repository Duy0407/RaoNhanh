        ...
        if ($cou_file > 0 || $cou_dang > 0) {
            ...
            if (mysql_num_rows($query_check->result) > 0) {
                ...
                for ($i = 0; $i < $cou_file; $i++) {
                    ...
                    $filename = '/pictures/du_lich/' . time() . '_' . $filename_strr;
                    $luu_anh = time() . '_' . $filename_strr;
                    $anh .= $filename . ';';
                    $filetmp_name = $_FILES['files']['tmp_name'][$i];
                    $dir = '../pictures/du_lich/';
                ...
                $query = new db_query("UPDATE `new` SET `new_title`='$tieu_de',`new_money`='$td_gia_spham', `new_city`='$tinh_thanh', `new_image`='$file_anh',
                                ...
                                `phuong_xa`= 0, `new_sonha`='', `dia_chi`='" . $dia_chi . "', `new_video`='$file_video', `new_address`='" . $dia_chi . "',
                ...
                //     'FromWeb' => $domain
                // ];
                // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa chỉnh sửa tin đăng sức khỏe sắc đẹp spa: " . $id_nd . " trên " . $domain;
                // $data_send_to_group = [
                //     'LiveChat' => $LiveChat,
