...
if (isset($_COOKIE['UID'])) {
    ...
    $nam_san_xuat = getValue('nam_san_xuat', 'int', 'POST', 0);
    $hop_so = getValue('hop_so', 'int', 'POST', 0);
    $nhien_lieu = getValue('nhien_lieu', 'int', 'POST', 0);
    $xuat_xu = getValue('xuat_xu', 'int', 'POST', 0);
    $so_cho = getValue('so_cho', 'int', 'POST', 0);
    $mau_sac = getValue('mau_sac', 'int', 'POST', 0);
    $kieu_dang = getValue('kieu_dang', 'int', 'POST', 0);
    $bao_hanh = getValue('bao_hanh', 'int', 'POST', 0);
    $tinhtrang = getValue('tinhtrang', 'int', 'POST', 0);
    $km_di = getValue('km_di', 'int', 'POST', 0);
    ...
    $td_phien_ban = getValue('td_phien_ban', 'int', 'POST', 0);
                ...
                if ($cou_file > 0 || $cou_dang > 0) {
                    ...
                    if (mysql_num_rows($query_check->result) > 0) {
                        ...
                        $query_dt = new db_query("INSERT INTO `new`(`new_title`, `link_title`,`new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,
                                                ...
                                                VALUES ('$tieu_de','$tieu_de','$td_gia_spham','$cate_id','$tinh_thanh','$user_id',
                                                '$file_anh','$ngay_dang','$user_type',1,'$dvi_tien','$user_name','$sdt_lhe','$email_lhe','$dia_chi',0,'$quan_huyen',
                                                '$phuong_xa','" . $so_nha . "','" . $dia_chi . "','$file_video','$ctiet_dmuc','$tinhtrang','$bao_hanh',2,'$ngay_dang'
                        ...
                        $query_decription = new db_query("INSERT INTO `new_description`(`new_id`, `new_description`,`hang`,`dong_xe`,`so_km_da_di`,`nam_san_xuat`,
                                                    `hop_so`,`nhien_lieu`,`xuat_xu`,`so_cho`,`mau_sac`,`kieu_dang`,`td_bien_soxe`,`phien_ban`,`canhan_moigioi`)
                                                    VALUES ('$id_dt','$mo_ta','$hang_xe','$dong_xe','$km_di','$nam_san_xuat','$hop_so','$nhien_lieu','$xuat_xu',
                                                    '$so_cho','$mau_sac','$kieu_dang','$td_bien_soxe','$td_phien_ban','$canhan_moigioi')");
                        ...
                        //     'FromWeb' => $domain
                        // ];
                        // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa đăng tin xe ô tô trên " . $domain;
                        // $message = "Tôi tên là: " . $user_name . ", " . $phone . "" . $email . "tôi vừa đăng tin xe ô tô trên " . $domain;
                        // $data_send_to_group = [
                        //     'LiveChat' => $LiveChat,
