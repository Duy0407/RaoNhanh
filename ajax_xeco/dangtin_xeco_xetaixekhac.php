...
if (isset($_COOKIE['UID'])) {
    ...
    $trong_tai = getValue('trong_tai', 'int', 'POST', 0);
    $nhien_lieu = getValue('nhienlieu', 'int', 'POST', 0);
            ...
            if ($user_id != 0 && $user_type != 0 && $tieu_de != "") {
                if ($cou_file > 0 || $cou_dang > 0) {
                    ...
                    if (mysql_num_rows($query_check->result) > 0) {
                        ...
                        $query_decription = new db_query("INSERT INTO `new_description`(`new_id`, `new_description`, `hang`, `trong_tai`, `nhien_lieu`, `mau_sac`,
                                                    `so_km_da_di`,`td_bien_soxe`,`canhan_moigioi`,`nam_san_xuat`,`xuat_xu`) VALUES ('$id_dt','$mo_ta','$hang_xe',
                                                    '$trong_tai','$nhien_lieu','$mau_sac','$km_di','$td_bien_soxe','$canhan_moigioi','$nam_san_xuat','$xuat_xu')");
                        include('../ajax/xac_thuc_chung.php');

                        // fastcgi_finish_request();
                        // $tt_crea = 'Xe cộ - xe tải xe khác';
