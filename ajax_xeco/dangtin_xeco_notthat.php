...
if (isset($_COOKIE['UID'])) {
    ...
    $loai_noi_that = getValue('loai_noi_that', 'int', 'POST', 0);
                ...
                if ($cou_file > 0 || $cou_dang > 0) {
                    ...
                    if (mysql_num_rows($query_check->result) > 0) {
                        ...
                        $query_dt = new db_query("INSERT INTO `new`(`new_title`, `link_title`, `new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,
                                            ...
                                            `new_buy_sell`, `new_update_time`) VALUES ('$tieu_de', '$tieu_de','$td_gia_spham','$cate_id','$tinh_thanh','$user_id',
                                            '$file_anh','$ngay_dang','$user_type',1,'$dvi_tien','$user_name','$sdt_lhe','$email_lhe','$dia_chi',0,'$td_lienhe_ngban',
                                            '$quan_huyen','$phuong_xa','" . $so_nha . "','" . $dia_chi . "','$file_video','$ctiet_dmuc','$tinhtrang',2,'$ngay_dang')");

                        $id_des = new db_query("SELECT LAST_INSERT_ID() AS id");
                        $id_dt = mysql_fetch_assoc($id_des->result)['id'];

                        $query_decription = new db_query("INSERT INTO `new_description`(`new_id`, `new_description`,`loai_noithat`,`canhan_moigioi`)
                                                        VALUES ('$id_dt','$mo_ta','$loai_noi_that','$canhan_moigioi')");
                        ...
                        //     'FromWeb' => $domain
                        // ];
                        // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa đăng tin xe cộ nội thất ô tô trên " . $domain;
                        // $message = "Tôi tên là: " . $user_name . ", " . $phone . "" . $email . "tôi vừa đăng tin xe cộ nội thất ô tô trên " . $domain;
                        // $data_send_to_group = [
                        //     'LiveChat' => $LiveChat,
