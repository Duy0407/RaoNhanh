...
$loai_noi_that = getValue('loai_noi_that', 'int', 'POST', '');
    ...
    if ($cou_file > 0 || $cou_dang > 0) {
        ...
        if (mysql_num_rows($query_check->result) > 0) {
            ...
            $query_xc = new db_query("UPDATE `new` SET `new_title`='$tieu_de',`new_money`='$td_gia_spham',
                                                            ...
                                                            `phuong_xa`='$phuong_xa',`new_phone` = '$sdt_lienhe',`new_email` = '$email_lhe' ,
                                                            `new_sonha`='".$so_nha."', `dia_chi`='".$dia_chi."', 
                                                            `new_video`='$file_video', `new_ctiet_dmuc`='$ctiet_dmuc',`new_tinhtrang`='$tinhtrang',
                                                            `thoigian_kmbd` = '$ngay_bat_dau',`thoigian_kmkt` = '$ngay_ket_thuc',
                                                            `soluong_min` = '$soluong_min',`soluong_max` = '$soluong_max' WHERE new_id = $id_nd ");

            $query_des = new db_query("UPDATE `new_description` SET `new_description`='$mo_ta',`loai_noithat`='$loai_noi_that',`canhan_moigioi`='$canhan_moigioi' WHERE new_id = $id_nd ");
