...
if ($user_id != "" && $user_type != "" && $tieu_de != "") {
    if ($cou_file > 0 || $cou_dang > 0) {
        ...
        if (mysql_num_rows($query_check->result) > 0) {
            ...
            // ---------------Tài khoản người bán---------------
            $query_xc = new db_query("UPDATE `new` SET `new_title`='$tieu_de',`new_money`='$td_gia_spham',
                                                        `new_city`='$tinh_thanh', `new_image`='$file_anh',
                                                        `new_update_time`='$ngay_dang',`new_unit`='$dvi_tien',
                                                        `quan_huyen`='$quan_huyen', `phuong_xa`='$phuong_xa', 
                                                        `new_sonha`='".$so_nha."',`new_phone` = '$sdt_lienhe',`new_email` = '$email_lhe' ,
                                                        `dia_chi`='".$dia_chi."', `new_video`='$file_video', `new_ctiet_dmuc`='$ctiet_dmuc',
                                                        `new_tinhtrang`='$tinhtrang',`new_baohanh`='$bao_hanh',
                                                        `chotang_mphi`='$td_lienhe_ngban',`thoigian_kmbd` = '$ngay_bat_dau',
                                                        `thoigian_kmkt` = '$ngay_ket_thuc',`soluong_min` = '$soluong_min',
                                                        `soluong_max` = '$soluong_max' WHERE new_id = $id_nd ");

            $query_des = new db_query("UPDATE `new_description` SET
                                             `new_description`='$mo_ta',`hang`='$hang_xe',
                                             `dong_xe`='$dong_xe',`so_km_da_di`='$km_di',
                                             `nam_san_xuat`='$nam_san_xuat', `hop_so`='$hop_so',
                                             `nhien_lieu`='$nhien_lieu',`xuat_xu`='$xuat_xu',
                                             `so_cho`='$so_cho',`mau_sac`='$mau_sac',`kieu_dang`='$kieu_dang',
                                             `td_bien_soxe`='$td_bien_soxe',`td_phien_ban`='$td_phien_ban',`canhan_moigioi`='$canhan_moigioi'
                                             WHERE new_id = $id_nd  ");
