...
if (isset($_COOKIE['UID'])) {
    ...
    $loai_xe_dap = getValue('loai_xe', 'int', 'POST', 0);
    $xuat_xu = getValue('xuat_xu', 'int', 'POST', 0);
    $kich_thuoc_khung = getValue('kich_thuoc_khung', 'int', 'POST', 0);
    $mau_sac = getValue('mau_sac', 'int', 'POST', 0);
    $chat_lieu_khung = getValue('chat_lieu_khung', 'int', 'POST', 0);
    $dong_xe = getValue('dong_xe', 'int', 'POST', 0);
        ...
        if ($cou_file > 0 || $cou_dang > 0) {
            ...
            if (mysql_num_rows($query_check->result) > 0) {
                ...
                $query_dt = new db_query("INSERT INTO `new`(`new_title`,`link_title`,`new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,
                                                ...
                                                '$td_lienhe_ngban','$quan_huyen','$phuong_xa','" . $so_nha . "','" . $dia_chi . "','$file_video','$ctiet_dmuc','$tinhtrang',
                                                '$bao_hanh',2,'$ngay_dang','$ngay_bat_dau','$ngay_ket_thuc','$soluong_min','$soluong_max')");

                $id_des = new db_query("SELECT LAST_INSERT_ID() AS id");
                $id_dt = mysql_fetch_assoc($id_des->result)['id'];

                $query_decription = new db_query("INSERT INTO `new_description`(`new_id`, `new_description`,`hang`,`loai_xe`,`xuat_xu`,`kich_co`,`mau_sac`,
                                                        `chat_lieu_khung`,`dong_xe`,`canhan_moigioi`) VALUES ('$id_dt','$mo_ta','$hang_xe','$loai_xe_dap','$xuat_xu',
                                                        '$kich_thuoc_khung','$mau_sac','$chat_lieu_khung','$dong_xe','$canhan_moigioi')");
                ...
                //     'FromWeb' => $domain
                // ];
                // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa đăng tin xe đạp trên " . $domain;
                // $message = "Tôi tên là: " . $user_name . ", " . $phone . "" . $email . "tôi vừa đăng tin xe đạp trên " . $domain;
                // $data_send_to_group = [
                //     'LiveChat' => $LiveChat,
