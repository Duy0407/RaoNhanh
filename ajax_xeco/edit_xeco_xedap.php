...
$kich_thuoc_khung = getValue('kich_thuoc_khung', 'int', 'POST', '');
...
if ($user_id != "" && $user_type != "" && $tieu_de != "") {
    if ($cou_file > 0 || $cou_dang > 0) {
        ...
        if (mysql_num_rows($query_check->result) > 0) {
            ...
            $query_des = new db_query("UPDATE `new_description` 
                                        SET  `new_description`='$mo_ta',`hang`='$hang_xe',`loai_xe`='$loai_xe',
                                        `xuat_xu`='$xuat_xu',`kich_co`='$kich_thuoc_khung',`mau_sac`='$mau_sac',
                                        `chat_lieu_khung`='$chat_lieu_khung',`canhan_moigioi`='$canhan_moigioi'  WHERE new_id = $id_nd ");
