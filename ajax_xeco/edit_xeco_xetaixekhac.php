    ...
    if ($cou_file > 0 || $cou_dang > 0) {
        ...
        if (mysql_num_rows($query_check->result) > 0) {
            ...
            $query_xc = new db_query("UPDATE `new` SET `new_title`='$tieu_de',`new_money`='$td_gia_spham',
                                                        ...
                                                        `chotang_mphi`='$td_lienhe_ngban', `quan_huyen`='$quan_huyen',
                                                        `phuong_xa`='$phuong_xa', `new_sonha`='" . $so_nha . "', 
                                                        `dia_chi`='" . $dia_chi . "', `new_video`='$file_video',
                                                        `new_ctiet_dmuc`='$ctiet_dmuc',`new_tinhtrang`='$tinhtrang',
                                                        `thoigian_kmbd` = '$ngay_bat_dau',`thoigian_kmkt` = '$ngay_ket_thuc',
                                                        `soluong_min` = '$soluong_min',`soluong_max` = '$soluong_max'
                                                        WHERE new_id = $id_nd ");

            $query_des = new db_query("UPDATE `new_description` SET  `new_description`='$mo_ta',`hang`='$hang_xe',
                                                                        `trong_tai`='$trong_tai',`nhien_lieu`='$nhien_lieu',
                                                                        `mau_sac`='$mau_sac',`so_km_da_di`='$km_di',
                                                                        `td_bien_soxe`='$td_bien_soxe',`canhan_moigioi`='$canhan_moigioi',
                                                                        `nam_san_xuat`='$nam_san_xuat',`xuat_xu`='$xuat_xu'
                                                                        WHERE new_id = $id_nd ");
