...
if (isset($_COOKIE['UID'])) {
    ...
    $loai_xe = getValue('loai_xe', 'int', 'POST', 0);
    $dung_tich_xe = getValue('dung_tich_xe', 'int', 'POST', 0);
        ...
        if ($cou_file > 0 || $cou_dang > 0) {
            ...
            if (mysql_num_rows($query_check->result) > 0) {
                ...
                $query_dt = new db_query("INSERT INTO `new`(`new_title`,`link_title`,`new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,
                                                ...
                                                '$td_lienhe_ngban','$quan_huyen','$phuong_xa','" . $so_nha . "','" . $dia_chi . "','$file_video','$ctiet_dmuc',
                                                '$tinhtrang','$bao_hanh',2,'$ngay_dang','$ngay_bat_dau','$ngay_ket_thuc','$soluong_min','$soluong_max')");

                $id_des = new db_query("SELECT LAST_INSERT_ID() AS id");
                $id_dt = mysql_fetch_assoc($id_des->result)['id'];

                $query_decription = new db_query("INSERT INTO `new_description`( `new_id`, `new_description`,`hang`,`dong_xe`,`loai_xe`,`so_km_da_di`,
                                                    `dung_tich`,`nam_san_xuat`,`td_bien_soxe`,`canhan_moigioi`) VALUES ('$id_dt','$mo_ta','$hang_xe','$dong_xe',
                                                    '$loai_xe','$km_di', '$dung_tich_xe','$nam_san_xuat','$td_bien_soxe','$canhan_moigioi')");
                ...
                //     'FromWeb' => $domain
                // ];
                // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa đăng tin xe máy trên " . $domain;
                // $message = "Tôi tên là: " . $user_name . ", " . $phone . "" . $email . "tôi vừa đăng tin xe máy trên " . $domain;
                // $data_send_to_group = [
                //     'LiveChat' => $LiveChat,
