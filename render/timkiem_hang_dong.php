...
<!-- Dụng cụ thể thao -->
	<?php if ($id_dm==75): ?>
		...
		<?php if ($id_hang==8): ?>
			<p class="w_100 sh_clr_two sh_size_three mb_5 cr_weight">Loại dụng cụ</p>
			<input type="text" name="loai75" class="m_timkiem_input" autocomplete="off" placeholder="Nhập loại dụng cụ">
		<?php endif ?>
	<?php endif ?>
<!-- PHỤ KIÊN THỂ THAO -->
	...
	<?php if ($id_dm==105): ?>
		...
		<?php if ($id_hang==8): ?>
			<p class="w_100 sh_clr_two sh_size_three mb_5 cr_weight">Loại phụ kiện </p>
			<input type="text" name="loai75" class="m_timkiem_input" autocomplete="off" placeholder="Nhập loại phụ kiện">
		<?php endif ?>
	<?php endif ?>
	...
	<?php if ($id_dm==6): ?>
		<?php if ($id_hang==34): ?>
			...
			<input type="text" name="hang" class="m_timkiem_input" autocomplete="off" placeholder="Nhập hãng">
		<?php endif ?>
		<?php if ($id_hang!=34): ?>
			<p class="w_100 sh_clr_two sh_size_three mb_5 cr_weight">Hãng</p>
			<?
	...
	<?php if ($id_dm==9): ?>
		...
		<?php if ($id_hang == 1286): ?>
			<p class="w_100 sh_clr_two sh_size_three mb_5 cr_weight">Dòng xe</p>
			<input type="text" name="dong_xe" class="m_timkiem_input" autocomplete="off" placeholder="Nhập dòng xe">
		<?php endif ?>
	<?php endif ?>
	...
	<?php if ($id_dm==81): ?>
		...
		<?php if ($id_hang == 2060): ?>
			<?
			$query = new db_query("SELECT * FROM nhom_sanpham_hinhdang WHERE id_cha=".$id_hang."  and id_danhmuc=".$id_dm."");
        	$hinhdangque = $query->result_array();
			?>
			<div class="thuoc_tinh w_100 mb_20 ">
				<p class="w_100 sh_clr_two sh_size_three mb_5 cr_weight">Hình dáng</p>
				<?
				...
				?>
			        <select name="hinhdang" class="form-control share_select2 w_100">
			            <option value="">Hình dáng</option>
			           	<?php foreach ($hinhdangque as $key => $value): ?>
			                <option value="<?=$value['id']?>"><?=$value['name']?></option>    
			            <?php endforeach ?>
			        </select>
	    	</div>
