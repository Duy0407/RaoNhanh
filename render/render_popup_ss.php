...
//     $user_id = $_COOKIE['UID'];
//     $type_us = $_COOKIE['UT'];
//     $query = new db_query("SELECT `id`, `new_id`, `new_type`, `user_id`, `type`, `cat_id`, `loai_sp`, `nhom_sp` FROM `bang_so_sanh` WHERE `user_id` = $user_id AND  `type` = $type_us ");
//     $result = $query->result_array();
//     foreach ($result as $val) {
//         $arr_item3 = [];
//         $arr_item3['new_id'] = $val['new_id'];
//         $arr_item3['loai_sp'] = $val['loai_sp'];
//         $arr_item3['nhom_sp'] = $val['nhom_sp'];
//         $arr_item3['new_type'] = $val['new_type'];
//         $arr_sp3[] = $arr_item3;
      
//     }
//     $arrSoSanh = [];
//     foreach ($arr_sp3 as $val) {
//         $querySp = new db_query("SELECT `new_id`, `new_title`, `new_money`, `gia_kt` ,`new_image`, `new_cate_id`, `new_create_time`, `new_unit`, `dia_chi`, `new_type`,`new_user_id`,`chotang_mphi` FROM `new` WHERE new_id ='" . $val['new_id'] . "'");
...
<? foreach ($arrSoSanh as $rows) :
    ...
    <div data="<?= $rows['new_id'] ?>" class="box_item_product d_flex">
        <div class="img_content">
            <div class="img_item">
                <img src="/pictures/<?= $img_tdnb1[0] ?>" alt="" onerror="this.src=`/images/anh_moi/dai_dien_avt.png`">
            </div>
            <div class="num_img">
                <? if ($img_tdnb2 > 1) { ?>
        ...
        <div class="text_content">
            ...
            <div class="combo_price d_flex">
                ...
                <? } ?>
                <div class="sosanh_item d_flex" data-id="<?= $rows['new_id'] ?>" data5="<?= search($arr_sp3, 'new_id', $rows['new_id'])[0]['nhom_sp'] ?>" data4="<?= $rows['new_user_id'] ?>" data3="<?= search($arr_sp3, 'new_id', $rows['new_id'])[0]['loai_sp'] ?>" data1="<?= search($arr_sp3, 'new_id', $rows['new_id'])[0]['new_type']  ?>" data2="<?= $rows['new_cate_id'] ?>" onclick="popup_ss(this)">
