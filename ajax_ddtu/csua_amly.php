        ...
        if ($cou_file > 0 || $cou_dang > 0) {
            ...
            if (mysql_num_rows($check_tkhoan->result) > 0) {
                ...
                if (mysql_num_rows($check_tde->result) > 0) {
                    ...
                    $update_des = new db_query("UPDATE `new_description` SET `new_description` = '$mo_ta', `thiet_bi` = '$thiet_bi', `hang` = '$hang',
                                            `knoi_internet` = '$ketnoi_inter', `do_phan_giai` = '$do_phangiai', `cong_suat` = '$cong_suat', `loai_chung` = '$loai',
                                            `man_hinh` = '$kich_thuoc', `canhan_moigioi` = '$canhan_moigioi', `xuat_xu` = '$xuat_xu' WHERE `new_id` = $id_cs ");

                    // fastcgi_finish_request();

                    // $tt_edit = 'đồ điện tử tivi, âm thanh: ' . $id_cs;
