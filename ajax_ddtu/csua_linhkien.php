      ...
      if ($cou_file > 0 || $cou_dang > 0) {
         ...
         if (mysql_num_rows($check_tkhoan->result) > 0) {
            ...
            if (mysql_num_rows($check_tde->result) > 0) {
               ...
               $update_des = new db_query("UPDATE `new_description` SET `new_description` = '$mo_ta', `thiet_bi` = '$thiet_bi', `link_kien_phu_kien` = '2',
                                            `loai_linhphu_kien` = '$linh_kien', `canhan_moigioi` = '$canhan_moigioi' WHERE `new_id` = $id_cs ");
