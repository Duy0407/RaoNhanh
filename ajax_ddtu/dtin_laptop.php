...
if (isset($_COOKIE['UID'])) {
    ...
    $hang = getValue('hang_may', 'int', 'POST', '');

    $dong = getValue('dong_may', 'str', 'POST', '');
    $dong = removeEmoji($dong);
    $dong = sql_injection_rp($dong);

    $bovi_xuly = getValue('bovi_xuly', 'int', 'POST', 0);
    $ram = getValue('ram_lap', 'int', 'POST', 0);
    $o_cung = getValue('o_cung', 'int', 'POST', 0);
    $loai_ocung = getValue('loai_ocung', 'int', 'POST', 0);
    $card_mhinh = getValue('card_mhinh', 'int', 'POST', 0);
                ...
                if ($cou_file > 0 || $cou_dang > 0) {
                    ...
                    if (mysql_num_rows($check_tde->result) > 0) {
                        ...
                        $inser_td = new db_query("INSERT INTO `new`(`new_title`,`link_title`, `new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,
                                            ...
                                            VALUES ('$tieu_de','$tieu_de','$gia_spham','$id_dm',0,'$user_id',
                                            '$file_anh','$ngay_dang','$user_type','1','$don_vi','$user_name','$sdt_lhe','$email_lhe','$diachi_nban',
                        ...
                        $inser_des = new db_query("INSERT INTO `new_description`(`new_id`, `new_description`, `bovi_xuly`, `ram`, `o_cung`,`loai_o_cung`, `man_hinh`,
                                            `kich_co`, `hang`, `dong_may`,`canhan_moigioi`) VALUES ('$id_dt','$mo_ta','$bovi_xuly','$ram','$o_cung','$loai_ocung',
                                            '$card_mhinh','$kich_co','" . $hang . "','$dong','" . $canhan_moigioi . "')");
                        ...
                        //     'FromWeb' => $domain
                        // ];
                        // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa đăng tin đồ điện tử laptop trên " . $domain;
                        // $message = "Tôi tên là: " . $user_name . ", " . $phone . "" . $email . "tôi vừa đăng tin đồ điện tử laptop trên " . $domain;
                        // $data_send_to_group = [
                        //     'LiveChat' => $LiveChat,
