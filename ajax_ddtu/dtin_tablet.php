...
if (isset($_COOKIE['UID'])) {
    ...
    $dong_may = getValue('dong', 'str', 'POST', '');
    $dong_may = removeEmoji($dong_may);
    $dong_may = sql_injection_rp($dong_may);

    $phien_ban = getValue('phien_ban', 'int', 'POST', 0);
    $sdung_sim = getValue('sdung_sim', 'int', 'POST', 0);
    ...
    $email_lhe = sql_injection_rp($email_lhe);
                ...
                if ($cou_file > 0 || $cou_dang > 0) {
                    ...
                    if (mysql_num_rows($check_tde->result) > 0) {
                        ...
                        $inser_td = new db_query("INSERT INTO `new`(`new_title`,`link_title`, `new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,
                                            ...
                                            '$user_id','$file_anh','$ngay_dang','$user_type','1','$don_vi','$user_name','$sdt_lhe','$email_ht','$diachi_nban',
                        ...
                        $inser_des = new db_query("INSERT INTO `new_description`(`new_id`, `new_description`, `hang`, `dong_may`, `man_hinh`,
                                            `dung_luong`,`sdung_sim`,`canhan_moigioi`,`phien_ban`) VALUES ('$id_dt','$mo_ta','$hang','$dong_may',
                                            '$kich_co','$dung_luong','$sdung_sim','$canhan_moigioi','$phien_ban')");
                        include('../ajax/xac_thuc_chung.php');

                        fastcgi_finish_request();

                        $tt_crea = 'Đồ điện tử máy tính bảng';
