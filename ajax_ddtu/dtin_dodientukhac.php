                ...
                if ($cou_file > 0 || $cou_dang > 0) {
                    ...
                    if (mysql_num_rows($check_tde->result) > 0) {
                        ...
                        $inser_td = new db_query("INSERT INTO `new`(`new_title`,`link_title`, `new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,
                                    ...
                                    `quan_huyen`, `phuong_xa`, `new_sonha`, `dia_chi`, `new_video`, `new_ctiet_dmuc`,`new_tinhtrang`,`new_baohanh`, `new_buy_sell`,
                                    `new_update_time`) VALUES ('$tieu_de','$link_title','$gia_spham','$id_dm',0,'$user_id','$file_anh','$ngay_dang','$user_type',
                                    '1','$don_vi','$user_name','$sdt_lhe','$email_lhe','$diachi_nban','$tang_mphi',0,0,'','$diachi_nban','$file_video','$chitiet_dm',
                                    '$tinh_trang','$bao_hanh',2,'$ngay_dang')");

                        $newid     = new db_query("SELECT LAST_INSERT_ID() AS id_dt");
                        $id_dt     = mysql_fetch_assoc($newid->result)['id_dt'];

                        $inser_des = new db_query("INSERT INTO `new_description`(`new_id`, `new_description`) VALUES ('$id_dt','$mo_ta')");
                        ...
                        //     'FromWeb' => $domain
                        // ];
                        // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa đăng tin đồ điện tử khác trên " . $domain;
                        // $message = "Tôi tên là: " . $user_name . ", " . $phone . "" . $email . "tôi vừa đăng tin đồ điện tử khác trên " . $domain;
                        // $data_send_to_group = [
                        //     'LiveChat' => $LiveChat,
                        ...
                        $tt_crea = 'Đồ điện tử - đồ điện tử khác';
