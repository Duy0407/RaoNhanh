        ...
        if ($cou_file > 0 || $cou_dang > 0) {
            ...
            if (mysql_num_rows($check_tkhoan->result) > 0) {
                ...
                if (mysql_num_rows($check_tde->result) > 0) {
                    ...
                    luu_index(0, $quan_huyen, $tinh_thanh, 0, 0, $arrcity2, $tags_tk1, $db_tags_vl, $db_cat_vl, $db_tkhoadb3);
                    $anh = '';
                    ...
                    $update_des = new db_query("UPDATE `new_description` SET `new_description` = '$mo_ta', `canhan_moigioi` = '$canhan_moigioi' WHERE `new_id` = $id_cs ");

                    // fastcgi_finish_request();

                    // $tt_edit = 'đồ điện tử khác: ' . $id_cs;
