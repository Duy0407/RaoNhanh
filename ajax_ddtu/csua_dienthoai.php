        ...
        if ($cou_file > 0 || $cou_dang > 0) {
            ...
            if (mysql_num_rows($check_tkhoan->result) > 0) {
                ...
                if (mysql_num_rows($check_tde->result) > 0) {
                    ...
                    $update_des = new db_query("UPDATE `new_description` SET `new_description` = '$mo_ta', `hang` = '$hang', `dong_may` = '$dong',
                                            `mau_sac` = '$mau_sac', `dung_luong` = '$dung_luong', `phien_ban` = '$phien_ban', `canhan_moigioi` = '$canhan_moigioi'
                                            WHERE `new_id` = $id_cs ");

                    fastcgi_finish_request();

                    // $tt_edit = 'đồ điện tử điện thoại: ' . $id_cs;
