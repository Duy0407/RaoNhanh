...
if (isset($_COOKIE['UID'])) {
    ...
    $hang = getValue('hang_may', 'int', 'POST', 0);
            ...
            if (mysql_num_rows($check_tkhoan->result) > 0) {
                ...
                if (mysql_num_rows($check_tde->result) > 0) {
                    ...
                    if ($dia_chi != '') {
                        ...
                        for($i = 0; $i < count($diachi1); $i++){
                            if($diachi1[$i] != ''){
                                $address = get_infor_from_address(rtrim($diachi1[$i], ','));
                    ...
                    $update_des = new db_query("UPDATE `new_description` SET `new_description` = '$mo_ta', `bovi_xuly` = '$bovi_xuly', `ram` = '$ram',
                                            `o_cung` = '$o_cung', `loai_o_cung` = '$loai_ocung', `man_hinh` = '$card_mhinh', `kich_co` = '$kich_co',
                                            `hang` = '$hang', `dong_may` = '$dong', `canhan_moigioi` = '$canhan_moigioi' WHERE `new_id` = $id_cs ");

                    // fastcgi_finish_request();

                    // $tt_edit = 'đồ điện tử laptop: ' . $id_cs;
