...
if (isset($_COOKIE['UID'])) {
    ...
    $ram = getValue('ram_pc', 'int', 'POST', 0);
                ...
                if ($cou_file > 0 || $cou_dang > 0) {
                    ...
                    if (mysql_num_rows($check_tde->result) > 0) {
                        ...
                        $inser_td = new db_query("INSERT INTO `new`(`new_title`,`link_title`, `new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,
                                    ...
                                    '$ngay_dang','$user_type',1,'$don_vi','$user_name','$sdt_lhe','$email_lhe','$diachi_nban','$tang_mphi',0,0,'','$diachi_nban',
                        ...
                        $inser_des = new db_query("INSERT INTO `new_description`(`new_id`, `new_description`, `bovi_xuly`, `ram`, `o_cung`,
                                `loai_o_cung`, `man_hinh`, `kich_co`, `canhan_moigioi`) VALUES ('$id_dt','$mo_ta','$bovi_xuly','$ram','$o_cung','$loai_ocung',
                                '$card_mhinh','$kich_co', '$canhan_moigioi')");
                        include('../ajax/xac_thuc_chung.php');

                        // fastcgi_finish_request();
                        // $tt_crea = 'Đồ điện tử máy tính để bàn';
