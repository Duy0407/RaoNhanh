...
if (isset($_COOKIE['UID'])) {
    ...
    $hang_may = getValue('hang_may', 'str', 'POST', '');

    $dong_may = getValue('dong_may', 'str', 'POST', '');
    $dong_may = removeEmoji($dong_may);
    $dong_may = sql_injection_rp($dong_may);

    $phien_ban = getValue('phien_ban', 'int', 'POST', '');
    $dung_luong = getValue('dung_luong', 'int', 'POST', 0);
                ...
                if ($cou_file > 0 || $cou_dang > 0) {
                    ...
                    if (mysql_num_rows($check_tde->result) > 0) {
                        ...
                        $inser_td = new db_query("INSERT INTO `new`(`new_title`,`link_title`, `new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,
                                                ...
                                                VALUES ('$tieu_de','$link_title','$gia_spham','$id_dm',0,'$user_id',
                                                '$file_anh','$ngay_dang','$user_type','1','$don_vi','$user_name','$sdt_lhe','$email_lhe','$diachi_nban','$tang_mphi',
                                                0,0,'','$diachi_nban','$file_video','$chitiet_dm','$tinh_trang','$bao_hanh',2,'$ngay_dang'
                        ...
                        $inser_des = new db_query("INSERT INTO `new_description`(`new_id`, `new_description`, `hang`, `dong_may`, `mau_sac`,`dung_luong`,
                                                `canhan_moigioi`,`phien_ban`) VALUES ('$id_dt','$mo_ta','$hang_may','$dong_may','$mau_sac','$dung_luong',
                                                '" . $canhan_moigioi . "', '" . $phien_ban . "')");
                        ...
                        //     'FromWeb' => $domain
                        // ];
                        // $MessageInforSupport = "Xin chào, ". $usc_company . "" . $phone . "" . $email . "tôi vừa đăng tin đồ điện tử điện thoại trên " . $domain;
                        // $message = "Tôi tên là: " . $user_name . ", " . $phone . "" . $email . "tôi vừa đăng tin đồ điện tử điện thoại trên " . $domain;
                        // $data_send_to_group = [
                        //     'LiveChat' => $LiveChat,
