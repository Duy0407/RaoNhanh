...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT'])) {
    ...
    $hang = removeEmoji($hang);
    $hang = sql_injection_rp($hang);
            ...
            if ($user_id != 0 && $user_type != 0 && $id_dm != 0 && $tieu_de != '') {
                if ($cou_file > 0 || $cou_dang > 0) {
                    ...
                    if (mysql_num_rows($check_tde->result) > 0) {
                        ...
                        $inser_des = new db_query("INSERT INTO `new_description`(`new_id`, `new_description`,`hang`,`dong_may`,`thiet_bi`, `canhan_moigioi`)
                                VALUES ('$id_dt','$mo_ta','$hang','$dong','$thiet_bi', '$canhan_moigioi')");
                        ...
                        //     'FromWeb' => $domain
                        // ];
                        // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa đăng tin đồ điện tử thiết bị đeo thông minh trên " . $domain;
                        // $message = "Tôi tên là: " . $user_name . ", " . $phone . "" . $email . "tôi vừa đăng tin đồ điện tử thiết bị thông minh trên " . $domain;
                        // $data_send_to_group = [
                        //     'LiveChat' => $LiveChat,
                        ...
                        $tt_crea = 'Đồ điện tử thiết bị đeo thông minh';
