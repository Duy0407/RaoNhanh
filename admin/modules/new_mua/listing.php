...
$db_cateogry = new db_query("SELECT cat_type,cat_name,cat_id FROM category WHERE cat_parent_id = 0 ORDER BY cat_type");
...
$total      = new db_count("SELECT count(*) AS count FROM " . $fs_table . " JOIN category ON new.new_cate_id = category.cat_id LEFT JOIN user ON new.new_user_id = user.usc_id
									 WHERE new_buy_sell = 1 " . $sql);

$total_row = $total->total;

$db_listing = new db_query("SELECT * FROM " . $fs_table . " JOIN category ON new.new_cate_id = category.cat_id LEFT JOIN user ON new.new_user_id = user.usc_id
									 WHERE new_buy_sell = 1 " . $sql . " ORDER BY " . $list->sqlSort() . $field_id . " DESC " . $list->limit($total_row));
...
<body>
   <div id="listing">
      ...
      while ($row = mysql_fetch_assoc($db_listing->result)) {
         ...
         $urlNews = replaceTitle($row['link_title']) . "-ct" . $row['new_id'] . ".html";
      ?>
