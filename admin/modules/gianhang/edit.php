...
$myform->add("usc_store_name", "usc_store_name", 0, 0, "", 1, "Bạn chưa nhập tên tài khoản", 0, "");
...
if ($action == "submitForm") {
	if ($_POST['usc_money2'] != "" && $_POST['usc_money2'] != 0) {
		if ($_POST['usc_money2'] < 0) {
			$errorMsg .= "Số tiền trừ phải lớn hơn 0";
		} else {
			if ($_POST['usc_money2'] > $taik_stien) {
	...
	if ($errorMsg == "") {
		...
		// them tien vao tk
		if (isset($_POST['usc_money1']) && $_POST['usc_money1'] != 0 && $_POST['usc_money1'] != "") {
			$tien_them = $_POST['usc_money1'];
			$thoi_gian = time();
			$noi_dung = "Nạp tiền";
			$inser_tthem = new db_query("UPDATE `user` SET `usc_money`= usc_money + $tien_them  WHERE `usc_id`= $record_id AND `usc_type`= 5 ");

			$inser_lsu = new db_query("INSERT INTO `history`(`his_id`, `his_user_id`, `his_price`, `his_price_suc`, `his_time`, `his_type`, `noi_dung`,`his_pb`)
                            VALUES ('','$record_id','$tien_them','$tien_them','$thoi_gian','5','$noi_dung','0')");
		}
		// tru tien tai khoan
		if (isset($_POST['usc_money2']) && $_POST['usc_money2'] != 0 && $_POST['usc_money2'] != "" && $_POST['usc_money2'] < $taik_stien) {
			$tien_tru = $_POST['usc_money2'];
			$inser_tthem = new db_query("UPDATE `user` SET `usc_money`= usc_money - $tien_tru  WHERE `usc_id`= $record_id AND `usc_type`= 5 ");
		}
...
<body topmargin="0" bottommargin="0" leftmargin="0" rightmargin="0">
	...
	<?= $form->text("Tên doanh nghiệp", "usc_store_name", "usc_store_name", $usc_store_name, "Tên tài khoản", 1, 250, "", 255) ?>
