...
<?
	class upload_file{
		...
		var $file_size			= 0;
		var $original_name	= "";
			function upload_file($upload_name, $upload_path, $extension_list, $limit_size, $check_image=0, $prefix="", $zip=0){
		// Nếu $upload_name == "" thì return luôn để phục vụ resize image
		if($upload_name == "") return;
		...
		if ($this->CheckFilePDF($_FILES[$upload_name]) == false){
	        $this->common_error	= '<p>Tệp file này không hỗ trợ.</p>';    
	       	$this->warning_error		= $this->common_error;
			return;
	 	}
		//Generate new filename
    ...
    $fileExtensions = array("pdf", "doc", "docx", "xls", "xlsx", "ppt", "pptx");
...
<?
	class upload_video{
		...
		var $file_size			= 0;
		var $original_name	= "";
			function upload_video($upload_name, $upload_path, $extension_list, $limit_size, $check_image=0, $prefix="", $zip=0){
		// Nếu $upload_name == "" thì return luôn để phục vụ resize image
		if($upload_name == "") return;
		...
		if ($this->CheckFileVideo($_FILES[$upload_name]["name"]) == true){
	        $this->common_error	= '<p>Trình tải video không hỗ trợ tệp file này.</p>';    
	       	$this->warning_error		= $this->common_error;
			return;
	 	}
		//Generate new filename
	...
	function CheckFileVideo ($file){
		$video_info = pathinfo($file);
    	$image_extension = strtolower($video_info["extension"]);
	    if(
	         $image_extension != "mp4"  &&
	         $image_extension != "avi"  &&
	         $image_extension != "mov"  &&
	         $image_extension != "3gp"  &&
	         $image_extension != "mpeg"
	      ){
	    	return true;
	    }
