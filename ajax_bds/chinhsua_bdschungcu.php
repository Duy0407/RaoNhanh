...
if (isset($_COOKIE['UID'])) {
    ...
    $id_cc = getValue('id_cc', 'int', 'POST', 0);
    ...
    $huong_ban_cong = getValue('huong_ban_cong', 'int', 'POST', 0);
    ...
    if ($user_id != 0 && $user_type != 0 && $tieu_de != "" && $id_cc != 0) {
        if ($cou_file > 0 || $cou_dang > 0) {
            ...
            if (mysql_num_rows($query_check->result) > 0) {
                ...
                $query_dt = new db_query("UPDATE `new` SET `new_title`='$tieu_de',`new_money`='$td_gia_spham', `new_city`='$tinh_thanh', `new_image`='$file_anh',
                                        ...
                                        `dc_unit`='$dvi_dc' WHERE new_id = $id_cc AND new_user_id = $user_id ");

                $query_des = new db_query("UPDATE `new_description` SET `new_description`='$mo_ta', `can_ban_mua`='$ban_thue', `ten_toa_nha`='$ten_toanha',
                                        `loai_hinh_canho`='$loai_hinh', `tinh_trang_bds`='$tinh_trang', `tang_so`='$tang_so', `so_pngu`='$so_phongngu',
                                        `so_pve_sinh`='$so_phong_vs' ,`huong_chinh`='$huong_cua', `huong_ban_cong`='$huong_ban_cong', `giay_to_phap_ly`='$giayto',
                                        `tinh_trang_noi_that`='$tinhtrangnt', `dien_tich`='$dientich', `chieu_dai`='$chieudai', `chieu_rong`='$chieungang',
                                        `td_block_thap` = '$td_block_thap', `canhan_moigioi`='$canhan_moigioi', `td_macanho`='$td_macan', `td_htmch_rt`='$td_htmch_rt',
                                        `cangoc`='$td_cangoc' WHERE new_id = $id_cc");
                ...
                //     'FromWeb' => $domain
                // ];
                // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa chỉnh sửa tin đăng bất động sản chung cư: " . $id_cc . " trên " . $domain;
                // $data_send_to_group = [
                //     'LiveChat' => $LiveChat,
