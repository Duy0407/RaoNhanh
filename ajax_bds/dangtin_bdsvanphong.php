...
if (isset($_COOKIE['UID'])) {
    ...
    $so_tang = getValue('so_tang', 'int', 'POST', 0);
    ...
    $td_block_thap = getValue('td_block_thap', 'str', 'POST', 0);
                ...
                if ($cou_file > 0 || $cou_dang > 0) {
                    ...
                    if (mysql_num_rows($query_check->result) > 0) {
                        ...
                        $query_dt = new db_query("INSERT INTO `new`(`new_title`,`link_title`,`new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,
                                            `new_create_time`, `new_type`,`new_buy_sell`,`new_active`,`new_unit`, `new_name`, `new_phone`, `new_email`, `new_address`,
                                            ...
                                            '$new_buy_sell',1,'$dvi_tien','$user_name','$sdt_lhe','$email_lhe','$dia_chi',0,'$quan_huyen','$phuong_xa',
                                            '" . $so_nha . "','" . $td_diachi_nha . "','$file_video','$ctiet_dmuc','$ngay_dang')");
                        ...
                        $query_decription = new db_query("INSERT INTO `new_description`( `new_id`, `new_description`,`can_ban_mua`, `ten_toa_nha`,
                                    `huong_chinh`, `giay_to_phap_ly`, `tinh_trang_noi_that`, `dien_tich`, `chieu_dai`, `chieu_rong`, `loaihinh_vp`,`tang_so`,
                                    `td_block_thap`,`canhan_moigioi`,`td_macanho`,`td_htmch_rt`,`dac_diem`)
                                    VALUES ('$id','$mo_ta','$ban_thue','$ten_toanha','$huong_cua','$giayto','$tinhtrangnt','$dientich','$chieudai','$chieungang',
                                    '$loai_hinh','$tang_so','$td_block_thap','$canhan_moigioi','$td_macan','$td_htmch_rt','$dacdiem')");
                        ...
                        //     'FromWeb' => $domain
                        // ];
                        // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa đăng tin bất động sản văn phòng trên " . $domain;
                        // $data_send_to_group = [
                        //     'LiveChat' => $LiveChat,
