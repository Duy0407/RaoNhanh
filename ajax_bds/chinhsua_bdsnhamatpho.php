...
if (isset($_COOKIE['UID'])) {
    ...
    $dac_diem = getValue('dac_diem', 'int', 'POST', 0);
    ...
    if ($user_id != 0 && $id_nd != 0 && $tieu_de != "") {
        if ($cou_file > 0 || $cou_dang > 0) {
            ...
            if (mysql_num_rows($query_check->result) > 0) {
                ...
                $query_nd = new db_query("UPDATE `new` SET `new_title`='$tieu_de',`new_money`='$td_gia_spham', `new_city`='$tinh_thanh',
                                `new_image`='$file_anh', `new_update_time`='$ngay_dang', `new_unit`='$dvi_tien',`quan_huyen`='$quan_huyen',
                                `phuong_xa`='$phuong_xa', `new_sonha`='" . $so_nha . "', `dia_chi`='" . $dia_chi . "', `new_video`='$file_video',
                                `new_ctiet_dmuc`='$ctiet_dmuc', `new_phone` = '$sdt_lhe', `new_email` = '$email_lhe' WHERE new_id = $id_nd");
                ...
                $query_decription = new db_query("UPDATE `new_description` SET  `new_description`='$mo_ta',`can_ban_mua`='$ban_thue', `ten_toa_nha`='$ten_toanha',
                                                ...
                                                `giay_to_phap_ly`='$giayto', `tinh_trang_noi_that`='$tinhtrangnt', `dac_diem`='$dac_diem', `dien_tich`='$dientich',
                                                `chieu_dai`='$chieudai', `chieu_rong`='$chieungang' WHERE new_id = $id_nd ");
                ...
                $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa chỉnh sửa tin đăng bất động sản nhà mặt phố: " . $id_nd . " trên " . $domain;
                $data_send_to_group = [
                    'LiveChat' => $LiveChat,
