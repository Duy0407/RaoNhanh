        ...
        if ($cou_file > 0 || $cou_dang > 0) {
            ...
            if (mysql_num_rows($query_check->result) > 0) {
                ...
                $query_decription = new db_query("UPDATE `new_description` SET  `new_description`='$mo_ta',`can_ban_mua`='$ban_thue', `ten_toa_nha`='$ten_toanha',
                                                ...
                                                `td_htmch_rt` = '$td_htmch_rt', `dientichsd`='$dientichsudung', `canhan_moigioi`='$canhan_moigioi'
                                                WHERE new_id = $id_nd ");
                ...
                $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa chỉnh sửa tin đăng bất động sản nhà riêng, nguyên căn: " . $id_nd . " trên " . $domain;
                $data_send_to_group = [
                    'LiveChat' => $LiveChat,
