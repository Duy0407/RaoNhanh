        ...
        // if ((time() - $tieptuc_dt) > 600) {
            if ($user_id != 0 && $user_type != 0 && $tieu_de != "") {
                if ($cou_file > 0 || $cou_dang > 0) {
                    $query_check1 = new db_query("SELECT `new_id` FROM `new` WHERE `new_user_id` = $user_id AND `new_title` = '$tieu_de' LIMIT 1  ");
                    if (mysql_num_rows($query_check1->result) > 0) {
                        ...
                        $query_dt = new db_query("INSERT INTO `new`(`new_title`,`link_title`,`new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`, `new_create_time`,
                             ...
                             `phuong_xa`, `new_sonha`, `dia_chi`, `new_video`, `new_ctiet_dmuc`,`new_update_time`) VALUES ('$tieu_de','$tieu_de','$td_gia_spham','$cate_id',
                             '$tinh_thanh',
                             '$user_id','$file_anh','$ngay_dang','$user_type','$new_buy_sell', 1,'$dvi_tien','$user_name','$sdt_lhe','$email_lhe','$dia_chi',
                             0,'$quan_huyen','$phuong_xa','" . $so_nha . "','" . $td_diachi_nha . "','$file_video','$ctiet_dmuc','$ngay_dang')");

                        $id_des = new db_query("SELECT LAST_INSERT_ID() AS id");
                        $id = mysql_fetch_assoc($id_des->result)['id'];

                        $query_decription = new db_query("INSERT INTO `new_description`(`new_id`, `new_description`,`can_ban_mua`,`ten_toa_nha`,`loai_hinh_dat`,
                                                `huong_chinh`, `giay_to_phap_ly`, `dien_tich`, `chieu_dai`,`chieu_rong`,`ten_phan_khu`,`td_macanho`,`dac_diem`,
                                                `canhan_moigioi`,`td_htmch_rt`) VALUES ('$id','$mo_ta','$ban_thue','$ten_du_an','$loai_hinh','$huongdat','$giayto',
                                                '$dientich','$chieudai','$chieungang','$td_tenpk_lo','$td_macan','$dacdiem','$canhan_moigioi','$td_htmch_rt')");
                        ...
                        //     'FromWeb' => $domain
                        // ];
                        // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa đăng tin bất động sản đất trên " . $domain;
                        // $data_send_to_group = [
                        //     'LiveChat' => $LiveChat,
