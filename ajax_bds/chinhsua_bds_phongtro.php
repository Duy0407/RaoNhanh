    ...
    if ($user_id != 0 && $id_nd != 0 && $tieu_de != "") {
        if ($cou_file > 0 || $cou_dang > 0) {
            ...
            if (mysql_num_rows($query_check->result) > 0) {
                ...
                $query_decription = new db_query("UPDATE `new_description` SET  `new_description`='$mo_ta', `tinh_trang_noi_that`='$tinhtrangnt', `dien_tich`='$dientich',
                                            `chieu_dai`='$chieudai', `chieu_rong`='$chieungang', `canhan_moigioi`='$canhan_moigioi' WHERE new_id = $id_nd ");
