...
if ($_COOKIE['UID']) {
    ...
    $so_tang = getValue('tongso_tang', 'int', 'POST', 0);
            ...
            if ($cou_file > 0 || $cou_dang > 0) {
                ...
                if (mysql_num_rows($query_check->result) > 0) {
                    ...
                    $query_dt = new db_query("INSERT INTO `new`(`new_title`,`link_title`,`new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,
                                                ...
                                                `new_update_time`,`datcoc`,`dc_unit`) VALUES ('$tieu_de','$tieu_de','$td_gia_spham','$cate_id','$tinh_thanh','$user_id','$file_anh',
                                                '$ngay_dang','$user_type', '$new_buy_sell',1,'$dvi_tien','$user_name','$sdt_lhe','$email_lhe',
                                                '$dia_chi',0,'$quan_huyen','$phuong_xa','" . $so_nha . "','" . $td_diachi_nha . "','$file_video','$ctiet_dmuc',
                                                '$ngay_dang','$tien_datcoc','$dc_unit')");

                    $id_des = new db_query("SELECT LAST_INSERT_ID() AS id");
                    $id = mysql_fetch_assoc($id_des->result)['id'];

                    $query_decription = new db_query("INSERT INTO `new_description`(`new_id`, `new_description`,`can_ban_mua`,`ten_toa_nha`,`tong_so_tang`,
                                                    `huong_chinh`, `giay_to_phap_ly`, `dac_diem`,`dien_tich`, `chieu_dai`, `chieu_rong`,`canhan_moigioi`,
                                                    `tinh_trang_noi_that`) VALUES ('$id','$mo_ta','$ban_thue','$ten_toanha','$so_tang','$huong_cua','$giayto',
                                                    '$dacdiem','$dientich','$chieudai','$chieungang','$canhan_moigioi','$tinhtrangnt')");
                    ...
                    //     'FromWeb' => $domain
                    // ];
                    // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa đăng tin bất động sản cửa hàng trên " . $domain;
                    // $data_send_to_group = [
                    //     'LiveChat' => $LiveChat,
                    ...
                    //     'new_tinhtrang' => $tinhtrangnt, // Tình trạng nội thất
                    //     'dien_tich' => $dientich,
