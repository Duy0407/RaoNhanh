...
if (isset($_COOKIE['UID'])) {
    ...
    $so_nhavs = getValue('so_phong_vs', 'int', 'POST', 0);
    ...
    $dac_diem = getValue('dacdiem', 'str', 'POST', '');
        ...
        if ($cou_file > 0 || $cou_dang > 0) {
            ...
            if (mysql_num_rows($query_check->result) > 0) {
                ...
                $query_nd = new db_query("UPDATE `new` SET `new_title`='$tieu_de',`new_money`='$td_gia_spham', `new_city`='$tinh_thanh', `new_image`='$file_anh',
                                        ...
                                        `new_phone`='$sdt_lhe', `new_email`='$email_lhe', `datcoc`='$tien_datcoc', `dc_unit`='$dc_new_unit', `new_address`='$td_diachi_nha'
                ...
                $query_decription = new db_query("UPDATE `new_description` SET  `new_description`='$mo_ta', `can_ban_mua`='$ban_thue', `ten_toa_nha`='$ten_toanha',
                                            `tong_so_tang`='$so_tang', `so_pve_sinh`='$so_nhavs', `so_pngu`='$so_phongngu', `huong_chinh`='$huong_cua',
                ...
                //     'FromWeb' => $domain
                // ];
                // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa chỉnh sửa tin đăng bất động sản nhà đất: " . $id_nd . " trên " . $domain;
                // $data_send_to_group = [
                //     'LiveChat' => $LiveChat,
