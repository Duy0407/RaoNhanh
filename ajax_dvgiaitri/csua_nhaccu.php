...
if (isset($_COOKIE['UID'])) {
    ...
    $loai_ncu = getValue('loai_ncu', 'int', 'POST', 0);
            ...
            if (mysql_num_rows($check_tkhoan->result) > 0) {
                ...
                if (mysql_num_rows($check_tde->result) > 0) {
                    ...
                    for ($i = 0; $i < $cou_file; $i++) {
                        ...
                        $file_name = '/pictures/dichvu_giaitri/' . time() . '_' . $_FILES['files']['name'][$i];
                        $anh .= $file_name . ';';
                    ...
                    $updat_td = new db_query("UPDATE `new` SET `new_title` = '$tieu_de', `new_money` = '$gia_spham', `new_city` = 0, `new_image` = '$file_anh',
                                            `new_unit` = '$don_vi', `chotang_mphi` = '$tang_mphi', `quan_huyen` = 0, `phuong_xa` = 0,`new_tinhtrang` = '$tinh_trang',
                                            `new_sonha`='', `dia_chi` = '" . $dia_chi . "', `new_video` = '$file_video', `new_ctiet_dmuc` = '$ctiet_dmuc',
                    ...
                    $update_des = new db_query("UPDATE `new_description` SET `new_description` = '$mo_ta', `loai_chung` = '$loai_ncu' , `canhan_moigioi` = '$canhan_moigioi' WHERE `new_id` = $id_nd ");
                    ...
                    //     'FromWeb' => $domain
                    // ];
                    // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa chỉnh sửa tin đăng dịch vụ giải trí nhạc cụ: " . $id_nd . " trên " . $domain;
                    // $data_send_to_group = [
                    //     'LiveChat' => $LiveChat,
