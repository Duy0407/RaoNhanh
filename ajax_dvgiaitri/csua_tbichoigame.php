            ...
            if (mysql_num_rows($check_tkhoan->result) > 0) {
                ...
                if (mysql_num_rows($check_tde->result) > 0) {
                    ...
                    $updat_td = new db_query("UPDATE `new` SET `new_title` = '$tieu_de', `new_money` = '$gia_spham', `new_city` = 0, `new_image` = '$file_anh',
                                            ...
                                            `new_update_time` = '$ngay_dang', `new_tinhtrang` = '$tinh_trang',`new_baohanh`='$bao_hanh', `new_address` = '" . $dia_chi . "',
                    ...
                    $update_des = new db_query("UPDATE `new_description` SET `new_description` = '$mo_ta', `hang` = '$hang', `canhan_moigioi`='$canhan_moigioi' WHERE `new_id` = $id_nd ");
                    ...
                    //     'FromWeb' => $domain
                    // ];
                    // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa chỉnh sửa tin đăng thiết bị chơi game: " . $id_nd . " trên " . $domain;
                    // $data_send_to_group = [
                    //     'LiveChat' => $LiveChat,
