...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT'])) {
    ...
    $list_tpham = new db_query("SELECT `id`, `ten_loai` FROM `loai_chung` WHERE `id_danhmuc` = $id ");
...
<head>
    ...
    <title>Đăng tin thực phẩm</title>
        ...
        <div class="tindang-container">
            ...
            <div class="tindang-content-cont hd-disflex">
                ...
                <div class="tindang-col-right" data="<?= $hom_nay ?>">
                    <form class="form-dtin-cont" id="form_thucpham" data="<?= $us_id ?>" data1="<?= $us_type ?>">
                        <div class="row-tin-dang danhmuc_tt">
                            <p class="font-dam hd_font15-17">Danh mục sản phẩm <span class="color_red">*</span></p>
                            <input type="text" class="dmuc-spham" readonly name="dmuc_thuc_pham" data="<?= $id ?>" value="Thực phẩm đồ uống >> Thực phẩm">
                        </div>
                        ...
                        <div class="row-tin-dang them_class1 box_input_infor">
                            <p class="font-dam hd_font15-17">Loại thực phẩm <span class="color_red">*</span></p>
                            <select class="slect-hang hd_height36 loai_thucpham" name="loai_tp">
                                <option value="">Loại thực phẩm</option>
                                <? while ($row_tpham = mysql_fetch_assoc($list_tpham->result)) { ?>
                                    <option value="<?= $row_tpham['id'] ?>"><?= $row_tpham['ten_loai'] ?></option>
                                <? } ?>
                            </select>
                        </div>
                        <div class="row-tin-dang box_input_infor">
                            <p class="font-dam hd_font15-17">Hạn sử dụng<span class="color_red">*</span></p>
                            <input class="input_infor_tag error" type="date" name="han_sd" id="han_sd" placeholder="Nhập tiêu đề">
                        </div>
                        ...
                        <div class="row-tin-dang box_input_infor">
                            ...
                            <input readonly type="text" name="td_dia_chi" class="td_ttin_diachi input_infor_tag error" readonly placeholder="Địa chỉ">
                        </div>
                        <div class="row-tin-dang box_input_infor">
                        ...
                        <div class="row-tin-dang cont-btn-sb hd-disflex hide-480-mobile">
                            ...
                            <button type="button" id="xoa_tddang_tin" class="btn-submit td-dang-tin thuc_pham hd_cspointer font-bold">ĐĂNG TIN</button>
                        </div>
                    </form>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    $(".td-dang-tin").click(function() {
        var form_thucpham = $("#form_thucpham");
        form_thucpham.validate({
            ...
            messages: {
                ...
                loai_tp: "Vui lòng chọn loại thực phẩm",
                han_sd: {
        ...
        if (form_thucpham.valid() === true) {
            ...
            // end
            $.ajax({
                url: '/ajax/dangtin_thuc_pham.php',
                type: 'POST',
                data: fd,
                contentType: false,
                processData: false,
                beforeSend: function() {
                    $('.thuc_pham').prop('disabled', true);
                },
                success: function(data) {
                    if (data == "") {
                        tbao_dtin_tcong();
