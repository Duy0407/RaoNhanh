...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT'])) {
    ...
    $list_dh = new db_query("SELECT `dh_id`,`id_nguoi_dh`, `id_nguoi_ban`, `id_spham`, `ghi_chu`, `loai_ttoan`, `tien_ttoan`, `dia_chi_nhanhang`, `trang_thai`,
                            `usc_name`, `usc_logo`,`chat365_id`,`usc_id`,`usc_phone`, `usc_type`, `new_title`, `new_money`, `new_unit`, `new_user_id`, `new_type`, `new_image` FROM `dat_hang`
                            INNER JOIN `new` ON `new`.`new_id` = `dat_hang`.`id_spham`
                            INNER JOIN `user` ON `user`.`usc_id` = `dat_hang`.`id_nguoi_dh`
                            WHERE `trang_thai` = 3 AND `id_nguoi_ban` = $id_user ");
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="/css/newCss/box-left_pers_seller_prof.css?v=<?= $css_vs ?>" />
                            ...
                            while ($row_tin = mysql_fetch_assoc($list_dh->result)) {
                                ...
                                <div class="content-item-1">
                                    ...
                                    <div class="box-right-content-bot">
                                        <div class="bot-l">
                                            ...
                                            <div class="content-bot-c">
                                                <a class="ten_sp color_orange" href="/chi-tiet-da-giao-hang-<?= $row_tin['dh_id'] ?>.html">
                                                    <?= $row_tin['new_title'] ?>
                                                </a>
                                    ...
                                    <div class="main-t-btn-ql">
                                        <button class="xacnhan-4" data="<?= $row_tin['dh_id'] ?>">Hoàn tất đơn hàng</button>
                                        <button type="button" class="huydon-4" data="<?= $row_tin['dh_id'] ?>">Hủy đơn</button>
                                    </div>
                                </div>
                        <? }
                        } ?>
    ...
    <div id="modal-huydon" class="modal">
        <div class="modal-content-huydon">
            <div class="huydon-header">
                <p>Yêu cầu hủy đơn hàng</p>
                <span class="close">&times;</span>
            </div>
            <div class="huydon-content">
                <div class="huydon-content-item">
                    <p class="huydon-item-p1">Bạn xác nhận đã nhận được hàng hoàn trả của người mua ?</p>
                    <p class="huydon-item-p1">Ngay khi bạn xác nhận, số tiền thanh toán/ đặt cọc bởi người mua sẽ được hoàn trả lại người mua. </p>
                </div>
            </div>
            <div class="huydon-footer">
                <button type="button" class="modal-btn-huy" id="">Hủy bỏ</button>
                <button type="button" class="modal-btn-xacnhan" id="xacnhan_huydon" data="">Xác nhận</button>
            </div>
        </div>
    </div>

    <div id="modal_dathang" class="modal popup_dhang_tcong" style="display: block;">
...
<script type="text/javascript">
    $(".huydon-4").click(function() {
        ...
        $("#modal-huydon").find("#xacnhan_huydon").attr("data", id_dh);
        $("#modal-huydon").show();
    });
