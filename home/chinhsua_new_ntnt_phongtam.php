...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0) {
    ...
    $qr_listds = new db_query("SELECT `new`.`new_id`, `new_title`, `new_money`, `new_cate_id`, `new_city`, `new_image`, `new_unit`, `new_phone`, `new_email`,
                            ...
                            `new_description`, `loai_sanpham`, `hinhdang`, `hang`, `canhan_moigioi`,`thoigian_kmbd`,`thoigian_kmkt`,`soluong_min`,`soluong_max` FROM `new`
    ...
    if (mysql_num_rows($qr_listds->result) > 0) {
        ...
        $qr_loai = new db_query("SELECT `id`, `ten_loai` FROM `loai_chung` WHERE `id_danhmuc` = " . $id_dm);

        if ($item_td['loai_sanpham'] != 2064) {
            $qr_thieu = new db_query("SELECT `id`, `ten_hang` FROM hang WHERE id_parent=" . $item_td['loai_sanpham'] . "  AND id_danhmuc=" . $id_dm . "");
        ...
        if ($item_td['loai_sanpham'] == 2060) {
            $qr_hdang = new db_query("SELECT `id`, `name` FROM nhom_sanpham_hinhdang WHERE id_cha=" . $item_td['loai_sanpham'] . "  AND id_danhmuc=" . $id_dm . "");
        };
    } else {
        header('Location: /');
    }
...
<head>
    ...
    <title>Chỉnh sửa nội thất phòng tắm</title>
                        ...
                        <? if ($item_td['loai_sanpham'] != 2064) { ?>
                            <div class="d_flex fl_cl box_input_infor w_100 b6_nl_title">
                                ...
                                <select class="b6_fr1_select slect-hang hd_height36 thuong_hieu" name="thuong_hieu">
                                    ...
                                    <? while ($row_thieu = mysql_fetch_assoc($qr_thieu->result)) { ?>
                                        <option value="<?= $row_thieu['id'] ?>" <?= ($row_thieu['id'] == $item_td['hang']) ? 'selected' : '' ?>>
                                            <?= $row_thieu['ten_hang'] ?>
                                        </option>
                                    <? } ?>
                                </select>
                            ...
                            <div class="d_flex fl_cl box_input_infor w_100 b6_nl_title">
                                ...
                                <select class="b6_fr1_select slect-hang hd_height36 hinh_dang" name="hinh_dang">
                                    ...
                                    <? while ($row_hdang = mysql_fetch_assoc($qr_hdang->result)) { ?>
                                        <option value="<?= $row_hdang['id'] ?>" <?= ($row_hdang['id'] == $item_td['hinhdang']) ?>>
                                            <?= $row_hdang['name'] ?>
                                        </option>
                                    <? } ?>
                                </select>
    ...
    <script type="text/javascript">
        ...
        $(".b11_btn_chinhsua").click(function() {
            ...
            form_pc.validate({
                ...
                messages: {
                    ...
                    hinh_dang: {
                        required: "Vui lòng chọn hình dạng",
                    },
                    tinh_trang: {
            ...
            if (form_pc.valid() === true) {
                ...
                if (loai_spham != 2064) {
                    fd.append('thuong_hieu', $(".thuong_hieu").val());
                };
                if (loai_spham == 2060) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '/ajax_noithat_ngoaithat/csua_phongtam.php',
                    data: fd,
                    processData: false,
                    contentType: false,
                    success: function(data) {
        ...
        function xem_trc_tin() {
            ...
            if (form_pc.valid() === true) {
                ...
                fd.append('loai_sp2', hinh_dang);
                fd.append('loai_sp3', thuong_hieu);
                fd.append('tinh_trang', tinh_trang);
