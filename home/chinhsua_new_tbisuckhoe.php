...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0 && $id_dm != 0) {
    ...
    $list_tin = new db_query("SELECT `new`.`new_id`, `new_title`,`link_title`,`new_money`, `new_cate_id`, 
                                ...
                                `new_description`,`loai_thiet_bi`,`canhan_moigioi`
    ...
    if (mysql_num_rows($list_tin->result) > 0) {
        ...
        // tag danh muc
        $ban = new db_query("SELECT `tags_id`, `ten_tags` FROM `key_tags` WHERE `id_danhmuc` = 59 AND type_tags = '" . $item_td['can_ban_mua'] . "' ");
...
<head>
    ...
    <title>Chỉnh sửa Thiết bị sức khỏe </title>
...
<body>
    ...
    <section id="m_dangtin_dogiadung" class="suckhoe dogiadung">
        ...
        <form class="form-dtin-cont form_bds_chung" id="form_tb_suckhoe" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_nd ?>">
                ...
                <div class="b6_loaihinh loai_dungcu d_flex fl_cl box_input_infor">
                    ...
                    <select class="slect-hang hd_height36 b6_lhinh_select loai_thiet_bi" name="loai_thiet_bi">
                        ...
                        <? foreach ($db_nspham as $rows) {
                            if ($rows['id_danhmuc'] == 59) { ?>
                                <option value="<?= $rows['id'] ?>" <?= ($rows['id'] == $thiet_bi) ? "selected" : "" ?>><?= $rows['name'] ?></option>
                        <? }
                        } ?>
                    </select>
                </div>
    ...
    <script>
        $(".b11_btn_chinhsua").click(function() {
            ...
            if (form_tb_suckhoe.valid() === true) {
                ...
                var user_id = $("#form_tb_suckhoe").attr("data");
                var user_type = $("#form_tb_suckhoe").attr("data1");
                var id_nd = $("#form_tb_suckhoe").attr("data2");
    ...
    <script>
        ...
        function xem_trc_tin() {
            var form_tb_suckhoe = $(".form-dtin-cont");
            form_tb_suckhoe.validate({
                errorPlacement: function(error, element) {
                    error.appendTo(element.parents(".box_input_infor"));
                    error.wrap("<span class='error'>");
