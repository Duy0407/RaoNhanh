...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT'])) {
    ...
    $list_dh = new db_query("SELECT `dh_id`,`id_nguoi_dh`, `id_nguoi_ban`, `id_spham`, `ghi_chu`, `loai_ttoan`, `tien_ttoan`, `dia_chi_nhanhang`, `trang_thai`,
                            `usc_name`, `usc_logo`, `usc_type`, `chat365_id`, `usc_id`, `usc_phone`, `new_title`, `new_money`, `new_unit`, `new_user_id`, `new_type`, `new_image`
                            FROM `dat_hang`
                            INNER JOIN `new` ON `new`.`new_id` = `dat_hang`.`id_spham`
                            INNER JOIN `user` ON `user`.`usc_id` = `dat_hang`.`id_nguoi_dh`
                            WHERE `trang_thai` = 0 AND `id_nguoi_ban` = $id_user AND `dh_active` = 1 ORDER BY `tgian_xacnhan` DESC ");
                    ...
                    <div class="box-right-content">
                        <? if (mysql_num_rows($list_dh->result) > 0) {
                            while ($row_tin = mysql_fetch_assoc($list_dh->result)) {
                                ...
                                <div class="content-item-1 scroll-ngang">
                                    <div class="box-right-content-top">
                                        ...
                                        <div class="content-top-right">
                                            <div class="top-right-text">
                                                <p class="color_orange text_status">CHỜ XÁC NHẬN</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="box-right-content-rec"></div>
                                    <div class="box-right-content-bot">
                                        <a href="/chi-tiet-cho-xac-nhan-nguoi-ban-<?= $row_tin['dh_id'] ?>.html">
                                            <div class="bot-l">
                                    ...
                                    <div class="box-right-content-rec"></div>
                                    <div class="main-t-btn-ql-ql">
                                        <button type="button" class="xacnhan-orange" data="<?= $row_tin['dh_id'] ?>">Xác nhận</button>
                                        <button type="button" class="huydon-1-detail xac_nhan_hdon" data="<?= $row_tin['dh_id'] ?>">Hủy đơn</button>
                                    </div>
                                </div>
                        <? }
                        } ?>
