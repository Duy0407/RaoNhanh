...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_sksd != 0) {
    $acc_id = $_COOKIE['UID'];
    $acc_type = $_COOKIE['UT'];
    $query = new db_query("SELECT * FROM `new` WHERE new_id = $id_sksd");
    $sql_nd = mysql_fetch_assoc($query->result);
    $query_des = new db_query("SELECT * FROM `new_description` WHERE new_id = $id_sksd");
    ...
    $id_cat_parent = 63;
    $query_lsp = new db_query("SELECT `id`, `ten_loai`,`id_danhmuc` FROM `loai_chung` WHERE id_danhmuc = '$id_cat_parent'");
...
<head>
    ...
    <title>Chỉnh sửa tin sức khỏe sắc đẹp Vật tư y tế</title>
        ...
        <div class="tindang-container">
            ...
            <div class="tindang-content-cont hd-disflex">
                ...
                <div class="tindang-col-right">
                    <form class="form-dtin-cont" id="form_vattu_yte" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_sksd ?>">
                        <div class="row-tin-dang danhmuc_tt">
                            <p class="font-dam hd_font15-17">Danh mục sản phẩm <span class="color_red">*</span></p>
                            <input type="text" data="<?= $sql_nd['new_cate_id'] ?>" class="dmuc-spham" readonly name="san-pham-laptop" placeholder="Sức khỏe - Sắc đẹp >> Vật tư y tế">
                        </div>

                        <div class="row-tin-dang box_input_infor">
                            <p class="font-dam hd_font15-17">Tiêu đề <span class="color_red">*</span></p>
                            <input class="input_infor_tag error" autocomplete="off" type="text" name="tieu_de" placeholder="Nhập tiêu đề" value="<?= $sql_nd['new_title'] ?>">
                        </div>

                        <div class="row-tin-dang box_input_infor">
                            <p class="font-dam hd_font15-17">Loại vật tư <span class="color_red">*</span></p>
                            <select id="loai_vattu" class="slect-hang  hd_height36" name="loai_vattu">
                                <option disabled selected value="">Loại vật tư</option>
                        ...
                        <!-- Hãng, tình trạng người bán -->
                        <div class="d_flex j_between mt20">
                            <div class="row-tin-dang rowflex2 no_mg">
                                <p class="font-dam hd_font15-17">Hãng</p>
                                <input type="text" name="hang" placeholder="Nhập hãng" value="<?= $sql_des['hang_vattu'] ?>">
                            </div>
                            <div class="row-tin-dang rowflex2 no_mg box_input_infor">
                        ...
                        <div class="box_input_infor mt20">
                            ...
                            <select id="chitiet_dm" class="slect-chitiet_dm hd_height36" style="width:100%" name="chitiet_dm">
                                ...
                                <?
                                $query_dm = new db_query("SELECT `tags_id`, `ten_tags` FROM `key_tags` WHERE id_parent = '" . $sql_des['loai_sanpham'] . "'");
                                ?>
                        ...
                        <div class="row-tin-dang cont-btn-sb hd-disflex hide-480-mobile">
                            ...
                            <button type="button" class="btn-submit td-dang-tin vattu_yte hd_cspointer font-bold">CHỈNH SỬA</button>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    $('#loai_vattu').on('change', function() {
        var id_vt = $("select[name='loai_vattu']").val();
        $.ajax({
            type: 'POST',
            url: '/render/render_vattu_yte.php',
            data: {
                id_vt: id_vt,
            },
    ...
    $(".vattu_yte").click(function() {
        ...
        if (form_vattu_yte.valid() === true) {
            var user_id = $("#form_vattu_yte").attr("data");
            var user_type = $("#form_vattu_yte").attr("data1");
            var id_sksd = $("#form_vattu_yte").attr("data2");
            ...
            var loai_vattu = $("select[name='loai_vattu']").val();
            ...
            fd.append('loai_vattu', loai_vattu);
            ...
            $.ajax({
                url: '/ajax_sksd/chinhsua_sksd_vattuyte.php',
                type: 'POST',
                data: fd,
                contentType: false,
                processData: false,
    ...
    function xem_trc_tin() {
        var form_vattu_yte = $(".form-dtin-cont");
        form_vattu_yte.validate({
            errorPlacement: function(error, element) {
                error.appendTo(element.parents(".box_input_infor"));
                error.wrap("<span class='error'>");
