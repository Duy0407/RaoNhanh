...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_cs != 0 && $id_dm != 0) {
    ...
    if (mysql_num_rows($check_tind->result) > 0) {
        $list_tt = new db_query("SELECT n.`new_title`, n.`new_money`, n.`gia_kt`, n.`new_cate_id`, n.`new_city`, n.`new_image`, n.`new_unit`, n.`new_tinhtrang`,
                                n.`quan_huyen`, n.`phuong_xa`, n.`new_sonha`, n.`dia_chi`, n.`new_video`,  n.`new_ctiet_dmuc`, n.`new_baohanh`,
                                d.`new_description`, d.`loai_sanpham`, n.`new_phone`, n.`new_email`
        ...
        $loai_spham = $row_tt['loai_sanpham'];
        ...
        $list_nhom = new db_query("SELECT `id`, `ten_loai` FROM `loai_chung` WHERE `id_danhmuc` = $id_dm ");
    } else {
        header('Location: /');
    }
} else {
        ...
        <div class="tindang-container">
            ...
            <div class="tindang-content-cont hd-disflex">
                ...
                <div class="tindang-col-right" data="<?= $id_cs ?>">
                    <form class="form-dtin-cont" id="form_ngoaithat" data="<?= $us_id ?>" data1="<?= $us_type ?>">
                        <div class="row-tin-dang danhmuc_tt">
                            <p class="font-dam hd_font15-17">Danh mục sản phẩm <span class="color_red">*</span></p>
                            <input data="<?= $id_dm ?>" type="text" class="dmuc-spham" readonly name="san-pham-laptop" placeholder="Nội thất - Ngoại thất >> Ngoại thất">
                        </div>
                        ...
                        <div class="row-tin-dang box_input_infor">
                            ...
                            <select class="slect-hang loaisanpham hd_height36" name="loai_sanpham" data="<?= $id_dm ?>" onchange="tags_doi(this)">
                                <option disabled selected value="">Loại sản phẩm</option>
                                <? while ($row_nhom = mysql_fetch_assoc($list_nhom->result)) { ?>
                                    <option value="<?= $row_nhom['id'] ?>" <?= ($row_nhom['id'] == $loai_spham) ? "selected" : "" ?>><?= $row_nhom['ten_loai'] ?></option>
                                <? } ?>
                            </select>
                        </div>
                        <div class="row-tin-dang box_input_infor">
                        ...
                        <div class="row-tin-dang-btn cont-btn-sb hd-disflex hide-480-mobile">
                            ...
                            <button type="button" class="btn-submit td-dang-tin ngoaithat hd_cspointer font-bold">ĐĂNG TIN</button>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    $(".ngoaithat").click(function() {
        ...
        if (form_ngoaithat.valid() === true) {
            ...
            var loai_spham = $("select[name='loai_sanpham']").val();
