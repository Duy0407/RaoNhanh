...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_dm != 0 && $id_cs != 0) {
    ...
    $list_tt = new db_query("SELECT n.`new_title`, n.`new_money`, n.`gia_kt`, n.`new_cate_id`, n.`new_city`, n.`new_image`, n.`new_unit`, n.`new_tinhtrang`,
                                ...
                                d.`new_description`, d.`loai_chung`, d.`mon_the_thao`, n.`new_email`, n.`new_phone` FROM `new` AS n
    ...
    $list_ktag = new db_query("SELECT `tags_id`, `ten_tags`  FROM `key_tags` WHERE `id_danhmuc` = $id_dm AND `id_parent` = $mon_tthao ");

    $list_mtt = new db_query("SELECT `id`, `ten_mon` FROM `mon_the_thao` ");
    $list_lc1 = "SELECT `id`, `ten_loai` FROM `loai_chung` WHERE `id_danhmuc` = $id_dm ";
    $list_lc = new db_query($list_lc1);
    $list_lc2 = new db_query($list_lc1);
} else {
    header('Location: /');
}
...
<head>
    ...
    <title>Chỉnh sửa thời trang thể thao</title>
...
<body>
        ...
        <div class="tindang-container">
            ...
            <div class="tindang-content-cont hd-disflex">
                ...
                <div class="tindang-col-right" data="<?= $id_cs ?>">
                    <form class="form-dtin-cont" id="form_thethaothoitrang" data="<?= $us_id ?>" data1="<?= $us_type ?>">
                        <div class="row-tin-dang danhmuc_tt">
                            <p class="font-dam hd_font15-17">Danh mục sản phẩm <span class="color_red">*</span></p>
                            <input type="text" class="dmuc-spham" readonly name="san-pham-laptop" data="<?= $id_dm ?>" placeholder="Thể thao >> Thời trang thể thao">
                        </div>
                        ...
                        <div class="row-tin-dang box_input_infor">
                            ...
                            <select class="slect-hang  hd_height36 mon_the_thao" name="mon_thethao" onchange="hang_doi(this)" data="<?= $id_dm ?>">
                                <option selected disabled value="">Môn thể thao</option>
                                <? while ($row_mtt = mysql_fetch_assoc($list_mtt->result)) { ?>
                                    <option value="<?= $row_mtt['id'] ?>" <?= ($row_tt['mon_the_thao'] == $row_mtt['id']) ? "selected" : "" ?>><?= $row_mtt['ten_mon'] ?></option>
                                <? } ?>
                            </select>
                        </div>
                        <!-- loại thời trang, tình trạng người bán -->
                        <div class="d_flex j_between dangtin_tt_fl_des">
                            <div class="row-tin-dang rowflex2 box_input_infor">
                                <p class="font-dam hd_font15-17">Loại thời trang <span class="color_red">*</span></p>
                                <select class="slect-hang  hd_height36 loai_dungcu_ban" name="loai_thoitrang_ban">
                                    <option value="">Loại thời trang</option>
                                    <? while ($row_lc = mysql_fetch_assoc($list_lc->result)) { ?>
                                        <option value="<?= $row_lc['id'] ?>" <?= ($row_lc['id'] == $row_tt['loai_chung']) ? "selected" : "" ?>> <?= $row_lc['ten_loai'] ?></option>
                                    <? } ?>
                                </select>
                            </div>
                            <div class="row-tin-dang rowflex2 box_input_infor">
                        ...
                        <div class="box_input_infor dong_doi">
                            ...
                            <select class="slect-chitiet_dm hd_height36" style="width:100%" name="chitiet_dm">
                                ...
                                <? while ($row_tags = mysql_fetch_assoc($list_ktag->result)) { ?>
                                    <option value="<?= $row_tags['tags_id'] ?>" <?= ($row_tt['new_ctiet_dmuc'] == $row_tags['tags_id']) ? "selected" : "" ?>><?= $row_tags['ten_tags'] ?></option>
                                <? } ?>
                            </select>
                        </div>
                        <div class="row-tin-dang box_input_infor">
                            <p class="font-dam hd_font15-17">Địa chỉ <span class="color_red">*</span></p>
                            <input type="text" name="td_dia_chi" class="td_ttin_diachi input_infor_tag error" value="<?= $row_tt['dia_chi'] ?>" autocomplete="off" placeholder="Địa chỉ" data-tt="<?= $tinh_thanh ?>" data-qh="<?= $quan_huyen ?>" data-px="<?= $phuong_xa ?>" data-sn="<?= $so_nha ?>">
                        </div>
                        <div class="row-tin-dang box_input_infor">
                        ...
                        <div class="row-tin-dang cont-btn-sb hd-disflex">
                            ...
                            <button type="button" class="btn-submit td-dang-tin tt_thethao hd_cspointer font-bold">ĐĂNG TIN</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <script>
        $(".tt_thethao").click(function() {
            var form_thethaothoitrang = $("#form_thethaothoitrang");
            form_thethaothoitrang.validate({
                ...
                messages: {
                    ...
                    loai_thoitrang_ban: "Vui lòng chọn loại thời trang",
            ...
            if (form_thethaothoitrang.valid() === true) {
                ...
                var loai_ttrang = $("select[name='loai_thoitrang_ban']").val();
                ...
                fd.append('loai_ttrang', loai_ttrang);
                ...
                $.ajax({
                    url: '/ajax/csua_thethao_ttrang.php',
                    type: 'POST',
                    data: fd,
                    processData: false,
                    contentType: false,
        ...
        $(".td-xem-truoc").click(function() {
            var form_thethaothoitrang = $(".form-dtin-cont");
            form_thethaothoitrang.validate({
                errorPlacement: function(error, element) {
                    error.appendTo(element.parents(".box_input_infor"));
                    error.wrap("<span class='error'>");
            ...
            if (form_thethaothoitrang.valid() === false) {
            ...
            if (form_thethaothoitrang.valid() === true) {
                ...
                fd.append('loai_sp2', loai_ttrang);
                fd.append('tinh_trang', tinhtrang_ban);
