...
$db_qr = new db_query("SELECT new_id,new_alias,new_picture,save_time_vl,new_user_id,usc_phone,usc_name,new_title,new_address,new_job_kind,new_job_detail,new_end_date,new_money_min,usc_logo,new_money_max,new_desc,new_exp,new_level,new_skill,new_quantity,new_view_count,numrate,new_type,new_level,new_exp ,new_pay_type,new_job_type,new_city,new_district,usc_city,usc_address
...
if ($row['new_alias'] != '') {
    ...
    $ali = replaceTitle($row['new_title']);
}

$urlcano = "https://raonhanh365.vn/viec-lam/".$ali."-p".$row['new_id'].".html";
...
<head>
    ...
    <title><?=$row['new_title']?> - <?= $row['new_id'] ?> | Raonhanh365</title>
    <meta name="robots" content="noindex,nofollow"/>
    <meta name="google-site-verification" content="8HoYLGUD3DXbNpPr7JHZjAxMghKbHnuSClcHal7OnRA" />
    <link rel="canonical" href="https://raonhanh365.vn/viec-lam/<?= replaceTitle($row['new_title']) ?>-p<?= $row['new_id'] ?>.html" />
    <meta name="description" content="<?=$row['new_title']?> <?= $row['new_id'] ?> của <?=$row['usc_name']?> mới nhất trên Raonhanh365.vn. Đăng ký ứng tuyển ngay hôm nay."/>
    <meta name="Keywords" content="<?=$row['new_title']?>"/>

    <meta property="og:locale" content="vi_VN" />
    <meta property="og:type" content="website" />
    <meta property="og:title" content="<?=$row['new_title']?> - <?= $row['new_id'] ?> | Raonhanh365"/>
    <meta property="og:description" content="<?=$row['new_title']?> <?= $row['new_id'] ?> của <?=$row['usc_name']?> mới nhất trên Raonhanh365.vn. Đăng ký ứng tuyển ngay hôm nay."/>
    ...
    <meta name="twitter:description" content="<?=$row['new_title']?> <?= $row['new_id'] ?> của <?=$row['usc_name']?> mới nhất trên Raonhanh365.vn. Đăng ký ứng tuyển ngay hôm nay." />
    <meta name="twitter:title" content="<?=$row['new_title']?> - <?= $row['new_id'] ?> | Raonhanh365" />
    ...
    <div class="detail-tuyendung">
        <div class="row">
            ...
            <div class="author">
                <div class="nguoidang">
                    <div class="images">
                        <img width="58" height="58"   onerror='this.onerror=null;this.src="/images/icon_raonhanh.png";' class="lazyload" src="/images/icon_raonhanh.png" data-src="<?= $row['usc_logo'] ?>" alt="avatar">
                    </div>
                ...
                <? 
                    $db_danh_gia_1 = new db_query("SELECT new_id as id from vieclam WHERE new_user_id = '".$row['new_user_id']."'");
                    $row_count_dat_gia = mysql_num_rows($db_danh_gia_1->result);
                ?>
                <div class="danhgia"><span> Đánh giá: </span><span class="au-r"><?=$row_count_dat_gia?></span></div>
                    ...
                    <div class="form-left">
                        <div class="chitietcv gr-tuyendung">
                            <div class="info">
                                ...
                                <p class="inf-bottom">
                                    <a href="/viec-lam-<?=replaceTitle($name_type)?>-n<?=$row['new_job_type']?>t0">Việc làm <?=$name_type?></a>
                                </p>
                            </div>
                        </div>
                        <div class="chitietcv gr-tuyendung">
                            <div class="info">
                                ...
                                <p class="inf-bottom">
                                    <a href="/tim-viec-lam-<?=replaceTitle($name_type_chi_tiet)?>-r<?=$row['new_job_detail']?>t0.html">Việc làm <?= $name_type_chi_tiet  ?></a>
                                </p>
                            </div>
                        </div>
                    ...
                    <div class="gr-mt">
                        <div class="mota">
                            ...
                            <div class="anh">
                                ...
                                <!-- <img src="<?= !empty($row['new_picture'])?$row['new_picture']:'../images/vieclam/anhbia.png' ?>" style="width: 100%;padding-top: 37px"  alt="<?= $row['new_title'] ?>"> -->
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="avatar">
                            <div class="images">
                                <a href="/viec-lam/<?= replaceTitle($rownew['new_title']) ?>-p<?= $rownew['new_id'] ?>.html">
                                    <img onerror='this.onerror=null;this.src="/images/icon_raonhanh.png";' class="lazyload" src="/images/loading.gif" data-src="<?= $rownew['new_picture'] ?>" alt="<?= $rownew['new_title'] ?>">
                                </a>
                            </div>
                            <div class="cty"><h3><a class="name_bai_lq" href="/viec-lam/<?= replaceTitle($rownew['new_title']) ?>-p<?= $rownew['new_id'] ?>.html"><?= $rownew['new_title'] ?></a></h3></div>
                        </div>
