...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0) {
    ...
    $list_tin = new db_query("SELECT `new`.`new_id`, `new_title`,`link_title`,`new_money`, `new_cate_id`, 
                                ...
                                `new_description`,`loai_sanpham`,`hang`,`canhan_moigioi`
    ...
    if (mysql_num_rows($list_tin->result) > 0) {
        ...
        // tag danh muc
        $ban = new db_query("SELECT `tags_id`, `ten_tags` FROM `key_tags` WHERE `id_danhmuc` = 108 AND type_tags = '" . $item_td['can_ban_mua'] . "' ");
...
<head>
    ...
    <title>Chỉnh sửa Dụng cụ - Phụ kiện làm đẹp </title>
...
<body>
    ...
    <section id="m_dangtin_sk_mypham" class="suckhoe">
        ...
        <form class="form-dtin-cont form_bds_chung" id="form_pk_lamdep" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_nd ?>" data3="<?= $xacthuc_lket ?>">
            <div class="container_content">
                    ...
                    <select id="loai_pk" class="b6_lpk_select slect-hang  hd_height36" name="loai_phukien">
                        ...
                        <? foreach ($db_lchung as $rows) {
                            if ($rows['id_cha'] == $id_cat_parent) { ?>
                                <option <?= ($rows['id'] == $item_td['loai_sanpham']) ? "selected" : "" ?> value="<?= $rows['id'] ?>"><?= $rows['ten_loai'] ?></option>
                        <? }
                        } ?>
                    </select>
                </div>
                ...
                <div class="b6_hangphukien d_flex fl_cl box_input_infor">
                    ...
                    <select class="slect-hang  hd_height36" name="hang">
                        ...
                        ?>
                                <option <?= ($hang['id'] == $item_td['hang']) ? "selected" : "" ?> value="<?= $hang['id'] ?>"><?= $hang['ten_hang'] ?></option>
                        <? }
                        } ?>
                    </select>
                </div>
    ...
    <script>
        ...
        $(".b11_btn_chinhsua").click(function() {
            ...
            if (form_pk_lamdep.valid() === true) {
                ...
                var id_nd = $("#form_pk_lamdep").attr("data2");
