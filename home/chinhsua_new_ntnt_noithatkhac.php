...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0) {
    ...
    $qr_listds = new db_query("SELECT `new`.`new_id`, `new_title`, `new_money`, `new_cate_id`, `new_city`, `new_image`, `new_unit`, `new_phone`, `new_email`,
                            ...
                            `new_description`, `canhan_moigioi`, `chat_lieu`,`thoigian_kmbd`,`thoigian_kmkt`,`soluong_min`,`soluong_max` FROM `new`
    ...
    if (mysql_num_rows($qr_listds->result) > 0) {
        ...
        $qr_clieu = new db_query("SELECT `id`, `name` FROM nhom_sanpham_chatlieu WHERE id_danhmuc = " . $id_dm);
...
<head>
    ...
    <title>Chỉnh sửa nội thất khác</title>
                        ...
                        <div class="d_flex fl_cl box_input_infor w_100">
                            ...
                            <select class="b6_fr1_select slect-hang hd_height36 chat_lieu" name="chat_lieu">
                                ...
                                <? while ($row_clieu = mysql_fetch_assoc($qr_clieu->result)) { ?>
                                    <option value="<?= $row_clieu['id'] ?>" <?= ($row_clieu['id'] == $item_td['']) ?>><?= $row_clieu['name'] ?></option>
                                <? } ?>
                            </select>
                        </div>
                    </div>
    ...
    <script type="text/javascript">
        $(".b11_btn_chinhsua").click(function() {
            ...
            if (form_pc.valid() === true) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '/ajax_noithat_ngoaithat/csua_ntkhac.php',
                    data: fd,
                    processData: false,
                    contentType: false,
