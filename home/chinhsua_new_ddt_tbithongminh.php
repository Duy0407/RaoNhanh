...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_dm != 0 && $id_nd != 0) {
    ...
    $qr_listds = new db_query("SELECT `new`.`new_id`, `new_title`, `new_money`, `new_city`, `new_image`, `new_unit`, `new_name`, `new_phone`, `new_email`,
                            ...
                            `new_baohanh`, `new_description`, `hang`, `dong_may`, `thiet_bi`, `canhan_moigioi`  FROM `new`
    ...
    if (mysql_num_rows($qr_listds->result) > 0) {
        ...
        $qr_thietbi = new db_query("SELECT `id`, `ten_loai`  FROM `loai_chung` WHERE `id_danhmuc` = $id_dm ");

        $bao_hanh = new db_query("SELECT `id_baohanh`, `tgian_baohanh`  FROM `bao_hanh` WHERE `id_danhmuc` = 1 ");

        if ($item_td['thiet_bi'] != 4345) {
            ...
            $query = new db_query("SELECT `id`, `ten_hang`  FROM `hang` WHERE `id_danhmuc` = $id_dm and `id_parent`= '" . $item_td['thiet_bi'] . "' ");

            if ($item_td['hang'] != 1766 && $item_td['hang'] != 1774) {
                $list_dong = new db_query("SELECT `id`, `ten_dong` FROM `dong` WHERE `id_hang` = '" . $item_td['hang'] . "' AND `id_danhmuc` = $id_dm ");
            }
        };
    } else {
        header('Location: /');
...
<head>
    ...
    <title>Chỉnh sửa tin đăng đồ tiện tử thiết bị thông minh</title>
...
<body>
                ...
                <div class="ctn_ct_box6 d_flex fl_cl">
                    ...
                    <div class="ctn_ct_b6_fr d_flex fl_row fl_st">
                        <div class="b6_fr1 d_flex fl_cl box_input_infor">
                            ...
                            <select class="b6_fr1_select slect-hang hd_height36" name="thiet_bi" data="<?= $id_dm ?>" onchange="tbi_doi(this)">
                                <option disabled value="">Chọn</option>
                                <? while ($row_tbi = mysql_fetch_assoc($qr_thietbi->result)) { ?>
                        ...
                        <div class="b6_fr2 d_flex fl_cl box_input_infor hang_doi">
                            <? if ($item_td['thiet_bi'] == 4345) { ?>
                                <p class="b9_fr2_title p_400_s15_l18">Hãng <span class="cl_red">*</span></p>
                                <input type="text" name="hang_tb" value="<?= $item_td['hang'] ?>" class="b3_fr1_input p_400_s14_l17 hang_tb" placeholder="Nhập hãng" autocomplete="off">
                            <? } else { ?>
                                <p class="b6_fr1_title p_400_s15_l18">Hãng <span class="cl_red">*</span></p>
                                <select class="b6_fr1_select slect-hang hd_height36 hang_tb" name="hang_tb" data="<?= $id_dm ?>" onchange="hang_doi(this)">
                                    <option disabled value="">Chọn</option>
                    ...
                    <div class="ctn_ct_b6_fr d_flex fl_row fl_st">
                        <div class="b6_fr2 d_flex fl_cl box_input_infor dong_doi">
                            <? if ($item_td['thiet_bi'] == 4345) { ?>
                                ...
                                <input type="text" name="dong_may" class="b3_fr1_input p_400_s14_l17 dong_may" value="<?= $item_td['dong_may'] ?>" placeholder="Nhập dòng" autocomplete="off">
                                <? } else {
                                if ($item_td['hang'] != 1766 && $item_td['hang'] != 1774) { ?>
                                ...
                                <? } else { ?>
                                    <p class="b9_fr2_title p_400_s15_l18">Dòng <span class="cl_red">*</span></p>
                                    <input type="text" name="dong_may" value="<?= $item_td['dong_may'] ?>" class="b3_fr1_input p_400_s14_l17 dong_may" placeholder="Nhập dòng" autocomplete="off">
                            <? }
                            } ?>
                        </div>
    ...
    <script type="text/javascript">
        var id_tb = $("select[name='thiet_bi']").val();
        if (id_tb == 4345) {
            $('.ctn_ct_box10').hide();
        };
        ...
        function tbi_doi(id) {
            ...
            if (id_tbi == 4345) {
                $('.ctn_ct_box10').hide();
                var dong = `<p class="b9_fr2_title p_400_s15_l18">Dòng <span class="cl_red">*</span></p>
