<?
    ...
    $catid = (int)$catid;
    $keyid = getValue("keyid","int","GET",0);
    $keyid = (int)$keyid;
    ...
    $db_key = new db_query("SELECT key_name FROM keyword WHERE key_id = '".$keyid."' LIMIT 1");
    $rowkey = mysql_fetch_assoc($db_key->result);
    $keyname = $rowkey['key_name'];
    $keyname_vl = trim(str_replace(array('Việc làm','việc làm','Tìm'), '', $keyname));
    ...
    if($keyid != 0 && $city == 0){
        $vlt = "(FIND_IN_SET(".$keyid.", new_job_detail))";
        ...
        // echo "SELECT new_id,usc_phone,save_time_vl,new_user_id,new_title,new_address,new_job_type,new_job_kind,new_end_date,new_desc,new_money_min,new_money_max,new_type,usc_name,cit_name,new_view_count,numrate,new_picture,new_job_detail
        // from vieclam 
        // join user on vieclam.new_user_id = user.usc_id 
        // join city2 on vieclam.new_city = city2.cit_id 
        // join keyword on vieclam.new_job_detail = keyword.key_id 
        // where ".$vlt." AND new_type=1";
        // die();
    ...
    $url_default = "https://raonhanh365.vn".rewriteKey1($catid,$catname,$city,$cit_name,$keyid,$keyname);
...
<head>
    ...
    <?if ($keyid != 0){?>
    <title>Việc làm <?=$keyname_vl?> mới nhất - <?= $m = date('m', time());?>/<?=$y = date('Y', time());?></title>
    <meta name="robots" content="noindex,nofollow"/>
    <link rel="canonical" href="https://raonhanh365.vn/tim-viec-lam-<?= replaceTitle($keyname_vl) ?>-r<?=$keyid?>t0.html" />
    <meta name="description" content="Danh sách việc làm <?=$keyname_vl?> mới nhất được cập nhật thường xuyên và liên tục trên Raonhanh365. Tin tuyển dụng việc làm <?=$keyname_vl?> lương cao - <?= $m = date('m', time());?>/<?=$y = date('Y', time());?>"/>
    <meta name="Keywords" content="việc làm <?=$keyname_vl?>, tìm việc làm <?=$keyname_vl?>"/>

    <meta property="og:locale" content="vi_VN" />
    <meta property="og:type" content="website" />
    <meta property="og:title" content="Việc làm <?=$keyname_vl?> mới nhất - <?= $m = date('m', time());?>/<?=$y = date('Y', time());?>"/>
    <meta property="og:description" content="Danh sách việc làm <?=$keyname_vl?> mới nhất được cập nhật thường xuyên và liên tục trên Raonhanh365. Tin tuyển dụng việc làm <?=$keyname_vl?> lương cao - <?= $m = date('m', time());?>/<?=$y = date('Y', time());?>"/>
    <meta property="og:site_name" content="raonhanh365.vn" />

    <meta name="twitter:card" content="summary" />
    <meta name="twitter:description" content="Danh sách việc làm <?=$keyname_vl?> mới nhất được cập nhật thường xuyên và liên tục trên Raonhanh365. Tin tuyển dụng việc làm <?=$keyname_vl?> lương cao - <?= $m = date('m', time());?>/<?=$y = date('Y', time());?>" />
    <meta name="twitter:title" content="Việc làm <?=$keyname_vl?> mới nhất - <?= $m = date('m', time());?>/<?=$y = date('Y', time());?>" />
    <?}?>
                ...
                ?>
                    ...
                    while ($row = mysql_fetch_assoc($db_qr->result)) {
                        ...
                        foreach ($arr_type_chi_tiet as $key => $value_1) { 
                            $name_type_chi_tiet[] = '<span style="color: #F26222;" class="cv">'.$arr_tag_ct[$value_1].'</span>';
                        }
