...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0) {
    $acc_id = $_COOKIE['UID'];
    $acc_type = $_COOKIE['UT'];
    $query = new db_query("SELECT `new_title`,`link_title`,`new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,`new_type`,
    ...
    $query_des = new db_query("SELECT `new_description`,`can_ban_mua`, `ten_toa_nha`, `tong_so_tang`, `huong_chinh`, `so_pngu`,`so_pve_sinh`,
                                `giay_to_phap_ly`, `tinh_trang_noi_that`, `dac_diem`, `dien_tich`, `chieu_dai`, `chieu_rong` FROM `new_description`
                                WHERE new_id = $id_nd");
...
<head>
    ...
    <title>Chỉnh sửa tin bất động sản nhà riêng, nguyên căn</title>
        ...
        <div class="tindang-container">
            ...
            <div class="tindang-content-cont hd-disflex">
                ...
                <div class="tindang-col-right">
                    <form class="form-dtin-cont" id="form_bds_nharieng" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_nd ?>">
                        <div class="row-tin-dang danhmuc_tt">
                            <p class="font-dam hd_font15-17">Danh mục sản phẩm <span class="color_red">*</span></p>
                            <input type="text" class="dmuc-spham" readonly name="san-pham-laptop" placeholder="Bất động sản >> Nhà riêng, nguyên căn">
                        </div>
...
<script>
    ...
    $(".dang_tin").click(function() {
        var form_bds_nharieng = $("#form_bds_nharieng");
        ...
        if (form_bds_nharieng.valid() === true) {
            var user_id = $("#form_bds_nharieng").attr("data");
            var user_type = $("#form_bds_nharieng").attr("data1");
            var id_nd = $("#form_bds_nharieng").attr("data2");
    ...
    function xem_trc_tin() {
        var form_bds_nharieng = $(".form-dtin-cont");
        form_bds_nharieng.validate({
            errorPlacement: function(error, element) {
                error.appendTo(element.parents(".box_input_infor"));
                error.wrap("<span class='error'>");
        ...
        if (form_bds_nharieng.valid() === false) {
