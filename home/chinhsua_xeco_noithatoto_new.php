...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0) {
    ...
    $list_tin = new db_query("SELECT `new`.`new_id`, `new_title`,`link_title`,`new_money`, `new_cate_id`, 
                                ...
                                `new_description`,`loai_noithat`,`canhan_moigioi`,
    ...
    if (mysql_num_rows($list_tin->result) > 0) {
        ...
        // tag danh muc
        $ban = new db_query("SELECT `tags_id`, `ten_tags` FROM `key_tags` WHERE `id_danhmuc` = 42 AND type_tags = '" . $item_td['can_ban_mua'] . "' ");
...
<head>
    ...
    <title>Chỉnh sửa xe cộ nội thất ô tô</title>
...
<body>
    ...
    <!-- content -->
    <section id="m_dangtin_xc_noithatoto" class="xeco">
        <div class="container_title">
            <p class="ctn_tt_p p_600_s24_l28">Chỉnh sửa</p>
        </div>
        <form class="form-dtin-cont form_bds_chung" id="form_xeco_noithatoto" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_nd ?>" data3="<?= $xacthuc_lket ?>">
                ...
                <div class="ctn_ct_box1 d_flex fl_cl w100">
                    ...
                    <div class="ctn_ct_b1_fr2 w100">
                        <div class="b1_fr2_title w100">
                            <p class="b1_fr2_title_p p_400_s16_l19 d_flex fl_row al_ct" data="42">
                    ...
                    <div class="ctn_ct_b6_ft d_flex fl_row fl_st">
                        <div class="b6_ft1 d_flex fl_cl box_input_infor">
                            <p class="b6_ft1_title p_400_s15_l18">
                                Loại nội thất <span class="cl_red">*</span>
                            </p>
                            <select id="loai_noi_that" class="slect-hang hd_height36" name="loai_noi_that">
                                <option selected disabled value="">Chọn</option>
                                <? foreach ($db_lchung as $nt) {
                                    if ($nt['id_cha'] == 42) {
                                ?>
                                        <option <?= ($nt['id'] == $item_td['loai_noithat']) ? "selected" : "" ?> value="<?= $nt['id'] ?>"><?= $nt['ten_loai'] ?></option>
                                <? }
                                } ?>
                            </select>
                        </div>
    ...
    <script>
        $(".b11_btn_chinhsua").click(function() {
            ...
            if (form_xeco_noithatoto.valid() === true) {
                ...
                var id_nd = $("#form_xeco_noithatoto").attr("data2");
                ...
                var xac_thuc = $("#form_xeco_noithatoto").attr("data3");
        ...
        function xem_trc_tin() {
            ...
            form_xeco_noithatoto.validate({
                ...
                messages: {
                    ...
                    td_gia_spham: "Vui lòng nhập giá sản phẩm",
                    chitiet_dm: "Vui lòng chọn chi tiết danh mục",
                    loai_noi_that: "Vui lòng nhập loại nội thất",
                    mota: {
                        required: "Vui lòng nhập mô tả",
                        minlength: "Mô tả ít nhất 10 ký tự",
                        maxlength: "Mô tả nhiều nhất 10000 ký tự",
