...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT'])) {
    ...
    $list_ncu = new db_query("SELECT `id`, `ten_loai` FROM `loai_chung` WHERE `id_danhmuc` = $id ");
...
<body>
            ...
            <div class="tindang-container">
                ...
                <div class="tindang-content-cont hd-disflex">
                    ...
                    <div class="tindang-col-right">
                        <form class="form-dtin-cont fid_gia_mb" id="form_dv_nhaccu" data="<?= $us_id ?>" data1="<?= $us_type ?>">
                            <div class="row-tin-dang danhmuc_tt">
                                <p class="font-dam hd_font15-17">Danh mục sản phẩm <span class="color_red">*</span></p>
                                <input type="text" class="dmuc-spham" readonly name="san-pham-laptop" data="<?= $id ?>" placeholder="Dịch vụ - Giải trí >> Nhạc cụ">
                            </div>
                            ...
                            <div class="row-tin-dang box_input_infor">
                                <p class="font-dam hd_font15-17">Loại nhạc cụ <span class="color_red">*</span></p>
                                <select class="slect-hang  hd_height36" name="loai_nhac_cu">
                                    <option value="">Loại nhạc cụ</option>
                                    <? while ($row_nc = mysql_fetch_assoc($list_ncu->result)) { ?>
                                        <option value="<?= $row_nc['id'] ?>"><?= $row_nc['ten_loai'] ?></option>
                                    <? } ?>
                                </select>
                            </div>
                            <div class="row-tin-dang box_input_infor">
                            ...
                            <div class="row-tin-dang d_8-7_tclass1">
                                <p class="font-dam hd_font15-17 d_8-7_tclass2">Giá <span style="color:#ff0000">*</span>
                                </p>
                                <div class="d_themdiv_gia_7_8 fid_gia_mb1">
                            ...
                            <div class="row-tin-dang cont-btn-sb hd-disflex">
                                ...
                                <button type="button" id="xoa_tddang_tin" class="btn-submit td-dang-tin nhaccu hd_cspointer font-bold">ĐĂNG TIN</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    <script>
        $(".td-dang-tin").click(function() {
            var form_dv_nhaccu = $("#form_dv_nhaccu");
            form_dv_nhaccu.validate({
                ...
                messages: {
                    ...
                    loai_nhac_cu: "Vui lòng chọn loại nhạc cụ",
                    tinhtrang: "Vui lòng chọn tình trạng nhạc cụ",
            ...
            if (form_dv_nhaccu.valid() === true) {
                ...
                var loai_ncu = $("select[name='loai_nhac_cu']").val();
                ...
                $.ajax({
                    type: 'POST',
                    url: '/ajax_dvgiaitri/dtin_nhaccu.php',
                    data: fd,
                    processData: false,
                    contentType: false,
                    beforeSend: function() {
                        $('.nhaccu').prop('disabled', true);
                    },
        ...
        $(".td-xem-truoc").click(function() {
            var form_dv_nhaccu = $(".form-dtin-cont");
            form_dv_nhaccu.validate({
                errorPlacement: function(error, element) {
                    error.appendTo(element.parents(".box_input_infor"));
                    error.wrap("<span class='error'>");
