...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0 && $id_dm != 0) {
    ...
    $qr_listds = new db_query("SELECT `new`.`new_id`, `new_title`, `new_money`, `new_cate_id`, `new_city`, `new_image`,
                            `new_unit`, `new_phone`, `new_email`, `new_address`, `chotang_mphi`, `quan_huyen`, `phuong_xa`, `new_sonha`, `dia_chi`, `new_video`,
                            `new_ctiet_dmuc`,`new_tinhtrang`, `new_baohanh`, `new_buy_sell`, `new_description`, `bovi_xuly`, `ram`, `o_cung`,
                            `loai_o_cung`, `man_hinh`, `kich_co`, `canhan_moigioi`  FROM `new`
...
<head>
    ...
    <title>Chỉnh sửa tin đăng đồ tiện tử máy tính để bàn</title>
...
<body>
    ...
    <section id="m_dangtin_xc_oto" class="xeco">
        <div class="container_title">
            <p class="ctn_tt_p p_600_s24_l28">Chỉnh sủa tin đăng</p>
        </div>
                        ...
                        <div class="b6_fr1 d_flex fl_cl box_input_infor">
                            ...
                            <select class="b6_fr1_select slect-hang hd_height36" name="bovi_xuly">
                                ...
                                <? while ($row_bv = mysql_fetch_assoc($bivi_xuly->result)) { ?>
                                    <option value="<?= $row_bv['bovi_id'] ?>" <?= ($row_bv['bovi_id'] == $item_td['bovi_xuly']) ? "selected" : "" ?>><?= $row_bv['bovi_ten'] ?></option>
                                <? } ?>
                            </select>
                        </div>
                        <div class="b6_fr2 d_flex fl_cl box_input_infor">
                            <p class="b6_fr1_title p_400_s15_l18">Ram</p>
                            <select class="b6_fr1_select slect-hang hd_height36" name="ram_pc">
                                <option value="">Chọn</option>
                                <? while ($row_ram = mysql_fetch_assoc($list_ram->result)) { ?>
                                    <option value="<?= $row_ram['id_dl'] ?>" <?= ($row_ram['id_dl'] == $item_td['ram']) ? 'selected' : '' ?>><?= $row_ram['ten_dl'] ?></option>
                                <? } ?>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="b6_fr1 d_flex fl_cl box_input_infor">
                            ...
                            <select class="b6_fr1_select slect-hang hd_height36" name="card_mhinh">
                                ...
                                <? while ($row_card = mysql_fetch_assoc($card_mhinh->result)) { ?>
                                    <option value="<?= $row_card['id_manhinh'] ?>" <?= ($row_card['id_manhinh'] == $item_td['man_hinh']) ? 'selected' : '' ?>><?= $row_card['ten_manhinh'] ?></option>
                                <? } ?>
                            </select>
                        </div>
                        <div class="b6_fr2 d_flex fl_cl box_input_infor">
                            ...
                            <select class="b6_fr1_select slect-hang hd_height36" name="kich_co">
                                ...
                                <? while ($row_kc = mysql_fetch_assoc($kich_co->result)) { ?>
                                    <option value="<?= $row_kc['id_manhinh'] ?>" <?= ($row_kc['id_manhinh'] == $item_td['kich_co']) ? 'selected' : '' ?>><?= $row_kc['ten_manhinh'] ?></option>
                                <? } ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="ctn_ct_b6_nl d_flex fl_cl box_input_infor">
                        ...
                        <div class="b6_nl_fr d_flex fl_row">
                            <div class="b6_nl_child b6_nl_xang d_flex fl_row al_ct ">
                                <input type="radio" name="tinhtrang_pc" value="1" <?= ($item_td['new_tinhtrang'] == 1) ? 'checked' : '' ?> class="img20 cursor_Pt b6_nl_xang_input b6_nl_input">
                                <p class="b6_nl_xang_text pdl_10">Mới</p>
                            </div>
                            <div class="b6_nl_child b6_nl_dau d_flex fl_row al_ct ">
                                <input type="radio" name="tinhtrang_pc" value="2" <?= ($item_td['new_tinhtrang'] == 2) ? 'checked' : '' ?> class="img20 cursor_Pt b6_nl_dau_input b6_nl_input">
                                <p class="b6_nl_dau_text pdl_10">Cũ (Đã sửa chữa)</p>
                            </div>
                            <div class="b6_nl_child b6_nl_hybrid d_flex fl_row al_ct ">
                                <input type="radio" name="tinhtrang_pc" value="3" <?= ($item_td['new_tinhtrang'] == 3) ? 'checked' : '' ?> class="img20 cursor_Pt b6_nl_hybrid_input b6_nl_input">
    ...
    <script>
        $(".b11_btn_chinhsua").click(function() {
            ...
            if (form_pc.valid() === true) {
                ...
                var tinh_trang = $("input[name='tinhtrang_pc']:checked").val();
