...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id != 0) {
    ...
    $list_dh = new db_query("SELECT `dh_id`, `id_nguoi_dh`, `id_nguoi_ban`, `id_spham`, `ghi_chu`, `loai_ttoan`, `tien_ttoan`, `dia_chi_nhanhang`, `trang_thai`,
                            `tgian_xnbh`, `usc_name`, `chat365_id`, `usc_id`, `usc_logo`, `usc_phone`, `new_title`, `new_money`, `new_unit`, `new_user_id`, `new_type`, `new_image`
                            FROM `dat_hang` INNER JOIN `new` ON `new`.`new_id` = `dat_hang`.`id_spham`
                            INNER JOIN `user` ON `user`.`usc_id` = `dat_hang`.`id_nguoi_ban`
                            WHERE `trang_thai` = 0 AND `id_nguoi_dh` = $id_user AND `dh_id` = $id ");
...
<body>
        ...
        <div class="box-right edit_pers">
            <div class="box-right-detail">
                <div class="detail-top">
                    <a href="/quan-ly-don-hang-mua.html"><img src="/images/bo-sung-raonhanh/muiten-back.svg" alt=""></a>
                ...
                <div class="detail-main">
                    <div class="detail-main-t">
                        <div class="scroll-ngang">
                            <div class="main-t-number">
                                <div class="outtrinh-img">
                                    <img src="/images/bo-sung-raonhanh/outtrinh.svg" alt="">
                                </div>
                        ...
                        <div class="main-t-p">
                            ...
                            <p class="color_orange text_status">CHỜ XÁC NHẬN</p>
                        </div>
                            ...
                            <div class="bot-ql">
                                ...
                                <div class="content-bot-acp">
                                    ...
                                    <div class="acp-ttdh-price">
                                        <p class="text_sp">Tổng tiền hàng: <span class="color_orange num_price"><span class="color_orange num_price"><?= number_format($row_tin['new_money']) ?> <?= $arr_dvtien[$row_tin['new_unit']] ?></span></p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="khach-hang-thanh-toan">
                            ...
                            <div class="pay-right">
                                <p class="text_title align_c">Thanh toán</p>
                                <p class="text_price no_italic font_w500">Tổng đơn hàng: <?= number_format($row_tin['new_money']) ?> <?= $arr_dvtien[$row_tin['new_unit']] ?></p>
                                <p class="text_price">Đã thanh toán: <?= number_format($row_tin['tien_ttoan']) ?> <?= $arr_dvtien[$row_tin['new_unit']] ?></p>
                                <p class="text_price">Số tiền còn lại: <?= number_format($row_tin['new_money'] - $row_tin['tien_ttoan']) ?> <?= $arr_dvtien[$row_tin['new_unit']] ?></p>
                            </div>
                        </div>
                        <div class="main-t-btn">
                            <button type="button" class="huydon-1-detail" data="<?= $row_tin['dh_id'] ?>">Hủy đơn</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <? include('../modals/huydon_hangnmua.php') ?>
...
<script type="text/javascript">
    $(".huydon-1-detail").click(function() {
