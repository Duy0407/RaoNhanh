...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_dm != 0 && $id_nd != 0) {
    ...
    $qr_listds = new db_query("SELECT `new`.`new_id`,`new_title`, `new_money`, `new_cate_id`, `new_city`, `new_image`, `new_unit`, `new_name`, `new_phone`,
                            `new_email`, `new_address`, `chotang_mphi`, `quan_huyen`, `phuong_xa`, `new_sonha`, `dia_chi`, `new_video`, `new_ctiet_dmuc`,
                            `new_tinhtrang`, `new_baohanh`, `new_description`, `hang`, `dong_may`, `mau_sac`,`dung_luong`,
                            `canhan_moigioi`,`phien_ban`  FROM `new`
    ...
    if (mysql_num_rows($qr_listds->result) > 0) {
        ...
        if (in_array($item_td['hang'], $check_hdthoai) == false && $item_td['hang'] != 1683) {
...
<head>
    ...
    <title>Chỉnh sửa tin đăng đồ tiện điện thoại</title>
                ...
                <div class="ctn_ct_box6 d_flex fl_cl">
                    ...
                    <div class="ctn_ct_b6_fr d_flex fl_row fl_st">
                        <div class="b6_fr1 d_flex fl_cl box_input_infor">
                            ...
                            <select class="b6_fr1_select slect-hang hd_height36" name="hang_phone" data="<?= $id_dm ?>" onchange="hang_doi(this)">
                        ...
                        <div class="b6_fr2 d_flex fl_cl box_input_infor dong_doi">
                            <? if (in_array($item_td['hang'], $check_hdthoai)) {
                                echo "";
                            } else if ($item_td['hang'] == 1683) { ?>
                    ...
                    <div class="ctn_ct_b6_fr d_flex fl_row fl_st">
                        ...
                        <div class="b6_fr2 d_flex fl_cl box_input_infor">
                            <p class="b6_fr1_title p_400_s15_l18">Màu sắc <span class="cl_red">*</span></p>
                            <select class="b6_fr1_select slect-hang hd_height36" name="mau_sac">
                                <option disabled value="">Chọn</option>
                                <? while ($row_ms = mysql_fetch_assoc($list_ms->result)) { ?>
                                    <option value="<?= $row_ms['id_color'] ?>" <?= ($row_ms['id_color'] == $item_td['mau_sac']) ? 'selected' : '' ?>><?= $row_ms['mau_sac'] ?></option>
                                <? } ?>
                            </select>
                        </div>
                    </div>
    ...
    <script>
        $(".b11_btn_chinhsua").click(function() {
            ...
            if (form_pc.valid() === true) {
                ...
                var phien_ban = $("input[name='phien_ban']:checked").val();
