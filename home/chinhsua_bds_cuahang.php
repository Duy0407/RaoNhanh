...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0) {
    $acc_id = $_COOKIE['UID'];
    $acc_type = $_COOKIE['UT'];
    $query = new db_query("SELECT `new_title`,`link_title`,`new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,new_cate_id,
    ...
    $query_des = new db_query("SELECT `new_description`,`can_ban_mua`, `ten_toa_nha`, `tong_so_tang`,
                    `huong_chinh`, `giay_to_phap_ly`, `tinh_trang_noi_that`, `dien_tich`, `chieu_dai`, `chieu_rong` FROM `new_description` WHERE new_id = $id_nd");
    $sql_des = mysql_fetch_assoc($query_des->result);
    $query_dm = new db_query("SELECT `tags_id`, `ten_tags`, `id_danhmuc`, `type_tags`, `id_parent` FROM `key_tags` WHERE id_danhmuc = '" . $sql_nd['new_cate_id'] . "' 
                            AND type_tags = '" . $sql_des['can_ban_mua'] . "'  ");
...
<head>
    ...
    <title>Chỉnh sửa tin bất động sản cửa hàng</title>
        ...
        <div class="tindang-container">
            ...
            <div class="tindang-content-cont hd-disflex">
                ...
                <div class="tindang-col-right">
                    <form class="form-dtin-cont" id="form_bds_cuahang" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_nd ?>">
                        <div class="row-tin-dang danhmuc_tt">
                            <p class="font-dam hd_font15-17">Danh mục sản phẩm <span class="color_red">*</span></p>
                            <input type="text" class="dmuc-spham" readonly name="san-pham-laptop" placeholder="Bất động sản >> Cửa hàng">
                        </div>
                        ...
                        <div class="row-tin-dang  box_input_infor">
                            ...
                            <input class="input_infor_tag error" type="text" name="ten_toanha" placeholder="Tên toà nhà/Khu dân cư" value="<?= $sql_des['ten_toa_nha'] ?>">
                        </div>
                            ...
                            <div class="row-tin-dang rowflex2">
                                ...
                                <select class="slect-hang  hd_height36" name="so_tang">
                                    ...
                                    <? foreach ($result_t as $tang) : ?>
                                        <option value="<?= $tang['id'] ?>" <?= ($sql_des['tong_so_tang'] == $tang['id'] ? 'selected' : '') ?>><?= $tang['so_luong'] ?></option>
                                    <? endforeach ?>
                                </select>
                            </div>
                            <div class="row-tin-dang rowflex2">
                            ...
                            <div class="d_themdiv_gia_7_8">
                                <div class="input-gia-cont d_8-7_tclass3">
                                    <div class="box_input_infor">
                                        <input class="input_infor_tag error price" onkeyup="format_gtri(this)" <?= ($sql_nd['new_money'] == 0) ? "disabled" : "" ?> type="text" name="td_gia_spham" placeholder="" id="gia-ban-sp" autocomplete="off" oninput="<?php echo $oninput; ?>" value="<?= $sql_nd['new_money'] ?>">
                                    </div>
    ...
    $(".dang_tin").click(function() {
        ...
        form_bds_cuahang.validate({
            ...
            messages: {
                ...
                nha_muathue: "Vui lòng chọn tên tòa nhà",
                tang_so: "Vui lòng nhập tầng",
                so_tang: "Vui lòng chọn tầng",
        ...
        if (form_bds_cuahang.valid() === true) {
            ...
            var id_nd = $("#form_bds_cuahang").attr("data2");
