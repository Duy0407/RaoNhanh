...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0 && $id_dm != 0) {
    ...
    $list_tin = new db_query("SELECT `new`.`new_id`, `new_title`,`link_title`,`new_money`, `new_cate_id`, 
                                ...
                                `new_description`,`canhan_moigioi`,`thoigian_kmbd`,`thoigian_kmkt`,`soluong_min`,`soluong_max`
    ...
    if (mysql_num_rows($list_tin->result) > 0) {
        ...
        // tag danh muc
        $ban = new db_query("SELECT `tags_id`, `ten_tags` FROM `key_tags` WHERE `id_danhmuc` = 84 AND type_tags = '" . $item_td['can_ban_mua'] . "' ");
...
<head>
    ...
    <title>Chỉnh sửa Thiết kế, Phong thủy </title>
...
<body>
    ...
    <!-- content -->
    <section id="m_dangtin_thucongmynghe" class="suckhoe thucongmynghe">
        <div class="container_title">
            <p class="ctn_tt_p p_600_s24_l28">Chỉnh sửa</p>
        </div>
        <form class="form-dtin-cont form_bds_chung" id="form_tcmn_thietkephongthuy" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_nd ?>" data3="<?= $xacthuc_lket ?>">
                ...
                <div class="ctn_ct_box1 d_flex fl_cl w100">
                    ...
                    <div class="ctn_ct_b1_fr2 w100">
                        <div class="b1_fr2_title w100">
                            <p class="b1_fr2_title_p p_400_s16_l19 d_flex fl_row al_ct" data="84">
                                Thủ công - Mỹ nghệ - Quà tặng<span class="d_flex fl_row al_ct pdl_5 pdr_5">
                                    <img src="/images/m_raonhanh_imgnew/double_angle.png" alt="" class="img_16">
                                </span> Thiết kế, Phong thủy
                            </p>
                        </div>
                    </div>
                </div>
    ...
    <script>
        $(".b11_btn_chinhsua").click(function() {
            var form_tcmn_thietkephongthuy = $("#form_tcmn_thietkephongthuy");
            form_tcmn_thietkephongthuy.validate({
            ...
            if (form_tcmn_thietkephongthuy.valid() === false) {
            ...
            if (form_tcmn_thietkephongthuy.valid() === true) {
                $("#xoa_tddang_tin").removeClass("b11_btn_dangtin");
                var user_id = $("#form_tcmn_thietkephongthuy").attr("data");
                var user_type = $("#form_tcmn_thietkephongthuy").attr("data1");
                var id_nd = $("#form_tcmn_thietkephongthuy").attr("data2");
                ...
                var xac_thuc = $("#form_tcmn_thietkephongthuy").attr("data3");
                ...
                $.ajax({
                    type: 'POST',
                    url: '/ajax_tcquatang/csua_thietke.php',
                    data: fd,
                    processData: false,
                    contentType: false,
    ...
    <script>
        ...
        function xem_trc_tin() {
            var form_tcmn_thietkephongthuy = $(".form-dtin-cont");
            form_tcmn_thietkephongthuy.validate({
                errorPlacement: function(error, element) {
                    error.appendTo(element.parents(".box_input_infor"));
                    error.wrap("<span class='error'>");
