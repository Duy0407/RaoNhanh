...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0 && $id_dm != 0) {
    ...
    if (mysql_num_rows($list_tin->result) > 0) {
        ...
        // tag danh muc
        $ban = new db_query("SELECT `tags_id`, `ten_tags` FROM `key_tags` WHERE `id_danhmuc` = 44 AND type_tags = '" . $item_td['can_ban_mua'] . "' ");
...
<body>
    ...
    <section id="m_dangtin_thoitrang" class="suckhoe thoitrang">
        ...
        <form class="form-dtin-cont form_bds_chung" id="form_thoitrang_nu" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_nd ?>" data3="<?= $xacthuc_lket ?>">
    ...
    <script>
        $(".b11_btn_chinhsua").click(function() {
            ...
            if (form_thoitrang_nu.valid() === true) {
                ...
                var id_nd = $("#form_thoitrang_nu").attr("data2");
