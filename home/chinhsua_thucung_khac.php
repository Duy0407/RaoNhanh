...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_tc != 0) {
    ...
    $query = new db_query("SELECT `new_title`,`link_title`,`new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`, `new_unit`,
                `new_phone`, `new_email`, `chotang_mphi`, `quan_huyen`,`phuong_xa`, `new_sonha`, `dia_chi`, `new_video`, `new_ctiet_dmuc`
                    FROM `new` WHERE `new_id` = $id_tc ");
    $sql_nd = mysql_fetch_assoc($query->result);
    $query_des = new db_query("SELECT `new_description`,`do_tuoi`,`kich_co`,`gioi_tinh` FROM `new_description` WHERE new_id = $id_tc");
    $sql_des = mysql_fetch_assoc($query_des->result);
    $query_dm = new db_query("SELECT `tags_id`, `ten_tags`, `id_danhmuc`, `type_tags`, `id_parent` FROM `key_tags` WHERE  id_danhmuc = 115 AND id_parent = 0");
...
<head>
    ...
    <title>Chỉnh sửa tin thú cưng khác</title>
        ...
        <div class="tindang-container">
            ...
            <div class="tindang-content-cont hd-disflex">
                ...
                <div class="tindang-col-right">
                    <form class="form-dtin-cont" id="form_thucung_khac" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_tc ?>">
                        <div class="row-tin-dang danhmuc_tt">
                            <p class="font-dam hd_font15-17">Danh mục sản phẩm <span class="color_red">*</span></p>
                            <input type="text" data="<?= $sql_nd['new_cate_id'] ?>" class="dmuc-spham" readonly name="san-pham-laptop" placeholder="Thú cưng >> Thú cưng khác">
                        </div>
                        ...
                        <div class="row-tin-dang box_input_infor">
                            ...
                            <input class="input_infor_tag error" type="text" name="do_tuoi" placeholder="Nhập độ tuổi" value="<?= $sql_des['do_tuoi'] ?>">
                        </div>
                        <div class="d_flex j_between box_input_infor">
                            <div class="row-tin-dang rowflex2">
                                <p class="font-dam hd_font15-17">Kích cỡ thú cưng <span class="color_red">*</span></p>
                                <input class="input_infor_tag error" type="text" name="kichco" placeholder="Nhập kích cỡ" value="<?= $sql_des['kich_co'] ?>">
                            </div>
                            <div class="row-tin-dang rowflex2">
                                ...
                                <select class="slect-hang  hd_height36" name="gioitinh">
                                    ...
                                    <?
                                    $query_gt = new db_query("SELECT `id`, `contents_name`, `id_danhmuc`, `type` FROM `thongtin_thucung` WHERE id_danhmuc = 115 AND `type` = 3");
                        ...
                        <div class="row-tin-dang cont-btn-sb hd-disflex hide-480-mobile">
                            ...
                            <button type="button" class="btn-submit td-dang-tin thucung_khac hd_cspointer font-bold">CHỈNH SỬA</button>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    $(".thucung_khac").click(function() {
        ...
        if (form_thucung_khac.valid() === true) {
            var user_id = $("#form_thucung_khac").attr("data");
            var user_type = $("#form_thucung_khac").attr("data1");
            var id_tc = $("#form_thucung_khac").attr("data2");
            ...
            var do_tuoi = $("input[name='do_tuoi']").val();
            var kichco = $("input[name='kichco']").val();
            ...
            $.ajax({
                url: '/ajax_thucung/edit_thucung_khac.php',
                type: 'POST',
                data: fd,
                contentType: false,
                processData: false,
    ...
    function xem_trc_tin() {
        var form_thucung_khac = $(".form-dtin-cont");
        form_thucung_khac.validate({
            errorPlacement: function(error, element) {
                error.appendTo(element.parents(".box_input_infor"));
                error.wrap("<span class='error'>");
        ...
        if (form_thucung_khac.valid() === false) {
        ...
        if (form_thucung_khac.valid() === true) {
            ...
            fd.append('free_gift', free_gift);
            fd.append('tieu_de', tieu_de);
            fd.append('loai_sp', do_tuoi);
            fd.append('loai_sp2', kichco);
            fd.append('loai_sp3', gioitinh);
