...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0) {
    ...
    if (mysql_num_rows($list_tin->result) > 0) {
        ...
        // tag danh muc
        $ban = new db_query("SELECT `tags_id`, `ten_tags` FROM `key_tags` WHERE `id_danhmuc` = 112 AND type_tags = '" . $item_td['can_ban_mua'] . "' ");
...
<head>
    ...
    <title>Chỉnh sửa Thú cưng Mèo </title>
...
<body>
    ...
    <section id="m_dangtin_thucung" class="suckhoe thucung">
        ...
        <form class="form-dtin-cont form_bds_chung" id="form_tc_meo" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_nd ?>">
                ...
                <div class="b6_loaihinh giongthucung loai_dungcu d_flex fl_cl box_input_infor">
                    ...
                    <select class="slect-hang hd_height36 b6_lhinh_select giong_thucung" name="giong_thucung">
                        ...
                        <? foreach ($result_meo as $meo) : ?>
                            <option <?= ($meo['id'] == $item_td['giong_thu_cung']) ? "selected" : "" ?> value="<?= $meo['id'] ?>"><?= $meo['giong_thucung'] ?></option>
                        <? endforeach ?>
                    </select>
    ...
    <script>
        $(".b11_btn_chinhsua").click(function() {
            ...
            if (form_tc_meo.valid() === true) {
                ...
                var id_nd = $("#form_tc_meo").attr("data2");
