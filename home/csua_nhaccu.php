...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_cs != 0 && $id_dm != 0) {
    ...
    if (mysql_num_rows($check_tind->result) > 0) {
        ...
        $list_ncu = new db_query("SELECT `id`, `ten_loai` FROM `loai_chung` WHERE `id_danhmuc` = $id_dm ");
            ...
            <div class="tindang-container">
                ...
                <div class="tindang-content-cont hd-disflex">
                    ...
                    <div class="tindang-col-right" data="<?= $id_cs ?>">
                        <form class="form-dtin-cont" id="form_dv_nhaccu" data="<?= $us_id ?>" data1="<?= $us_type ?>">
                            <div class="row-tin-dang danhmuc_tt">
                                <p class="font-dam hd_font15-17">Danh mục sản phẩm <span class="color_red">*</span></p>
                                <input type="text" class="dmuc-spham" readonly name="san-pham-laptop" data="<?= $id_dm ?>" placeholder="Dịch vụ - Giải trí >> Nhạc cụ">
                            </div>
                            ...
                            <div class="row-tin-dang box_input_infor">
                                ...
                                <select class="slect-hang  hd_height36" name="loai_nhac_cu">
                                    ...
                                    <? while ($row_nc = mysql_fetch_assoc($list_ncu->result)) { ?>
                                        <option value="<?= $row_nc['id'] ?>" <?= ($row_tt['loai_chung'] == $row_nc['id']) ? "selected" : "" ?>><?= $row_nc['ten_loai'] ?></option>
                                    <? } ?>
                                </select>
                            </div>
                            <div class="row-tin-dang box_input_infor">
                            ...
                            <div class="row-tin-dang cont-btn-sb hd-disflex">
                                ...
                                <button type="button" class="btn-submit td-dang-tin nhaccu hd_cspointer font-bold">ĐĂNG TIN</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    <script>
        $(".nhaccu").click(function() {
            ...
            if (form_dv_nhaccu.valid() === true) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '/ajax_dvgiaitri/csua_nhaccu.php',
                    data: fd,
                    processData: false,
                    contentType: false,
