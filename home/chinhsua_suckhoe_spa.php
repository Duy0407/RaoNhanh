...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_sksd != 0) {
    ...
    $query = new db_query("SELECT`new_title`,`link_title`,`new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,
                        `new_create_time`, `new_type`,`new_active`,`new_unit`, `new_name`, `new_phone`, `new_email`, `new_address`, `da_ban`,`chotang_mphi`,
                        `quan_huyen`, `phuong_xa`, `new_sonha`, `dia_chi`, `new_video`, `new_ctiet_dmuc`, `new_buy_sell`,`new_update_time` FROM `new`
                        WHERE `new_id` = $id_sksd");
    $sql_nd = mysql_fetch_assoc($query->result);
    $query_des = new db_query("SELECT `new_description` FROM `new_description` WHERE new_id = $id_sksd ");
    $sql_des = mysql_fetch_assoc($query_des->result);
    $query_dm = new db_query("SELECT `tags_id`, `ten_tags`, `id_danhmuc`, `type_tags`, `id_parent` FROM `key_tags` WHERE id_danhmuc = 62");
...
<head>
    ...
    <title>Chỉnh sửa tin sức khỏe sắc đẹp Spa</title>
        ...
        <div class="tindang-container">
            ...
            <div class="tindang-content-cont hd-disflex">
                ...
                <div class="tindang-col-right">
                    <form class="form-dtin-cont" id="form_spa" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_sksd ?>">
                        <div class="row-tin-dang danhmuc_tt">
                            <p class="font-dam hd_font15-17">Danh mục sản phẩm <span class="color_red">*</span></p>
                            <input type="text" data="<?= $sql_nd['new_cate_id'] ?>" class="dmuc-spham" readonly name="san-pham-laptop" placeholder="Sức khỏe - Sắc đẹp >> Spa">
                        </div>
                        ...
                        <div class="row-tin-dang ">
                            ...
                            <div class="input-gia-cont">
                                <div class="box_input_infor bd_solid">
                        ...
                        <div class="box_input_infor mt20">
                            ...
                            <select class="slect-chitiet_dm hd_height36" style="width:100%" name="chitiet_dm">
                                ...
                                <?
                                $sql_dm = new db_query("SELECT `tags_id`, `ten_tags` FROM `key_tags` WHERE id_danhmuc = 62");
                                ?>
                                <? while ($ctiet = mysql_fetch_assoc($sql_dm->result)) { ?>
                        ...
                        <div class="row-tin-dang cont-btn-sb hd-disflex hide-480-mobile">
                            ...
                            <button type="button" class="btn-submit td-dang-tin dt_spa hd_cspointer font-bold">ĐĂNG TIN</button>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    ...
    $(".dt_spa").click(function() {
        ...
        if (form_spa.valid() === true) {
            var user_id = $("#form_spa").attr("data");
            var user_type = $("#form_spa").attr("data1");
            var id_sksd = $("#form_spa").attr("data2");
            ...
            $.ajax({
                url: '/ajax_sksd/chinhsua_sksd_spa.php',
                type: 'POST',
                data: fd,
                contentType: false,
                processData: false,
