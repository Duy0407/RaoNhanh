...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0) {
    ...
    $query = new db_query("SELECT `new_title`,`link_title`,`new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,
                    ...
                    `quan_huyen`, `phuong_xa`, `new_sonha`, `dia_chi`, `new_video`, `new_ctiet_dmuc`,`new_update_time` FROM `new` WHERE new_id = $id_nd");
    $sql_nd = mysql_fetch_assoc($query->result);
    $query_des = new db_query("SELECT `new_description`,`can_ban_mua`, `ten_toa_nha`,
                        `loai_hinh_canho`, `tang_so`, `so_pngu`,`so_pve_sinh`,`huong_chinh`, `huong_ban_cong`, `tinh_trang_bds`, `giay_to_phap_ly`,
                        `tinh_trang_noi_that`, `dien_tich`, `chieu_dai`, `chieu_rong` FROM `new_description` WHERE new_id = $id_nd");
...
<head>
    ...
    <title>Chỉnh sửa tin bất động sản căn hộ chung cư</title>
        ...
        <div class="tindang-container">
            ...
            <div class="tindang-content-cont hd-disflex">
                ...
                <div class="tindang-col-right">
                    <form class="form-dtin-cont" id="form_bds_chungcu" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_nd ?>">
                        <div class="row-tin-dang danhmuc_tt">
                            <p class="font-dam hd_font15-17">Danh mục sản phẩm <span class="color_red">*</span></p>
                            <input type="text" data="27" class="dmuc-spham" readonly name="san-pham-laptop" placeholder="Bất động sản >> Căn hộ chung cư">
                        </div>
                        ...
                        <div class="row-tin-dang box_input_infor">
                            <p class="font-dam hd_font15-17">Loại hình căn hộ <span class="color_red">*</span></p>
                            <select class="slect-hang  hd_height36" name="loaihinh">
                                <option disabled selected value="">Loại hình căn hộ</option>
                                <option value="1" <? if ($sql_des['loai_hinh_canho'] == 1) echo 'selected' ?>>Chung cư</option>
                                <option value="2" <? if ($sql_des['loai_hinh_canho'] == 2) echo 'selected' ?>>Duplex</option>
                                <option value="3" <? if ($sql_des['loai_hinh_canho'] == 3) echo 'selected' ?>>Penthouse</option>
                                <option value="4" <? if ($sql_des['loai_hinh_canho'] == 4) echo 'selected' ?>>Căn hộ dịch vụ, mini</option>
                                <option value="5" <? if ($sql_des['loai_hinh_canho'] == 5) echo 'selected' ?>>Tập thể, cư xá</option>
                                <option value="6" <? if ($sql_des['loai_hinh_canho'] == 6) echo 'selected' ?>>Officetel</option>
                            </select>
                        </div>
                        <div class="row-tin-dang box_input_infor">
                            <p class="font-dam hd_font15-17">Tình trạng bất động sản <span class="color_red">*</span></p>
                            <div class="d_flex top15 space_b">
                                <div class="checkbox1 w50 d_flex align_c">
                                    <input type="radio" class="checkbox_dacdiem dacdiem1" name="tinh_trang" <?= $sql_des['tinh_trang_bds'] == 1 ? 'checked' : '' ?>>
                                    <label class="color-blk">Đã bàn giao</label>
                                </div>
                                <div class="checkbox1 w50 d_flex align_c">
                                    <input type="radio" class="checkbox_dacdiem dacdiem2" name="tinh_trang" <?= $sql_des['tinh_trang_bds'] == 2 ? 'checked' : '' ?>>
                                    <label class="color-blk">Chưa bàn giao</label>
                                </div>
                            </div>
                        </div>
                        <div class="row-tin-dang box_input_infor">
                            <p class="font-dam hd_font15-17">Tầng số <span class="color_red">*</span></p>
                            <input type="text" name="tangso" placeholder="Tầng số" onkeydown="javascript: return ['Backspace','Delete','ArrowLeft','ArrowRight'].includes(event.code) ? true : !isNaN(Number(event.key)) && event.code!=='Space'" value="<?= $sql_des['tang_so'] ?>">
                        </div>
                            ...
                            <div class="row-tin-dang rowflex2 box_input_infor">
                                ...
                                <select class="slect-hang  hd_height36" name="so_nhavs">
                                    ...
                                    <? while ($vs = mysql_fetch_assoc($sql_vs->result)) { ?>
                                        <option value="<?= $vs['id'] ?>" <?= ($vs['id'] == $sql_des['so_pve_sinh']) ? 'selected' : "" ?>><?= $vs['so_luong'] ?></option>
                                    <? } ?>
                                </select>
                            </div>
                        </div>
                        <div class="row-tin-dang">
                            <p class="font-dam hd_font15-17">Hướng ban công</p>
                            <select class="slect-hang hd_height36" name="huong_ban_cong">
                                <option disabled selected value="">Chọn</option>
                                <option value="1" <? if ($sql_des['huong_ban_cong'] == 1) echo 'selected' ?>>Đông</option>
                                <option value="2" <? if ($sql_des['huong_ban_cong'] == 2) echo 'selected' ?>>Tây </option>
                                <option value="3" <? if ($sql_des['huong_ban_cong'] == 3) echo 'selected' ?>>Nam </option>
                                <option value="4" <? if ($sql_des['huong_ban_cong'] == 4) echo 'selected' ?>>Bắc </option>
                                <option value="5" <? if ($sql_des['huong_ban_cong'] == 5) echo 'selected' ?>>Đông bắc </option>
                                <option value="6" <? if ($sql_des['huong_ban_cong'] == 6) echo 'selected' ?>>Đông nam </option>
                                <option value="7" <? if ($sql_des['huong_ban_cong'] == 7) echo 'selected' ?>>Tây bắc </option>
                                <option value="8" <? if ($sql_des['huong_ban_cong'] == 8) echo 'selected' ?>>Tây nam </option>
                            </select>
                        </div>
                        <div class="row-tin-dang">
                        ...
                        <div class="row-tin-dang position_ral box_input_infor validate_input">
                            <p class="font-dam hd_font15-17">Diện tích <span class="color_red">*</span></p>
                            <input type="text" name="dientich" placeholder="Diện tích" onkeydown="javascript: return ['Backspace','Delete','ArrowLeft','ArrowRight'].includes(event.code) ? true : !isNaN(Number(event.key)) && event.code!=='Space'" value="<?= $sql_des['dien_tich'] ?>">
                            ...
                            <div class="d_themdiv_gia_7_8">
                                <div class="input-gia-cont d_8-7_tclass3">
                                    <div class="box_input_infor">
                                        <input class="input_infor_tag error price" type="text" <?= ($sql_nd['new_money'] == 0) ? "disabled" : "" ?> name="td_gia_spham" onkeyup="format_gtri(this)" id="gia-ban-sp" autocomplete="off" oninput="<?php echo $oninput; ?>" value="<?= $sql_nd['new_money'] ?>">
                                    </div>
...
<script>
    ...
    $(".dang_tin").click(function() {
        ...
        if (form_bds_chungcu.valid() === true) {
            ...
            var id_cc = $("#form_bds_chungcu").attr("data2");
            ...
            if (count != 0) {
                ...
                tinh_trang = (count == 1) ? ($(checkBox[0]).is(':checked') ? 1 : 2) : 3;
            }
            var tang_so = $("input[name='tangso']").val();
            var tongso_tang = $("select[name='tongso_tang']").val();
    ...
    $(".td-xem-truoc").click(function() {
        ...
        if (form_bds_chungcu.valid() === true) {
            ...
            fd.append('loai_sp8', tongso_tang);
