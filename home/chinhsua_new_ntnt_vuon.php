...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0) {
    ...
    $qr_listds = new db_query("SELECT `new`.`new_id`, `new_title`, `new_money`, `new_cate_id`, `new_city`, `new_image`, `new_unit`, `new_phone`, `new_email`,
                            ...
                            `new_description`, `canhan_moigioi` ,`thoigian_kmbd`,`thoigian_kmkt`,`soluong_min`,`soluong_max` FROM `new`
...
<head>
    ...
    <title>Chỉnh sửa nội thất vườn</title>
    ...
    <script type="text/javascript">
        $(".b11_btn_chinhsua").click(function() {
            ...
            if (form_pc.valid() === true) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '/ajax_noithat_ngoaithat/csua_vuon.php',
                    data: fd,
                    processData: false,
                    contentType: false,
