...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0) {
    $acc_id = $_COOKIE['UID'];
    $acc_type = $_COOKIE['UT'];
    $query = new db_query("SELECT `new_id`, `new_title`,`link_title`,`new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`, `new_create_time`,
                             `new_type`, `new_buy_sell`,`new_active`,`new_unit`, `new_name`, `new_phone`, `new_email`, `new_address`, `da_ban`, `quan_huyen`,
                             `phuong_xa`, `new_sonha`, `dia_chi`, `new_video`, `new_ctiet_dmuc`,`new_update_time` FROM `new` WHERE new_id = $id_nd");

    if (mysql_num_rows($query->result) > 0) {
        $sql_nd = mysql_fetch_assoc($query->result);

        $query_des = new db_query("SELECT `new_des_id`, `new_id`, `new_description`,`can_ban_mua`, `ten_toa_nha`,`loai_hinh_dat`,
                             `huong_chinh`, `giay_to_phap_ly`, `dien_tich`, `chieu_dai`, `chieu_rong` FROM `new_description` WHERE new_id = $id_nd");

        $sql_des = mysql_fetch_assoc($query_des->result);

        $can_ban_mua = $sql_des['can_ban_mua'];
        $query_dm = new db_query("SELECT `tags_id`, `ten_tags` FROM `key_tags` WHERE `id_danhmuc` = 12 AND `type_tags` = $can_ban_mua ");
...
<head>
    ...
    <title>Chỉnh sửa tin bất động sản đất</title>
        ...
        <div class="tindang-container">
            ...
            <div class="tindang-content-cont hd-disflex">
                ...
                <div class="tindang-col-right">
                    <form class="form-dtin-cont" id="form_bds_dat" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_nd ?>">
                        ...
                        <div class="row-tin-dang">
                            ...
                            <input class="input_infor_tag error" type="text" name="ten_du_an" placeholder="Tên dự án" value="<?= $sql_des['ten_toa_nha'] ?>">
                        </div>
                        ...
                        <div class="row-tin-dang box_input_infor">
                            ...
                            <select class="slect-hang  hd_height36" name="loai_hinh">
                                ...
                                <option value="1" <? if ($sql_des['loai_hinh_dat'] == 1) echo 'selected' ?>>Đất thổ cư</option>
                                <option value="2" <? if ($sql_des['loai_hinh_dat'] == 2) echo 'selected' ?>>Đất nền dự án</option>
                                <option value="3" <? if ($sql_des['loai_hinh_dat'] == 3) echo 'selected' ?>>Đất công nghiệp</option>
                                <option value="4" <? if ($sql_des['loai_hinh_dat'] == 4) echo 'selected' ?>>Đất nông nghiệp</option>
                            </select>
                        </div>
                        ...
                        <!-- chiều dài, chiều rộng người bán -->
                            ...
                            <div class="d_themdiv_gia_7_8">
                                <div class="input-gia-cont d_8-7_tclass3">
                                    <div class="box_input_infor">
                                        <input class="input_infor_tag error" type="text" name="td_gia_spham" value="<?= ($sql_nd['new_money'] != 0) ? number_format($sql_nd['new_money']) : "" ?>" <?= ($sql_nd['new_money'] != 0) ? "" : "disabled" ?> onkeyup="format_gtri(this)" id="gia-ban-sp" autocomplete="off" oninput="<?php echo $oninput; ?>">
                                    </div>
                                    <div class="money_div d_8-7_tclass5">
                                        <select class="dt-money-up donvi_ban" name="dvi_tien">
                                            <option value="1" <?= ($sql_des['new_unit'] == 1) ? 'selected' : '' ?>>VNĐ</option>
                                            <option value="2" <?= ($sql_des['new_unit'] == 1) ? 'selected' : '' ?>>USD</option>
                                            <option value="3" <?= ($sql_des['new_unit'] == 1) ? 'selected' : '' ?>>EURO</option>
                                        </select>
                                    </div>
                                </div>
                                <span class="sp-lienhe-nban d_8-7_tclass4">
                                    <? if ($sql_nd['new_money'] == 0) { ?>
                                        <input type="checkbox" name="td-lienhe_ngban" placeholder="" checked class="lien-he-ngban">
                                    <? } else { ?>
                        ...
                        <div class="row-tin-dang box_input_infor">
                            ...
                            <select id="chitiet_dm" class="slect-chitiet_dm hd_height36 slect-hang" name="chitiet_dm">
                        ...
                        <div class="row-tin-dang box_input_infor">
                            ...
                            <input readonly type="text" name="td_dia_chi" class="td_ttin_diachi input_infor_tag error" data-tt="<?= $tinh_thanh ?>" data-qh="<?= $quan_huyen ?>" data-px="<?= $phuong_xa ?>" data-sn="<?= $so_nha ?>" placeholder="Địa chỉ" value="<?= $sql_nd['dia_chi']  ?>">
                        </div>
                        <div class="row-tin-dang box_input_infor">
...
<script>
    ...
    $(".dang_tin").click(function() {
        ...
        if (form_bds_dat.valid() === true) {
            ...
            var id = $("#form_bds_dat").attr("data2");
