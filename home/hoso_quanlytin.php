...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT'])) {
    ...
    if ($type_user == 5) {
        ...
        // Tất cả tin đăng
        $all_tin_dn = new db_query("SELECT `new_id`, `new_title`, `link_title`, `new_cate_id`, `da_ban`,`sluong_daban`,`new_active` ,
        `new_money`, `new_unit`, `new_image`, `new_create_time`, `dia_chi`,
        ...
        AND `new_buy_sell` = 2 AND `new_cate_id` != 120 AND `new_cate_id` != 121 ORDER BY `new_id` DESC $limit");
        $tong_tin = new db_query("SELECT COUNT(`new_id`) as tong FROM `new` WHERE `new_type` = $usertype 
        AND `new_user_id` = $id_user AND `new_buy_sell` = 2 AND `new_cate_id` != 120 AND `new_cate_id` != 121 $limit");
        $result_tong = mysql_fetch_assoc($tong_tin->result);
        $tong = $result_tong['tong'];
        // tin đăng đăng
        $tong_tinghim = new db_query("SELECT new_id FROM `new` WHERE `new_type` = $usertype AND `new_user_id` = $id_user 
        AND `new_buy_sell` = 2 AND `new_cate_id` != 120 AND `new_cate_id` != 121 AND `da_ban` = 0 AND new_active !=0 AND tong_sluong !=0");
        ...
        // tin đã bán
        $tong_tinban = new db_query("SELECT new_id FROM `new` WHERE `new_type` = $usertype AND `new_user_id` = $id_user 
        AND `new_buy_sell` = 2 AND `new_cate_id` != 120 AND `new_cate_id` != 121 AND `da_ban` = 1");
        $result_tongtinban = mysql_num_rows($tong_tinban->result);
        // tin da an
        $tong_tindaan = new db_query("SELECT new_id FROM `new` WHERE `new_type` = $usertype AND `new_user_id` = $id_user 
        AND `new_buy_sell` = 2 AND `new_cate_id` != 120 AND `new_cate_id` != 121 AND `new_active` = 0 ");
            ...
            <div class="d_flex j_between hs_df_1">
                ...
                <div class="khoiphai_quanly khoiphai_quanly_df">
                    ...
                    <div class="d_them_div_qltin">
                        ...
                        <!-- TẤT CẢ TIN -->
                        <div class="khoi_noidungban khoi_noidungban_df_qlt">
                            <? while ($showtin = (mysql_fetch_assoc($all_tin_dn->result))) {
                                $avatar = $showtin['new_image'];
                                $avatar = explode(';', $avatar);
                                $cat_id = $showtin['new_cate_id'];
                                $id_tin = $showtin['new_id'];
                            ?>
                                <div class="khoicon_ndban d_flex fl_cl">
                                    <div class="d_them_div_flex d_flex fl_row al_ct">
                                        <div class="anh_daidientin">
                                            <a href="/<?= replaceTitle($showtin['link_title']) ?>-c<?= $showtin['new_id'] ?>.html" class="img_tinban">
                                                <img src="<?= $avatar[0] ?>" alt="" class="anh_avt_tin" onerror="this.src=`/images/anh_moi/dai_dien_avt.png`">
                                                <? if ($showtin['da_ban'] == 0 && $xacthuc_lket == 1) { ?>
                                                    <!-- đang ghim -->
                                                    <? if ($showtin['new_pin_home'] != 0 || $showtin['new_pin_cate'] != 0 || $showtin['new_day_tin'] != "") { ?>
                                                        <p class="text-main-content-ghim">ĐANG GHIM</p>
                                                        <!-- het hang -->
                                                    <? } else if ($showtin['tong_sluong'] == 0) { ?>
                                                        <p class="hethang_img">Hết hàng</p>
                                                        <!-- tin dang an -->
                                                    <? } else if ($showtin['new_active'] == 0) { ?>
                                                        <p class="dangan_img">Tin đang ẩn</p>
                                                    <? } ?>
                                                <? } else if ($showtin['da_ban'] == 0 && $xacthuc_lket == 0) { ?>
                                                    <!-- đang ghim -->
                                                    <? if ($showtin['new_pin_home'] != 0 || $showtin['new_pin_cate'] != 0 || $showtin['new_day_tin'] != "") { ?>
                                                        <p class="text-main-content-ghim">ĐANG GHIM</p>
                                                        <!-- het hang -->
                                        ...
                                        <div class="khoi_text_ban khoi_text_ban_qltin">
                                            <div class="text-tilte d_flex fl_row al_ct jtf_spb">
                                                <a href="/<?= replaceTitle($showtin['link_title']) ?>-c<?= $showtin['new_id'] ?>.html" class="title_ban title_ban_df font-16 mb20 elipsis1"><?= $showtin['new_title'] ?></a>
                                            <?if($showtin['daban']== 0 && $showtin['new_active']!=0){?>
                                                <div class="m_refresh_qltb d_flex fl_row al_ct cursor_Pt" data="<?= $id_tin ?>" data1="<?= $showtin['new_create_time'] ?>">
                                            ...
                                            <div class="time_ban_qltdn d_flex fl_row al_ct">
                                                ...
                                                <p class="font-14 font-bold time_ban time_ban_df"><?= lay_tgian($showtin['new_create_time']) ?></p>
                                            </div>
                                            <div class="address_ban_qltdn d_flex fl_row al_ct">
                                                ...
                                                <p class="font-14 address_ban address_ban_qltl_df elipsis1"><?= $showtin['dia_chi'] ?></p>
                                            </div>
                                            <div class="m_tieuthu_qlt d_flex fl_row jtf_spb">
                                                <div class="m_money_qlt">
                                                    <? if ($showtin['chotang_mphi'] == 1) { ?>
                                                        <div class="money-qlt">Cho tặng miễn phí</div>
                                                    <? } else if ($showtin['new_money'] > 0) { ?>
                                                        <div class="money-qlt d_flex fl_row al_ct">
                                                            <div class="pdr_5">
                                                                <?= number_format($showtin['new_money']) ?> <?= $arr_dvtien[$showtin['new_unit']] ?>
                                                            </div>
                                                            <? if ($xacthuc_lket == 1) { ?>
                                    ...
                                    <div class="div_3cham_bao_nut">
                                        ...
                                        <? } else { ?>
                                            ...
                                            <div class="btn_div_3cham_bao_nut btn_qlt_df btn_qlt_df_chung btn_qlt_df_3 hid_1200">
                                                <div class="m_anhientin ctn_anhien_<?= $id_tin ?>" data="<?= $id_tin ?>" data1="<?= $showtin['new_active'] ?>">
                                                    <!-- an -->
                                                    <? if ($showtin['new_active'] == 1) { ?>
                                                ...
                                                <? if ($xacthuc_lket == 0 && $showtin['new_active'] == 1) { ?>
                                                    <span class="m_daban_cxtlk cursor_Pt df_daban" data="<?= $id_tin ?>" data1="<?= $showtin['da_ban'] ?>">
                                                ...
                                                <? if ($showtin['new_pin_home'] == 0 && $showtin['new_pin_cate'] == 0 && $showtin['new_day_tin'] == "") { ?>
                                                    <? if ($xacthuc_lket == 1  && $showtin['new_active'] == 0) { ?>
                                                        <span class="m_boder_bannhanhhon border-gt btn_qlt_df_3_d <?= ($xacthuc_lket == 1 && $showtin['new_active'] == 0) ? "op0_5" : "aaaa" ?>">
                                                    ...
                                                    <? } else if ($xacthuc_lket == 1  && $showtin['new_active'] == 1) { ?>
                                                        <span class="m_boder_bannhanhhon border-gt btn_qlt_df_3_d <?= ($xacthuc_lket == 1 && $showtin['new_active'] == 1) ? "op0_5" : "aaaa" ?>">
                                                            <div class="btn-gt cus_poi text-gt d_flex fl_row al_ct" data="<?= $id_tin ?>">
                                                                <img src="/images/m_raonhanh_imgnew/cartspeed.png" alt="Bán nhanh hơn" class="m_bannhanhhon_qlt img_18">
                                                                <span class="text-gt m_txt_bannh p_400_s15_l18" data="<?= $id_tin ?>">Bán nhanh hơn</span>
                                                            </div>
                                                        </span>
                                                    <? } else if ($xacthuc_lket == 0  && $showtin['new_active'] == 0) { ?>
                                                ...
                                                <span class="border-st btn_qlt_df_3_d m_dn_st">
                                                    <a href="/<?= duong_dan($id_tin, $cat_id) ?>">
                                                        <div class="btn-st cus_poi d_flex fl_row m_dn_btnst">
                                                            <img data-src="/images/m_raonhanh_imgnew/edit-cam.png" src="/images/m_raonhanh_imgnew/edit-2.svg" class="edit-news img_18">
                        ...
                        <div class="m_phantrang w100 d_flex al_ct jtf_ct">
                            <ul class="m_pt_ul d_flex fl_row al_ct">
                                <?= generatePageBar3('', $page, $current_page, $tong, $url, '?', '', 'active', 'preview', '<', 'next', '>', '', '', '', ''); ?>
                            </ul>
                        </div>
                        <!--  -->
                    </div>
