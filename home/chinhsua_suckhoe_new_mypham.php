...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0) {
    ...
    if (mysql_num_rows($list_tin->result) > 0) {
        ...
        // tag danh muc
        $ban = new db_query("SELECT `tags_id`, `ten_tags` FROM `key_tags` WHERE `id_danhmuc` = 61 AND type_tags = '" . $item_td['can_ban_mua'] . "' ");
...
<body>
    ...
    <!-- content -->
    <section id="m_dangtin_sk_mypham" class="suckhoe" data="<?= $hom_nay ?>">
        <div class="container_title">
            <p class="ctn_tt_p p_600_s24_l28">Chỉnh sửa</p>
        </div>
        <form class="form-dtin-cont form_bds_chung" id="form_mypham" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_nd ?>" data3="<?= $xacthuc_lket ?>">
            <div class="container_content">
                    ...
                    <select id="loai_hinh" class="b6_lhinh_select slect-hang  hd_height36" name="loaihinh">
                        ...
                        <? foreach ($db_lchung as $rows) {
                            if ($rows['id_danhmuc'] == 61) { ?>
                                <option value="<?= $rows['id'] ?>" <?= ($rows['id'] == $item_td['loai_hinh_sp']) ? "selected" : "" ?>><?= $rows['ten_loai'] ?></option>
                        <? }
                        } ?>
                    </select>
                </div>
                ...
                <div class="b6_loaimypham d_flex fl_cl box_input_infor">
                    ...
                    <select id="loai_mypham" class="b6_lmp_select slect-hang hd_height36" name="loai_mypham">
                        <? foreach ($db_lchung as $rows) {
                            if ($rows['id_cha'] == $item_td['loai_hinh_sp']) { ?>
                ...
                <div class="b6_hangmypham d_flex fl_cl box_input_infor">
                    ...
                    <select id="hang_mypham" class="slect-hang  hd_height36" name="hang">
                        <? foreach ($db_hang as $rows) {
                            if ($rows['id_parent'] == $item_td['loai_hinh_sp']) { ?>
                                <option value="<?= $rows['id'] ?>" <?= ($rows['id'] == $item_td['hang']) ? "selected" : "" ?>><?= $rows['ten_hang'] ?></option>
                        <? }
                        } ?>
                    </select>
                </div>
    ...
    <script>
        ...
        $(".b11_btn_chinhsua").click(function() {
            ...
            if (form_mypham.valid() === true) {
                ...
                var id_nd = $("#form_mypham").attr("data2");
