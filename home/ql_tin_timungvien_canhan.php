...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT'])) {
    ...
    if ($type_user == 1 || $type_user == 5) {
        ...
        switch ($type_list) {
            case 1: // tin đang tìm ứng viên - da_ban = 0
                ...
                WHERE `new_type` = $usertype AND `new_user_id` = $id_user AND `new_cate_id` = 120 AND `da_ban` = 0 AND `new_active` != 0 ORDER BY `new_id` DESC $limit");
                break;
            case 2: // tin dừng tìm ứng viên - da_ban = 1
                $all_tin = new db_query("SELECT new.`new_id`,`new_title`,`link_title`,`new_cate_id`,`da_ban`,`new_money`,`gia_kt`,`new_unit`,`new_image`,`new_create_time`,`dia_chi`,`chotang_mphi`,`new_pin_cate`,`new_pin_home`,`new_active`,`han_su_dung`
                FROM `new` LEFT JOIN `new_description` ON `new`.`new_id` = `new_description`.`new_id`
                WHERE `new_type` = $usertype AND `new_user_id` = $id_user AND `new_cate_id` = 120 AND `da_ban` = 1 AND `new_active` != 0 ORDER BY `new_id` DESC $limit");
                break;
            case 3: // tin dừng tìm ứng viên - new_active = 0 
                $all_tin = new db_query("SELECT new.`new_id`,`new_title`,`link_title`,`new_cate_id`,`da_ban`,`new_money`,`gia_kt`,`new_unit`,`new_image`,
                    ...
                    FROM `new` LEFT JOIN `new_description` ON `new`.`new_id` = `new_description`.`new_id`
                    WHERE `new_type` = $usertype AND `new_user_id` = $id_user AND `new_cate_id` = 120 AND `new_active` = 0 ORDER BY `new_id` DESC $limit");
                break;
            default: // tất cả tin đăng
                $type_list = 0;
                $all_tin = new db_query("SELECT new.`new_id`,`new_title`,`link_title`,`new_cate_id`,`da_ban`,`new_money`,`gia_kt`,`new_unit`,`new_image`,`new_create_time`,`dia_chi`,`chotang_mphi`,`new_pin_cate`,`new_pin_home`,`new_active`,`han_su_dung`
                FROM `new` LEFT JOIN `new_description` ON `new`.`new_id` = `new_description`.`new_id`
                WHERE `new_type` = $usertype AND `new_user_id` = $id_user AND `new_cate_id` = 120 ORDER BY `new_id` DESC $limit");
                break;
        }
        // tất cả tin đăng
        $tongtin = new db_query("SELECT new_id FROM `new` WHERE `new_type` = $usertype AND `new_user_id` = $id_user AND `new_cate_id` = 120 ");
        $tongtin = mysql_num_rows($tongtin->result);
        // tin đang tìm ứng viên - da_ban = 0
        $tinconhan = new db_query("SELECT new.new_id FROM `new` LEFT JOIN `new_description` ON new.new_id=new_description.new_id WHERE `new_type` = $usertype AND `new_user_id` = $id_user AND `new_cate_id` = 120 AND `new_active` != 0 AND `da_ban` = 0");
        $tinconhan = mysql_num_rows($tinconhan->result);
        // tin dừng tìm ứng viên - da_ban = 1
        $tinhethan = new db_query("SELECT new.new_id FROM `new` LEFT JOIN `new_description` ON new.new_id=new_description.new_id WHERE `new_type` = $usertype AND `new_user_id` = $id_user AND `new_cate_id` = 120 AND `new_active` != 0 AND `da_ban` = 1");
        $tinhethan = mysql_num_rows($tinhethan->result);
        // tin dừng tìm việc - new_active = 0
        $tindaantuv = new db_query("SELECT new.new_id FROM `new` LEFT JOIN `new_description` ON new.new_id=new_description.new_id WHERE `new_type` = $usertype AND `new_user_id` = $id_user AND `new_cate_id` = 120 AND `new_active` = 0");
        $result_tindaantuv = mysql_num_rows($tindaantuv->result);
...
<head>
    ...
    <title>Quản lý tin tìm ứng viên làm | RAONHANH365.VN</title>
...
<body>
    ...
    <section class="news_managenment_post d_flex tca_tdang_cnhan">
        ...
        ?>
        <div class="box-right" id="m_tuv_cn">
            <div class="box_rightdf" data="<?= $id_user ?>" data2="<?= $type_user ?>">
                <div class="scroll themvao">
                    <div class="title-qlt">
                        <span class="all-posts">
                            <a href="/ho-so-<?= ($type_user == 1) ? "nguoi-ban-ca-nhan/" : "" ?>quan-ly-tin-tim-ung-vien.html" class="text-title-qlt <?= ($type_list == 0) ? "active" : "" ?>">Tất cả tin đăng (<?= $tongtin; ?>)</a>
                        </span>
                        <span class="postting">
                            <a href="/ho-so-<?= ($type_user == 1) ? "nguoi-ban-ca-nhan/" : "" ?>quan-ly-tin-tim-ung-vien.html?typelist=1" class="text-title-qlt <?= ($type_list == 1) ? "active" : "" ?>">Tin đang tìm ứng viên (<?= $tinconhan ?>)</a>
                        </span>
                        <span class="news-sold">
                            <a href="/ho-so-<?= ($type_user == 1) ? "nguoi-ban-ca-nhan/" : "" ?>quan-ly-tin-tim-ung-vien.html?typelist=2" class="text-title-qlt <?= ($type_list == 2) ? "active" : "" ?>">Tin đã tìm ứng viên (<?= $tinhethan ?>)</a>
                        </span>
                        <span class="news-sold">
                            <a href="/ho-so-<?= ($type_user == 1) ? "nguoi-ban-ca-nhan/" : "" ?>quan-ly-tin-tim-ung-vien.html?typelist=3" class="text-title-qlt <?= ($type_list == 3) ? "active" : "" ?>">Tin đã ẩn (<?= $result_tindaantuv ?>)</a>
                        </span>
                    </div>
                </div>
                <ul class="content-qlt" id="all-posts">
                    ...
                    ?>
                        <li class="main-content main_contentdf">
                            <div class="chu d_flex fl_cl">
                                <div class="box_chu d_flex fl_row al_ct">
                                    <div class="anh anh_qltin_df">
                                        <? if ($showtin_nguoiban['new_pin_home'] != 0 || $showtin_nguoiban['new_pin_cate'] != 0 || $showtin_nguoiban['new_day_tin'] != "") { ?>
                                ...
                                <div class="btn_qlt_df btn_qlt_df_chung btn_qlt_df_3 hid_1200">
                                    ...
                                    <div class="m_anhientin ctn_anhien_<?= $id_tin ?>" data="<?= $id_tin ?>" data1="<?= $showtin_nguoiban['new_active'] ?>">
                                        ...
                                        <? if ($showtin_nguoiban['new_active'] == 1) { ?>
                                            <span class="m_antin cursor_Pt">
                                                <div class="d_flex fl_row al_ct">
                                                    <img src="/images/m_raonhanh_imgnew/eye-slash.svg" alt="Ẩn tin" class="m_antin_qlt img_20 ">
                                        ...
                                        <? } else { ?>
                                            ...
                                            <span class="m_antin cursor_Pt">
                                                <div class="d_flex fl_row al_ct">
                                                    <img src="/images/m_raonhanh_imgnew/eye.svg" alt="Hiện tin" class="m_hientin_qlt img_20 ">
                                    ...
                                    <? if ($showtin_nguoiban['da_ban'] == 1) { ?>
                                        <span class="border-dbl btn_qlt_df_3_d m_tuv_btn_dtl">
                                            <div class="btn-dbl btn_dbldf cus_poi m_tuv_dtl" data="<?= $id_tin ?>" data-update="<?= ($showtin_nguoiban['da_ban'] == 1) ? "sold" : (($showtin_nguoiban['han_su_dung'] <= $time) ? "/" . duong_dan($id_tin, $cat_id) : "") ?>">
                                                <img data-src="/images/newImages/rotate-left.png" src="/images/newImages/rotate-left.png" class="rotate-left">
                                                <span class="text-qlt m_tuv_txt_dtl">Đăng tìm lại</span>
                                            </div>
                                        </span>
                                    <? } else { ?>
                                        <span class="border-ddldb btn_qlt_df_3_d m_tuv_btn_dtuv">
                                            <div class="cus_poi btn-dtv m_tuv_dtuv" data="<?= $id_tin ?>" data2="<?= $daban ?>">
                                                <img data-src="/images/newImages/verify.png" src="/images/newImages/verify.png" class="verify">
                                                <span class="text-qlt m_tuv_txt_dtuv">Đã tìm ứng viên</span>
                                            </div>
                                        </span>
                                        <? if ($showtin_nguoiban['new_pin_home'] == 0 && $showtin_nguoiban['new_pin_cate'] == 0 && $showtin_nguoiban['new_day_tin'] == "") { ?>
                                            <span class="border-gt btn_qlt_df_3_d m_tuv_btn_tnh">
                                                <div class="btn-gt cus_poi m_tuv_tnh">
                                                    <img data-src="/images/newImages/fluent_pin-48-regular.png" src="/images/newImages/fluent_pin-48-regular.png" class="fluent_pin">
                                                    <span class="text-gt m_tuv_txt_tnh" data="<?= $id_tin ?>">Tuyển nhanh hơn</span>
                                                </div>
                                            </span>
                                        <? } ?>
                                        <span class="border-st btn_qlt_df_3_d m_tuv_st_box">
                                            <a href="/<?= duong_dan($id_tin, $cat_id) ?>" class="m_tuv_btn_st">
                                                <div class="btn-st cus_poi m_tuv_st">
                                                    <img data-src="/images/newImages/edit-2.png" src="/images/newImages/edit-2.png" class="edit-news">
                                                    <span class="text-qlt m_tuv_txt_st">Sửa tin</span>
                                                </div>
                                            </a>
                                        </span>
                                    <? } ?>
    ...
    <script type="text/javascript">
        ...
        $('.btn-dtv').click(function() {
            ...
            $(".danhdaudaban .text_title_modal").html("Đánh dấu là đã tìm được ứng viên");
