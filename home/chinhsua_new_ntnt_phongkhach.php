...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0) {
    ...
    $qr_listds = new db_query("SELECT `new`.`new_id`, `new_title`, `new_money`, `new_cate_id`, `new_city`, `new_image`, `new_unit`, `new_phone`, `new_email`,
                            ...
                            `new_description`, `nhom_sanpham`, `loai_sanpham`, `chat_lieu`, `canhan_moigioi` 
    ...
    if (mysql_num_rows($qr_listds->result) > 0) {
        ...
        $qr_nhom = new db_query("SELECT `id`, `name`  FROM `nhom_sanpham` WHERE `id_danhmuc` = " . $id_dm);

        if ($item_td['nhom_sanpham'] == 2 || $item_td['nhom_sanpham'] == 3 || $item_td['nhom_sanpham'] == 5) {
            $qr_loai = new db_query("SELECT `id`, `ten_loai` FROM loai_chung WHERE id_cha = " . $item_td['nhom_sanpham'] . " AND id_danhmuc = " . $id_dm . " ");
        };

        if ($item_td['nhom_sanpham'] != 5) {
            $qr_clieu = new db_query("SELECT `id`, `name` FROM nhom_sanpham_chatlieu WHERE id_cha = " . $item_td['nhom_sanpham'] . "  AND id_danhmuc = " . $id_dm . "");
        };

        if ($item_td['nhom_sanpham'] != 4 && $item_td['nhom_sanpham'] != 6 && $item_td['nhom_sanpham'] != 7) {
...
<head>
    ...
    <title>Chỉnh sửa tin đăng nội thất phòng khách</title>
...
<body>
                ...
                <div class="b6_nl_title d_flex fl_cl">
                    <div class="d_flex fl_row fl_st w_100">
                        <div class="d_flex fl_cl box_input_infor w_100">
                            <p class="b6_fr1_title p_400_s15_l18">Nhóm sản phẩm <span class="cl_red">*</span></p>
                        ...
                        <? if ($item_td['nhom_sanpham'] == 2 || $item_td['nhom_sanpham'] == 3 || $item_td['nhom_sanpham'] == 5) { ?>
                            <div class="d_flex fl_cl box_input_infor w_100 b6_nl_title">
                                ...
                                <select class="b6_fr1_select slect-hang hd_height36 loai_sanpham" name="loai_sanpham">
                                    ...
                                    <? while ($row_loai = mysql_fetch_assoc($qr_loai->result)) {  ?>
                                        <option value="<?= $row_loai['id'] ?>" <?= ($row_loai['id'] == $item_td['loai_sanpham']) ? 'selected' : '' ?>>
                                            <?= $row_loai['ten_loai'] ?>
                                        </option>
                                    <? } ?>
                                </select>
                        ...
                        if ($item_td['nhom_sanpham'] != 5) { ?>
                            <div class="d_flex fl_cl box_input_infor w_100 b6_nl_title">
                                ...
                                <select class="b6_fr1_select slect-hang hd_height36 chat_lieu" name="chat_lieu">
                                    <option disabled value="">Chọn</option>
                                    <? while ($row_cl = mysql_fetch_assoc($qr_clieu->result)) {  ?>
                                        <option value="<?= $row_cl['id'] ?>" <?= ($row_cl['id'] == $item_td['chat_lieu']) ? 'selected' : '' ?>>
                                            <?= $row_cl['name'] ?>
                                        </option>
                                    <? } ?>
                                </select>
    ...
    <script type="text/javascript">
        var id_nhom = $(".nhom_spham").val();
        if (id_nhom == 4 || id_nhom == 6 || id_nhom == 7) {
            $(".ctn_ct_box10").hide();
        };
        ...
        $(".b11_btn_chinhsua").click(function() {
            ...
            form_pc.validate({
                ...
                messages: {
                    ...
                    loai_sanpham: {
                        required: "Vui lòng chọn lọa sản phẩm",
                    },
                    chat_lieu: {
            ...
            if (form_pc.valid() === true) {
                ...
                if (nhom_spham == 2 || nhom_spham == 3 || nhom_spham == 5) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '/ajax_noithat_ngoaithat/csua_phongkhach.php',
                    data: fd,
                    processData: false,
                    contentType: false,
                    success: function(data) {
