...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT'])) {
    ...
    $list_dh = new db_query("SELECT `dh_id`,`id_nguoi_dh`, `id_nguoi_ban`, `id_spham`, `ghi_chu`, `loai_ttoan`, `tien_ttoan`, `dia_chi_nhanhang`, `trang_thai`,
                        `ly_do_hdon`, `usc_name`, `usc_type`, `chat365_id`, `usc_id`, `usc_phone`, `usc_logo`, `new_title`, `new_money`, `new_unit`,
                        `new_user_id`, `new_type`, `new_image` FROM `dat_hang`
                        INNER JOIN `new` ON `new`.`new_id` = `dat_hang`.`id_spham`
                        INNER JOIN `user` ON `user`.`usc_id` = `dat_hang`.`id_nguoi_ban`
                        WHERE `id_nguoi_dh` = $id_user AND (`trang_thai` = 5 OR `nguoimua_huydh` = 1) ORDER BY `tgian_huydhang` DESC ");
                            ...
                            while ($row_tin = mysql_fetch_assoc($list_dh->result)) {
                                ...
                                <div class="content-item-1 scroll-ngang">
                                    ...
                                    <div class="box-right-content-bot">
                                        <a href="chi-tiet-don-huy-nguoi-mua.html">
                                            <div class="bot-l">
                                                <div class="content-bot-l anhdaid_tin">
                                                    <img src="<?= $anh_tin[0] ?>" alt="">
                                                </div>
                                                <div class="content-bot-c">
                                                    <a href="/chi-tiet-don-huy-nguoi-mua-<?= $row_tin['dh_id'] ?>.html" class="ten_sp color_orange"><?= $row_tin['new_title'] ?></a>
                                                    <p class="text_sp">Mã đơn hàng: <?= $row_tin['id_spham'] ?></p>
                                                </div>
                                            </div>
                                        </a>
                                    </div>
                                    <div class="content-bot-donhuy">
