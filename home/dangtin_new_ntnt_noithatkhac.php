...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT'])) {
    ...
    $qr_chatlieu = new db_query("SELECT `id`, `name` FROM nhom_sanpham_chatlieu WHERE id_danhmuc = " . $id);
                ...
                <div class="b6_nl_title d_flex fl_cl">
                    <div class="d_flex fl_row fl_st w_100">
                        <div class="d_flex fl_cl box_input_infor w_100">
                            ...
                            <select class="b6_fr1_select slect-hang hd_height36 chat_lieu" name="chat_lieu" data="<?= $id ?>">
                                <option disabled selected value="">Chọn</option>
                                <? while ($row_clieu = mysql_fetch_assoc($qr_chatlieu->result)) { ?>
                                    <option value="<?= $row_clieu['id'] ?>"><?= $row_clieu['name'] ?></option>
                                <? } ?>
                            </select>
                        </div>
                    </div>
    ...
    <script type="text/javascript">
        $(".b11_btn_dangtin").click(function() {
            ...
            if (form_pc.valid() === true) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '/ajax_noithat_ngoaithat/dtin_noithatkhac.php',
                    data: fd,
                    processData: false,
                    contentType: false,
