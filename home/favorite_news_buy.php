...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT'])) {
    ...
    if ($type_user == 1) {
        $tinyt_ngban = new db_query("SELECT `new_title`, `new_type`, `chotang_mphi`, `new_money`, `gia_kt`, `new_create_time`, `new_cate_id`, `new_unit`,
                                ...
                                WHERE t.`user_id` = $id_user AND t.`usc_type` = $type_user AND n.`new_buy_sell` = 1 ORDER BY `id` DESC ");
    } else {
        header('Location: /');
    }
} else {
                        ...
                        <ul class="content-ul-fav" id="all-posts">
                            ...
                            ?>
                                <li class="main-content">
                                    <div class="anh">
                                        <a href=""><img class="img-fav lazyload" src="/images/newImages/mau-1.png" data-src="<?= $avata_tinyt[0] ?>" onerror="this.src=`/images/anh_moi/dai_dien_avt.png`"></a>
                                    </div>
                                    <div class="chu tca_ythich_tin">
                                        <div class="text-tilte">
                                            <a href="/<?= replaceTitle($row_nb['new_title']) ?>-ct<?= $row_nb['new_id'] ?>.html" class="text-title-fav text_ellipsis">
                                                <?= $row_nb['new_title'] ?>
                                            </a>
                                        </div>
                                        <div class="text-btn">
                                                ...
                                                <div class="ythich_tienyc">
                                                    <div class="money-fav">
                                                        ...
                                                        <? } else if ($row_nb['new_money'] != 0 && $row_nb['gia_kt'] == 0) { ?>
                                                            Từ <?= number_format($row_nb['new_money']) ?> <?= $arr_dvtien[$row_nb['new_unit']] ?>
                                                        <? } else if ($row_nb['new_money'] == 0 && $row_nb['gia_kt'] != 0) { ?>
                                                            Đến <?= number_format($row_nb['gia_kt']) ?> <?= $arr_dvtien[$row_nb['new_unit']] ?>
                                                        <? } else { ?>
                                                            Thỏa thuận
                                                        <? } ?>
                                                    </div>
