...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0) {
    ...
    if (mysql_num_rows($list_tin->result) > 0) {
        ...
        // tag danh muc
        $ban = new db_query("SELECT `tags_id`, `ten_tags` FROM `key_tags` WHERE `id_danhmuc` = 115 AND type_tags = '" . $item_td['can_ban_mua'] . "' ");
        $result_ban = ($ban->result_array());

        $sql_khac = new db_query("SELECT `id`, `giong_thucung` FROM `giong_thu_cung` WHERE id_danhmuc = 115");
        $result_khac = $sql_khac->result_array();
    } else {
        header('Location: /');
    }
} else {
...
<head>
    ...
    <title>Chỉnh sửa Thú cưng khác</title>
...
<body>
    ...
    <section id="m_dangtin_thucung" class="suckhoe thucung">
        ...
        <form class="form-dtin-cont form_bds_chung" id="form_tc_khac" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_nd ?>">
            <div class="container_content">
                ...
                <div class="ctn_ct_box1 d_flex fl_cl w100">
                    ...
                    <div class="ctn_ct_b1_fr2 w100">
                        <div class="b1_fr2_title w100">
                            <p class="b1_fr2_title_p p_400_s16_l19 d_flex fl_row al_ct" data="115">
                ...
                <div class="b6_loaihinh giongthucung loai_dungcu d_flex fl_cl box_input_infor">
                    ...
                    <select class="slect-hang hd_height36 b6_lhinh_select giong_thucung" name="giong_thucung">
                        ...
                        <? foreach ($result_khac as $khac) : ?>
                            <option <?= ($khac['id'] == $item_td['giong_thu_cung']) ? "selected" : "" ?> value="<?= $khac['id'] ?>"><?= $khac['giong_thucung'] ?></option>
                        <? endforeach ?>
                    </select>

                </div>
                <!-- do tuoi -->
                <div class="dotuoi_thucung d_flex fl_cl box_input_infor m_input_chung pd_t_20">
                    <p class="b6_lhinh_txt p_400_s15_l18">
                        Độ tuổi <span class="cl_red">*</span>
                    </p>
                    <input type="text" value="<?= $item_td['do_tuoi'] ?>" class="do_tuoi input_infor_tag" name="do_tuoi" placeholder="Nhập độ tuổi">
                </div>
                <!-- kich co -->
                <div class="kichco_thucung d_flex fl_cl box_input_infor m_input_chung pd_t_20">
                    <p class="b6_lhinh_txt p_400_s15_l18">
                        Kích cỡ thú cưng <span class="cl_red">*</span>
                    </p>
                    <input type="text" value="<?= $item_td['kich_co'] ?>" class="kichco input_infor_tag" name="kichco" placeholder="Nhập Kích cỡ thú cưng">
                </div>
                <!-- do tuoi -->
                <div class="b6_loaihinh gioitinh_thucung d_flex fl_cl box_input_infor pd_t_20">
                    ...
                    <select class="slect-hang hd_height36 b6_lhinh_select gioitinh" name="gioitinh">
                        ...
                        <? foreach ($db_ttthucung as $gt) {
                            if ($gt['id_danhmuc'] == 115 && $gt['type'] == 3) { ?>
    ...
    <script>
        $(".b11_btn_chinhsua").click(function() {
            var form_tc_khac = $("#form_tc_khac");
            form_tc_khac.validate({
            ...
            if (form_tc_khac.valid() === false) {
                $("#xoa_tddang_tin").addClass("b11_btn_dangtin");
                event.preventDefault();
                event.stopPropakhaction();
            ...
            if (form_tc_khac.valid() === true) {
                $("#xoa_tddang_tin").removeClass("b11_btn_dangtin");
                var user_id = $("#form_tc_khac").attr("data");
                var user_type = $("#form_tc_khac").attr("data1");
                var id_dm = $(".b1_fr2_title_p").attr('data');
                var id_nd = $("#form_tc_khac").attr("data2");
    ...
    <script>
        ...
        function xem_trc_tin() {
            var form_tc_khac = $(".form-dtin-cont");
            form_tc_khac.validate({
                errorPlacement: function(error, element) {
                    error.appendTo(element.parents(".box_input_infor"));
                    error.wrap("<span class='error'>");
