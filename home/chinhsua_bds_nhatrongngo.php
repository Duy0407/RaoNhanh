...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0) {
    $acc_id = $_COOKIE['UID'];
    $acc_type = $_COOKIE['UT'];
    $query = new db_query("SELECT `new_title`,`link_title`,`new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`, `new_type`, `new_unit`,
                        `new_name`, `new_phone`, `new_email`, `new_address`, `quan_huyen`, `phuong_xa`, `new_sonha`, `dia_chi`, `new_video`, `new_ctiet_dmuc`
                        FROM `new` WHERE new_id = $id_nd");
    $sql_nd = mysql_fetch_assoc($query->result);

    $query_des = new db_query("SELECT `new_description`,`can_ban_mua`, `ten_toa_nha`, `tong_so_tang`,`huong_chinh`, `so_pngu`,`so_pve_sinh`,
                            `giay_to_phap_ly`, `tinh_trang_noi_that`,`dac_diem`,
                            `dien_tich`, `chieu_dai`, `chieu_rong` FROM `new_description` WHERE new_id = $id_nd");
...
<head>
    ...
    <title>Chỉnh sửa tin bất động sản nhà trong ngõ</title>
        ...
        <div class="tindang-container">
            ...
            <div class="tindang-content-cont hd-disflex">
                ...
                <div class="tindang-col-right">
                    <form class="form-dtin-cont" id="form_bds_nhatrongngo" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_nd ?>">
                        <div class="row-tin-dang danhmuc_tt">
                            <p class="font-dam hd_font15-17">Danh mục sản phẩm <span class="color_red">*</span></p>
                            <input type="text" data="26" class="dmuc-spham" readonly name="san-pham-laptop" placeholder="Bất động sản >> Nhà trong ngõ">
                        </div>
                            ...
                            <div class="row-tin-dang rowflex2">
                                ...
                                <select class="slect-hang  hd_height36" name="so_tang">
                                    ...
                                    <? while ($tang = mysql_fetch_assoc($sql_tang->result)) { ?>
                                        <option value="<?= $tang['id'] ?>" <?= ($tang['id'] == $sql_des['tong_so_tang']) ? 'selected' : "" ?>><?= $tang['so_luong'] ?></option>
                                    <? } ?>
                                </select>
                            </div>
                            <div class="row-tin-dang rowflex2">
                            ...
                            <div class="d_themdiv_gia_7_8">
                                <div class="input-gia-cont d_8-7_tclass3">
                                    <div class="box_input_infor">
                                        <input class="input_infor_tag error price" type="text" onkeyup="format_gtri(this)" <?= ($sql_nd['new_money'] == 0) ? "disabled" : "" ?> name="td_gia_spham" id="gia-ban-sp" autocomplete="off" oninput="<?php echo $oninput; ?>" value="<?= $sql_nd['new_money'] ?>">
                                    </div>
...
<script>
    ...
    $(".dang_tin").click(function() {
        ...
        if (form_bds_nhatrongngo.valid() === true) {
            ...
            var id = $("#form_bds_nhatrongngo").attr("data2");
