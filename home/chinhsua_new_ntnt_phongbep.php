...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_nd != 0) {
    ...
    if (mysql_num_rows($qr_listds->result) > 0) {
        ...
        if ($item_td['nhom_sanpham'] == 18 || $item_td['nhom_sanpham'] == 19) {
        ...
        if ($item_td['nhom_sanpham'] != 20) {
            $list_ktag = new db_query("SELECT `tags_id`, `ten_tags` FROM `key_tags` WHERE `id_danhmuc` = $id_dm AND `id_parent` = '" . $item_td['nhom_sanpham'] . "' ");
...
<head>
    ...
    <title>Chỉnh sửa nội thất phòng bếp</title>
    ...
    <script type="text/javascript">
        ...
        $(".b11_btn_chinhsua").click(function() {
            ...
            if (form_pc.valid() === true) {
                ...
                if (nhom_spham == 18 || nhom_spham == 19) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '/ajax_noithat_ngoaithat/csua_phongbep.php',
                    data: fd,
                    processData: false,
                    contentType: false,
                    success: function(data) {
