...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_sksd != 0) {
    ...
    $query = new db_query("SELECT `new_title`,`link_title`,`new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,
                                `new_unit`, `new_phone`, `new_email`, `chotang_mphi`,
                                `quan_huyen`, `phuong_xa`, `new_sonha`, `dia_chi`, `new_video`, `new_ctiet_dmuc`, `new_buy_sell` FROM `new`  WHERE `new_id` = $id_sksd ");
    $sql_nd = mysql_fetch_assoc($query->result);
    $query_des = new db_query("SELECT `new_description`, `loai_sanpham`, `han_su_dung` FROM `new_description` WHERE new_id = $id_sksd");
    ...
    $query_lsp = new db_query("SELECT `id`, `ten_loai`, `id_cha`, `id_danhmuc` FROM `loai_chung` WHERE id_cha = 109");
...
<head>
    ...
    <title>Chỉnh sửa tin sức khỏe sắc đẹp Thực phẩm chức năng</title>
        ...
        <div class="tindang-container">
            ...
            <div class="tindang-content-cont hd-disflex">
                ...
                <div class="tindang-col-right">
                    <form class="form-dtin-cont" id="form_tpchucnang" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_sksd ?>">
                        <div class="row-tin-dang danhmuc_tt">
                            <p class="font-dam hd_font15-17">Danh mục sản phẩm <span class="color_red">*</span></p>
                            <input type="text" data="<?= $sql_nd['new_cate_id'] ?>" class="dmuc-spham" readonly name="san-pham-laptop" placeholder="Sức khỏe - Sắc đẹp >> Thực phẩm chức năng">
                        </div>
                        ...
                        <div class="row-tin-dang box_input_infor">
                            <p class="font-dam hd_font15-17">Loại thực phẩm chức năng <span class="color_red">*</span></p>
                            <select class="slect-hang  hd_height36" name="loai_thucpham">
                                <option disabled selected value="">Loại thực phẩm chức năng</option>
                        ...
                        <div class="row-tin-dang box_input_infor">
                            ...
                            <input class="input_infor_tag error" type="date" name="han_sd" placeholder="Nhập tiêu đề" value="<?= date('Y-m-d', $sql_des['han_su_dung']) ?>">
                        </div>
                        ...
                        <div class="box_input_infor">
                            ...
                            <select class="slect-chitiet_dm hd_height36" style="width:100%" name="chitiet_dm">
                                ...
                                <?
                                $query_dm = new db_query("SELECT `tags_id`, `ten_tags` FROM `key_tags` WHERE id_danhmuc = 109");
                                ?>
                        ...
                        <div class="row-tin-dang cont-btn-sb hd-disflex hide-480-mobile">
                            ...
                            <button type="button" class="btn-submit td-dang-tin tpchucnang hd_cspointer font-bold">CHỈNH SỬA</button>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    $(".tpchucnang").click(function() {
        ...
        if (form_tpchucnang.valid() === true) {
            ...
            var id_sksd = $("#form_tpchucnang").attr("data2");
            if ($("input[name='free_gift']").is(":checked")) {
                var free_gift = 1;
            } else {
                var free_gift = 0;
            };
            console.log(free_gift);
    ...
    function xem_trc_tin() {
        ...
        form_tpchucnang.validate({
            ...
            messages: {
                ...
                han_sd: "Vui lòng nhập hạn sử dụng",
                gia_mongmuon1: {
                    required: "Vui lòng nhập giá mong muốn",
                },
                gia_mongmuon2: {
                    required: "Vui lòng nhập giá mong muốn",
                    gianguoimua: "Giá bắt đầu nhỏ hơn giá kết thúc",
                },
                td_gia_spham: "Vui lòng nhập giá sản phẩm",
                chitiet_dm: "Vui lòng chọn chi tiết danh mục",
                td_dia_chi: "Vui lòng nhập địa chỉ",
