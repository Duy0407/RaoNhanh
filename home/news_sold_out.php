...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT'])) {
    ...
    if ($type_user == 1) {
        ...
        $all_tin = new db_query("SELECT `new_id` FROM `new` WHERE `new_type` = $usertype AND `new_user_id` = $id_user
                    AND `new_buy_sell` = 2 AND `new_cate_id` != 120 AND `new_cate_id` != 121 AND tong_sluong = 0 ORDER BY `new_id` DESC ");
        ...
        // Hết hàng
        $tongsoluong = new db_query("SELECT new_id, `new_title`, `link_title`,tong_sluong,`sluong_daban`,`new_cate_id`, `da_ban`,`new_active` ,`new_money`, `new_unit`, `new_image`, `new_create_time`, `dia_chi`,
        `chotang_mphi`, `new_pin_cate`, `new_pin_home`, `new_day_tin` FROM `new` WHERE `new_user_id` = $id_user AND `new_buy_sell` = 2 
        AND `new_cate_id` != 120 AND `new_cate_id` != 121 AND new_active = 1 AND tong_sluong = 0 $sql_limit");
        ...
        <div class="box-right">
            ...
            <div class="box_rightdf" data="<?= $id_user ?>" data2="<?= $type_user ?>">
                ...
                <ul class="content-qlt" id="all-posts">
                    <? while ($showtin_hethang = (mysql_fetch_assoc($tongsoluong->result))) {
                        $avatar_tinthuong = $showtin_hethang['new_image'];
                        $avatar_tinthuong = explode(';', $avatar_tinthuong);
                        $id_tin = $showtin_hethang['new_id'];
                        $cat_id = $showtin_hethang['new_cate_id'];
                        $daban = $showtin_hethang['da_ban'];
                    ?>
                        <li class="main-content main_contentdf">
                            <div class="chu">
                                <div class="m_box_chu d_flex fl_row w100">
                                    <div class="anh anh_qltin_df">
                                        <!-- <? if ($showtin_hethang['da_ban'] == 0) { ?>
                                            <? if ($showtin_hethang['new_pin_home'] != 0 || $showtin_hethang['new_pin_cate'] != 0 || $showtin_hethang['new_day_tin'] != "") { ?>
                                        ...
                                        <a href="/<?= replaceTitle($showtin_hethang['link_title']) ?>-c<?= $id_tin ?>.html">
                                    ...
                                    <div class="m_box-txttilte d_flex fl_cl w100">
                                        <div class="text-tilte">
                                            ...
                                            <a href="/<?= replaceTitle($showtin_hethang['link_title']) ?>-c<?= $id_tin ?>.html">
                                                <div class="text-title-qlt text_ellipsis pr_10"><?= $showtin_hethang['new_title'] ?></div>
                                            </a>
                                        </div>
                                        <div class="text-btn">
                                            <div class="menu-bot">
                                                <div class="time-qlt d_flex fl_row al_ct">
                                                    ...
                                                    <div class="pdl_6"><?= lay_tgian($showtin_hethang['new_create_time']); ?></div>
                                                </div>
                                                <div class="m_lct_qlt d_flex fl_row al_ct">
                                                    ...
                                                    <div class="address-qlt pdl_6"><?= $showtin_hethang['dia_chi'] ?></div>
                                                </div>
                                                <div class="m_tieuthu_qlt d_flex fl_row jtf_spb">
                                                    <div class="m_money_qlt">
                                                        <? if ($showtin_hethang['chotang_mphi'] == 1) { ?>
                                                            <div class="money-qlt">Cho tặng miễn phí</div>
                                                        <? } elseif ($showtin_hethang['new_money'] > 0) { ?>
                                                            <div class="money-qlt d_flex fl_row al_ct ">
                                                                <div class="pdr_5">
                                                                    <?= number_format($showtin_hethang['new_money']) ?> <?= $donvitien[$showtin_hethang['new_unit']] ?>
                                                                </div>
                                                                <? if ($xacthuc_lket == 1) { ?>
                                                    ...
                                                    <? if ($xacthuc_lket == 1) { ?>
                                                        <!-- <div class="m_amount_qlt d_flex fl_row al_ct">
                                                            ...
                                                            <p class="m_amount_qlt_text pdl_5 pdr_5 p_400_s14_l17">
                                                                Số lượng: <?= $showtin_hethang['tong_sluong'] ?>
                                                            </p>
                                                        ...
                                                        <div class="m_sold_qlt d_flex fl_row al_ct">
                                                            ...
                                                            <p class="m_sold_qlt_text pdl_5 p_400_s14_l17">
                                                                Đã bán: <?= $showtin_hethang['sluong_daban'] ?>
                                                            </p>
                                                        </div> -->
                                                    <? } ?>
                                                </div>
                                ...
                                <? if ($showtin_hethang['da_ban'] == 1) { ?>
                                <? } else { ?>
                                    <div class="btn_qlt_df btn_qlt_df_chung btn_qlt_df_3 hid_1200">
                                        <div class="m_anhientin ctn_anhien_<?= $id_tin ?>" data="<?= $id_tin ?>" data1="<?= $showtin_hethang['new_active'] ?>">
                                            <!-- an -->
                                            <? if ($showtin_hethang['new_active'] == 1) { ?>
                                        ...
                                        <? if ($showtin_hethang['new_pin_home'] == 0 && $showtin_hethang['new_pin_cate'] == 0 && $showtin_hethang['new_day_tin'] == "") { ?>
                                            <!-- <span class="border-gt btn_qlt_df_3_d">
                                                <div class="btn-gt cus_poi text-gt" data="<?= $id_tin ?>">
                                                    <img src="/images/m_raonhanh_imgnew/cartspeed.png" alt="Bán nhanh hơn" class="m_bannhanhhon_qlt img_18">
                                                    <span class="text-gt" data="<?= $id_tin ?>">Bán nhanh hơn</span>
                                                </div>
                                            </span> -->
                                        <? } ?>
            ...
            <div class="m_phantrang w100 d_flex al_ct jtf_ct">
                <ul class="m_pt_ul d_flex fl_row al_ct">
                    <?= generatePageBar3('', $page, $current_page, $result_tongsoluong, $url, '?', '', 'active', 'preview', '<', 'next', '>', '', '', '', ''); ?>
                </ul>
            </div>
            <!--  -->
        </div>
