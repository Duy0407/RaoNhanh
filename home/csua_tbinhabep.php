...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_cs != 0 && $id_dm != 0) {
    ...
    if (mysql_num_rows($check_tind->result) > 0) {
        $list_tt = new db_query("SELECT n.`new_title`, n.`new_money`, n.`gia_kt`, n.`new_cate_id`, n.`new_city`, n.`new_image`, n.`new_unit`, n.`new_tinhtrang`,
                                ...
                                d.`new_description`, d.`loai_thiet_bi`, d.`loai_sanpham`, n.`new_phone`, n.`new_email`
        ...
        if ($thiet_bi != 37) {
            ...
            $list_lspham = new db_query("SELECT `id`, `ten_loai` FROM `loai_chung` where `id_cha` = $thiet_bi AND `id_danhmuc` = $id_dm ");
        }
    } else {
        header('Location: /');
    }
...
<head>
    ...
    <title>Đăng tin thiết bị nhà bếp</title>
        ...
        <div class="tindang-container">
            ...
            <div class="tindang-content-cont hd-disflex">
                ...
                <div class="tindang-col-right" data="<?= $id_cs ?>">
                    <form class="form-dtin-cont" id="form_tb_nhabep" data="<?= $us_id ?>" data1="<?= $us_type ?>">
                        <div class="row-tin-dang danhmuc_tt">
                            <p class="font-dam hd_font15-17">Danh mục sản phẩm <span class="color_red">*</span></p>
                            <input data="<?= $id_dm ?>" type="text" class="dmuc-spham" readonly name="san-pham-laptop" placeholder="Đồ gia dụng  >>  Thiết bị nhà bếp">
                        </div>
                            ...
                            <? if ($thiet_bi != 37) { ?>
                                <div class="row-tin-dang box_input_infor">
                                    ...
                                    <select class="slect-hang  hd_height36 loai_sanpham" name="loai_sanpham">
                                        ...
                                        <? while ($row_lspham = mysql_fetch_assoc($list_lspham->result)) { ?>
                                            <option value="<?= $row_lspham['id'] ?>" <?= ($row_lspham['id'] == $row_tt['loai_sanpham']) ? "selected" : "" ?>><?= $row_lspham['ten_loai'] ?></option>
                                        <? } ?>
                                    </select>
                                </div>
                            <? } ?>
                        ...
                        <div class="row-tin-dang cont-btn-sb hd-disflex hide-480-mobile">
                            ...
                            <button type="button" class="btn-submit td-dang-tin tb_nhabep hd_cspointer font-bold">ĐĂNG TIN</button>
                        </div>
                    </form>
                </div>
            </div>
...
<script type="text/javascript">
    $('.loai_thiet_bi').change(function() {
        ...
        $.ajax({
            url: "/render/dangtin_tbnhabep.php",
            type: 'POST',
            data: {
                nhom: id_tbi,
                id_dm: id_dm
    ...
    $(".tb_nhabep").click(function() {
        var form_tb_nhabep = $("#form_tb_nhabep");
        form_tb_nhabep.validate({
        ...
        if (form_tb_nhabep.valid() === true) {
            ...
            $.ajax({
                type: 'POST',
                url: '/ajax_dogiadung/csua_tbinhabep.php',
                data: fd,
                processData: false,
                contentType: false,
                success: function(data) {
