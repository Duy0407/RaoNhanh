...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_cs != 0) {
    ...
    $query_des = new db_query("SELECT `new_description`,`nhom_sanpham`,`loai_sanpham`,`chat_lieu` FROM `new_description` WHERE new_id = $id_cs");
    $sql_des = mysql_fetch_assoc($query_des->result);

    $id_nhom = $sql_des['nhom_sanpham'];

    $query_dm = new db_query("SELECT `tags_id`, `ten_tags`, `id_danhmuc`, `type_tags`, `id_parent` FROM `key_tags` WHERE id_danhmuc = 78 AND id_parent = '$id_nhom'");
    ...
    $list_ktag_query = new db_query("SELECT `tags_id`, `ten_tags`  FROM `key_tags` WHERE `id_danhmuc` = 78 ");
    $query = new db_query("SELECT *  FROM `nhom_sanpham` WHERE `id_danhmuc` = 78");
...
<head>
    ...
    <title>Chỉnh sửa tin nội thất phòng khách</title>
        ...
        <div class="tindang-container">
            ...
            <div class="tindang-content-cont hd-disflex">
                ...
                <div class="tindang-col-right">
                    <form class="form-dtin-cont" id="form_noithatpk" data2="<?= $id_cs ?>">
                        <div class="row-tin-dang danhmuc_tt">
                            <p class="font-dam hd_font15-17">Danh mục sản phẩm <span class="color_red">*</span></p>
                            <input type="text" data="<?= $sql_nd['new_cate_id'] ?>" class="dmuc-spham" readonly name="san-pham-laptop" placeholder="Nội thất - Ngoại thất >> Nội thất phòng khách">
                        </div>
                        ...
                        <div class="row-tin-dang box_input_infor">
                            ...
                            <select class="slect-hang nhomsanpham hd_height36" name="nhomsanpham" data="78">
                        ...
                        <div class="append_nhom">
                            <? if ($id_nhom == 2 || $id_nhom == 3 || $id_nhom == 5) {
                                $query = new db_query("SELECT * FROM loai_chung WHERE id_cha = $id_nhom  AND id_danhmuc = 78");
                                ...
                                <div class="row-tin-dang box_input_infor">
                                    ...
                                    <select class="slect-hang  hd_height36 loai_sanpham" name="loai_sanpham">
                                        ...
                                        <?php foreach ($loai as $key => $value) : ?>
                                            <option value="<?= $value['id'] ?>" <?= ($sql_des['loai_sanpham'] == $value['id']) ? "selected" : "" ?>><?= $value['ten_loai'] ?></option>
                                        <?php endforeach ?>
                                    </select>
                                </div>
                            <? } ?>
                            ...
                            <?php if ($id_nhom != 5) :
                                $query = new db_query("SELECT * FROM nhom_sanpham_chatlieu WHERE id_cha = $id_nhom AND id_danhmuc = 78");
                                ...
                                <div class="row-tin-dang box_input_infor">
                                    ...
                                    <select class="slect-hang  hd_height36" name="chat_lieu">
                                        ...
                                        <?php foreach ($chat as $key => $value) : ?>
                                            <option value="<?= $value['id'] ?>" <?= ($sql_des['chat_lieu'] == $value['id']) ? "selected" : "" ?>><?= $value['name'] ?></option>
                                        <?php endforeach ?>
                                    </select>
                                </div>
                            <?php endif ?>
                        ...
                        <div class="row-tin-dang box_input_infor tags_doi">
                            <? if ($id_nhom == 1 || $id_nhom == 2 || $id_nhom == 3 || $id_nhom == 5) { ?>
                        ...
                        <div class="row-tin-dang-btn cont-btn-sb hd-disflex hide-480-mobile">
                            ...
                            <button type="button" class="btn-submit td-dang-tin noithatpk hd_cspointer font-bold">CHỈNH SỬA</button>
                        </div>
                    </form>
                </div>
            </div>
...
<script type="text/javascript">
    $('.nhomsanpham').change(function() {
        ...
        $.ajax({
            url: "../render/dangtinphongkhach.php",
            type: 'GET',
            data: {
                nhom: nhom,
                id_dm: id_dm
    ...
    // ---------------Validate đăng tin nội, ngoại thất-----------------
    $(".noithatpk").click(function() {
        var form_noithatpk = $("#form_noithatpk");
        form_noithatpk.validate({
        ...
        if (form_noithatpk.valid() === true) {
            var id_cs = $("#form_noithatpk").attr("data2");
            var tieu_de = $("input[name='tieu_de']").val();
            var loai = 0;
            var chatlieu = 0;
            var nhom = $("select[name='nhomsanpham']").val();
            if (nhom == 2 || nhom == 3 || nhom == 5) loai = $('.loai_sanpham').val();
            if (nhom != 5) chatlieu = $("select[name='chat_lieu']").val();
            ...
            $.ajax({
                type: 'POST',
                url: '/ajax_noithat_ngoaithat/edit_phongkhach.php',
                data: fd,
                processData: false,
                contentType: false,
                success: function(data) {
