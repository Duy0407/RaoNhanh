...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_dm != 0 && $id_nd != 0) {
    ...
    $qr_listds = new db_query("SELECT `new`.`new_id`, `new_title`, `new_money`, `new_city`, `new_image`, `new_unit`, `new_name`, `new_phone`, `new_email`,
                            ...
                            `new_baohanh`, `new_description`, `loai_linhphu_kien`, `thiet_bi`, `canhan_moigioi`  FROM `new`
    ...
    if (mysql_num_rows($qr_listds->result) > 0) {
        ...
        $querylk = new db_query("SELECT id, ten_loai  FROM `loai_chung` WHERE `id_danhmuc` = $id_dm ");
...
<head>
    ...
    <title>Chỉnh sửa tin đăng đồ tiện tử linh kiện</title>
                ...
                <div class="ctn_ct_box6 d_flex fl_cl">
                    ...
                    <div class="ctn_ct_b6_fr d_flex fl_row fl_st">
                        <div class="b6_fr1 d_flex fl_cl box_input_infor">
                            ...
                            <select class="b6_fr1_select slect-hang hd_height36" name="linh_kien" data="<?= $id_dm ?>" onchange="hang_doi(this), lpkien_doi(this)">
    ...
    <script type="text/javascript">
        ...
        $(".b11_btn_chinhsua").click(function() {
            ...
            if (form_pc.valid() === true) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '/ajax_ddtu/csua_linhkien.php',
                    data: fd,
                    processData: false,
                    contentType: false,
                    success: function(data) {
