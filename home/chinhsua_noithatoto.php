...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_xc != 0) {
    ...
    $query = new db_query("SELECT n.`new_title`, n.`new_money`, n.`gia_kt`, n.`new_cate_id`, n.`new_city`, n.`new_image`, n.`new_unit`, n.`new_tinhtrang`,
                            ...
                            FROM `new` AS n WHERE n.`new_id` = $id_xc  ");
    $sql_nd = mysql_fetch_assoc($query->result);
    $query_des = new db_query("SELECT * FROM `new_description` WHERE new_id = $id_xc");
    $sql_des = mysql_fetch_assoc($query_des->result);
    $query_dm = new db_query("SELECT `tags_id`, `ten_tags`, `id_danhmuc`, `type_tags`, `id_parent` FROM `key_tags` WHERE  id_danhmuc = 42 ");
...
<head>
    ...
    <title>Chỉnh sửa tin nội thất ô tô</title>
        ...
        <div class="tindang-container">
            ...
            <div class="tindang-content-cont hd-disflex">
                ...
                <div class="tindang-col-right">
                    <form class="form-dtin-cont" id="form_xeco_noithatoto" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_xc ?>">
                        <div class="row-tin-dang danhmuc_tt">
                            <p class="font-dam hd_font15-17">Danh mục sản phẩm <span class="color_red">*</span></p>
                            <input type="text" data="<?= $sql_nd['new_cate_id'] ?>" class="dmuc-spham" readonly name="san-pham-laptop" placeholder="Xe cộ  >>  Nội thất ô tô">
                        </div>
                        ...
                        <div class="row-tin-dang box_input_infor">
                            <p class="font-dam hd_font15-17">Loại nội thất<span class="color_red">*</span></p>
                            <select class="slect-hang  hd_height36" name="loai_noi_that">
                                <option disabled value="">Loại nội thất</option>
                                <? foreach ($result_nt as $nt) : ?>
                                    <option <?= ($nt['id'] == $sql_des['loai_noithat']) ? "selected" : "" ?> value="<?= $nt['id'] ?>"><?= $nt['ten_loai'] ?></option>
                                <? endforeach ?>
                            </select>
                        </div>
                        <div class="row-tin-dang box_input_infor">
                        ...
                        <div class="row-tin-dang cont-btn-sb hd-disflex">
                            ...
                            <button type="button" class="btn-submit dt_noithat td-dang-tin hd_cspointer font-bold">CHỈNH SỬA</button>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    $(".dt_noithat").click(function() {
        var form_xeco_noithatoto = $("#form_xeco_noithatoto");
        ...
        if (form_xeco_noithatoto.valid() === true) {
            var user_id = $("#form_xeco_noithatoto").attr("data");
            var user_type = $("#form_xeco_noithatoto").attr("data1");
            var id_xc = $("#form_xeco_noithatoto").attr("data2");
            ...
            var loai_noi_that = $("select[name='loai_noi_that']").val();
            ...
            fd.append('loai_noi_that', loai_noi_that);
            ...
            $.ajax({
                url: '/ajax_xeco/edit_xeco_notthat.php',
                type: 'POST',
                data: fd,
                contentType: false,
                processData: false,
    ...
    function xem_trc_tin() {
        var form_xeco_noithatoto = $(".form-dtin-cont");
        form_xeco_noithatoto.validate({
            errorPlacement: function(error, element) {
                error.appendTo(element.parents(".box_input_infor"));
                error.wrap("<span class='error'>");
        ...
        if (form_xeco_noithatoto.valid() === false) {
        ...
        if (form_xeco_noithatoto.valid() === true) {
            ...
            fd.append('free_gift', free_gift);
            fd.append('tieu_de', tieu_de);
            fd.append('loai_sp', loai_noi_that);
