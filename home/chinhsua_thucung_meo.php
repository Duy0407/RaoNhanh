...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT']) && $id_tc != 0) {
    ...
    $query = new db_query("SELECT`new_title`,`link_title`,`new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,
                ...
                `quan_huyen`, `phuong_xa`, `new_sonha`, `dia_chi`, `new_video`, `new_ctiet_dmuc` FROM `new` WHERE `new_id` = $id_tc ");
    ...
    $query_dm = new db_query("SELECT `tags_id`, `ten_tags`, `id_danhmuc`, `type_tags`, `id_parent` FROM `key_tags` WHERE  id_danhmuc = 112 AND id_parent = 0");
...
<head>
    ...
    <title>Chỉnh sửa tin thú cưng mèo</title>
        ...
        <div class="tindang-container">
            ...
            <div class="tindang-content-cont hd-disflex">
                ...
                <div class="tindang-col-right">
                    <form class="form-dtin-cont" id="form_thucung_meo" data="<?= $acc_id ?>" data1="<?= $acc_type ?>" data2="<?= $id_tc ?>">
                        <div class="row-tin-dang danhmuc_tt">
                            <p class="font-dam hd_font15-17">Danh mục sản phẩm <span class="color_red">*</span></p>
                            <input type="text" data="<?= $sql_nd['new_cate_id'] ?>" class="dmuc-spham" readonly name="san-pham-laptop" placeholder="Thú cưng >> Mèo">
                        </div>
                        ...
                        <div class="row-tin-dang box_input_infor">
                            ...
                            <select class="slect-hang  hd_height36" name="giong_thucung">
                                ...
                                <?
                                $sql_meo = new db_query("SELECT `id`, `giong_thucung` FROM `giong_thu_cung` WHERE id_danhmuc = 112");
                                $result_meo = $sql_meo->result_array();
                                ?>
                                <? foreach ($result_meo as $meo) : ?>
                                    <option <?= ($meo['id'] == $sql_des['giong_thu_cung']) ? "selected" : "" ?> value="<?= $meo['id'] ?>"><?= $meo['giong_thucung'] ?></option>
                                <? endforeach ?>
                            </select>
                        </div>
                        <div class="row-tin-dang box_input_infor">
                            ...
                            <select class="slect-hang  hd_height36" name="do_tuoi">
                                ...
                                <?
                                $query_dt = new db_query("SELECT `id`, `contents_name`, `id_danhmuc`, `type` FROM `thongtin_thucung` WHERE id_danhmuc = 112 AND `type` = 1");
                        ...
                        <div class="d_flex j_between">
                            <div class="row-tin-dang rowflex2 box_input_infor">
                                ...
                                <select class="slect-hang  hd_height36" name="kichco">
                                    ...
                                    <?
                                    $query_kc = new db_query("SELECT `id`, `contents_name`, `id_danhmuc`, `type` FROM `thongtin_thucung` WHERE id_danhmuc = 112 AND `type` = 2");
                            ...
                            <div class="row-tin-dang rowflex2 box_input_infor">
                                ...
                                <select class="slect-hang  hd_height36" name="gioitinh">
                                    ...
                                    <?
                                    $query_gt = new db_query("SELECT `id`, `contents_name`, `id_danhmuc`, `type` FROM `thongtin_thucung` WHERE id_danhmuc = 112 AND `type` = 3");
                        ...
                        <div class="row-tin-dang">
                            ...
                            <div class="input-gia-cont">
                                <div class="box_input_infor">
                                    <input class="input_infor_tag error" type="text" name="td_gia_spham" onkeyup="format_gtri(this)" placeholder="" autocomplete="off" oninput="<?= $oninput ?>" id="gia-ban-sp" onkeydown="javascript: return ['Backspace','Delete','ArrowLeft','ArrowRight'].includes(event.code) ? true : !isNaN(Number(event.key)) && event.code!=='Space'" value="<?= ($sql_nd['new_money'] > 0) ? number_format($sql_nd['new_money']) : "" ?>" <?= ($sql_nd['new_money'] > 0) ? "" : "disabled" ?>>
                                </div>
                        ...
                        <div class="row-tin-dang cont-btn-sb hd-disflex hide-480-mobile">
                            ...
                            <button type="button" class="btn-submit td-dang-tin thucung_meo hd_cspointer font-bold">CHỈNH SỬA</button>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    $(".thucung_meo").click(function() {
        ...
        if (form_thucung_meo.valid() === true) {
            var user_id = $("#form_thucung_meo").attr("data");
            var user_type = $("#form_thucung_meo").attr("data1");
            var id_tc = $("#form_thucung_meo").attr("data2");
            ...
            $.ajax({
                url: '/ajax_thucung/edit_thucung_meo.php',
                type: 'POST',
                data: fd,
                contentType: false,
                processData: false,
    ...
    function xem_trc_tin() {
        var form_thucung_meo = $(".form-dtin-cont");
        form_thucung_meo.validate({
            errorPlacement: function(error, element) {
                error.appendTo(element.parents(".box_input_infor"));
                error.wrap("<span class='error'>");
        ...
        if (form_thucung_meo.valid() === false) {
