...
if (isset($_COOKIE['UID'])) {
    ...
    $loai_spham = getValue('loai_sanpham', 'int', 'POST', 0);
        ...
        if ($cou_file > 0 || $cou_dang > 0) {
            ...
            if (mysql_num_rows($check_tkhoan->result) > 0) {
                ...
                if (mysql_num_rows($check_tde->result) > 0) {
                    ...
                    $update_des = new db_query("UPDATE `new_description` SET `new_description` = '$mo_ta', `nhom_sanpham` = '$nhom_spham', `loai_sanpham` = '$loai_spham',
                                            `chat_lieu` = '$chat_lieu', `canhan_moigioi` = '$canhan_moigioi' WHERE `new_id` = $id_nd ");
                    include('../ajax/xac_thuc_chung.php');

                    // fastcgi_finish_request();

                    // $tt_edit = 'nội thất văn phòng: ' . $id_nd;
