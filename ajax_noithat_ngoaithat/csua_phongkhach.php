        ...
        if ($cou_file > 0 || $cou_dang > 0) {
            ...
            if (mysql_num_rows($check_tde->result) > 0) {
                ...
                $query_nt = new db_query("UPDATE `new` SET `new_title`='$tieu_de', `new_money` = '$gia_spham', `new_city`='$tinh_thanh',`new_image`='$file_anh',
                                        ...
                                        `new_tinhtrang`='$tinh_trang', `new_phone` = '$sdt_lhe', `new_email` = '$email_lhe', `chotang_mphi` = '$tang_mphi',
                                        `new_address` = '$dia_chi',`thoigian_kmbd` = '$ngay_bat_dau',`thoigian_kmkt` = '$ngay_ket_thuc',
                                        `soluong_min` = '$soluong_min',`soluong_max` = '$soluong_max' WHERE new_id = $id_nd AND new_user_id = $user_id ");
                ...
                $query_des = new db_query("UPDATE `new_description` SET `new_description`='$mo_ta',`nhom_sanpham`='$nhom_spham', `loai_sanpham`='$loai_sanpham',
                                        `chat_lieu`='$chat_lieu', `canhan_moigioi` = '$canhan_moigioi' WHERE new_id = $id_nd ");
                include('../ajax/xac_thuc_chung.php');

                // fastcgi_finish_request();

                // $tt_edit = 'nội thất phòng khách: ' . $id_nd;
