...
if (isset($_COOKIE['UID']) && isset($_COOKIE['UT'])) {
    ...
    $thuong_hieu = getValue('thuong_hieu', 'int', 'POST', 0);
        ...
        if ($cou_file > 0 || $cou_dang > 0) {
            ...
            if (mysql_num_rows($check_tde->result) > 0) {
                ...
                $query_nt = new db_query("UPDATE `new` SET `new_title`='$tieu_de', `new_money`='$gia_spham', `new_city`='$tinh_thanh',`new_image`='$file_anh',
                                        ...
                                        `new_buy_sell` = 2, `chotang_mphi` = '$tang_mphi',`thoigian_kmbd` = '$ngay_bat_dau',`thoigian_kmkt` = '$ngay_ket_thuc',
                                        `soluong_min` = '$soluong_min',`soluong_max` = '$soluong_max'
                                         WHERE new_id = $id_nd AND new_user_id = $user_id ");

                $query_des = new db_query("UPDATE `new_description` SET `new_description`='$mo_ta', `loai_sanpham`='$loai_spham', `hinhdang`='$hinh_dang',
                                        `hang`='$thuong_hieu', `canhan_moigioi` = '$canhan_moigioi' WHERE new_id = $id_nd ");
                include('../ajax/xac_thuc_chung.php');

                // fastcgi_finish_request();

                // $tt_edit = 'nội thất phòng tắm: ' . $id_nd;
