            ...
            if ($user_id != 0 && $user_type != 0 && $id_dm != 0 && $tieu_de != '') {
                if ($cou_file > 0 || $cou_dang > 0) {
                    ...
                    if (mysql_num_rows($check_tde->result) > 0) {
                        ...
                        $inser_des = new db_query("INSERT INTO `new_description`(`new_id`, `new_description`,`loai_sanpham`, `hinhdang`, `hang`, `canhan_moigioi`)
                                                VALUES ('$id_dt','$mo_ta','$loai_spham','$hinh_dang','$thuong_hieu','$canhan_moigioi')");
                        include('../ajax/xac_thuc_chung.php');

                        // fastcgi_finish_request();

                        // $tt_crea = 'Nội thất - Ngoại thất Phòng tắm';
