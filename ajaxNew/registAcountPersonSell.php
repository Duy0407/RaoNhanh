...
if($email != '' && $password != '' && $name != '' && $phone != '' && $city != 0 && $address != ''){
    $data_check = new db_query("SELECT usc_id FROM user WHERE usc_email = '$email' AND usc_type = 1");
    if(mysql_num_rows($data_check->result) == 0){
        ...
        $qr_insert = new db_query("
            ...
            `usc_address`, `usc_type`, `usc_time`, `usc_city`,  `client_ip`)
            VALUES ('$name','$password_md5','$phone','$email','$birth','$gender','$address',1,'$time','$city','$ip')
        ");
