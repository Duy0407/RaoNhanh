...
if (isset($_COOKIE['UID'])) {
    ...
    $han_sd = getValue('hansudung', 'str', 'POST', '');
    ...
    if ($user_id != 0 && $id_nd != 0 && $tieu_de != "") {
        if ($cou_file > 0 || $cou_dang > 0) {
            ...
            if (mysql_num_rows($query_check->result) > 0) {
                ...
                `new_update_time`='$ngay_dang',`new_unit`='$donvi_ban',`chotang_mphi`='$td_lienhe_ngban', `quan_huyen`= 0, `phuong_xa`= 0,
                `new_sonha`='', `dia_chi`='" . $dia_chi . "', `new_video`='$file_video', `new_ctiet_dmuc`='$ctiet_dmuc', `new_buy_sell` = '$new_buy_sell',
                `new_phone`='$sdt_lhe',`new_email` ='$email_lhe',`thoigian_kmbd` = '$ngay_bat_dau',`thoigian_kmkt` = '$ngay_ket_thuc',
                `soluong_min` = '$soluong_min',`soluong_max` = '$soluong_max' WHERE new_id = $id_nd ");

                $query_des = new db_query("UPDATE `new_description` SET  `new_description`='$mo_ta', `loai_sanpham`='$loai_thucpham', `han_su_dung`='$han_sd',`canhan_moigioi` = '$canhan_moigioi'
                                WHERE new_id = $id_nd ");
                ...
                //     'FromWeb' => $domain
                // ];
                // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa chỉnh sửa tin đăng thực phẩm chức năng: " . $id_nd . " trên " . $domain;
                // $data_send_to_group = [
                //     'LiveChat' => $LiveChat,
