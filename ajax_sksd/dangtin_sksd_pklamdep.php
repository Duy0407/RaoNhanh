        ...
        if ($cou_file > 0 || $cou_dang > 0) {
            ...
            if (mysql_num_rows($query_check->result) > 0) {
                ...
                $query_dt = new db_query("INSERT INTO `new`(`new_title`,`link_title`,`new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`,
                                    ...
                                    '$td_gia_spham','$cate_id','$tinh_thanh','$user_id','$file_anh','$ngay_dang','$user_type',1,'$dvi_tien','$user_name','$sdt_lhe',
                                    '$email_lhe','" . $dia_chi . "',0,'$td_lienhe_ngban',0,0,'','" . $dia_chi . "','$file_video',
                                    '$ctiet_dmuc','$tinhtrang','$new_buy_sell','$ngay_dang','$ngay_bat_dau','$ngay_ket_thuc','$soluong_min','$soluong_max')");

                $id_des = new db_query("SELECT LAST_INSERT_ID() AS id");
                $id_dt = mysql_fetch_assoc($id_des->result)['id'];

                $query_decription = new db_query("INSERT INTO `new_description`(`new_id`, `new_description`,`loai_sanpham`,`hang`,`canhan_moigioi`)
                                            VALUES ('$id_dt','$mo_ta','$loai_phukien','$hang','$canhan_moigioi')");
                ...
                //     'FromWeb' => $domain
                // ];
                // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa đăng tin phụ kiện làm đẹp trên " . $domain;
                // $data_send_to_group = [
                //     'LiveChat' => $LiveChat,
