...
if (isset($_COOKIE['UID'])) {
    ...
    if ($user_id != 0 && $tieu_de != '' && $id_nd != 0 && $id_dm != 0) {
            ...
            if (mysql_num_rows($check_tkhoan->result) > 0) {
                ...
                if (mysql_num_rows($check_tde->result) > 0) {
                    ...
                    $updat_td = new db_query("UPDATE `new` SET `new_title` = '$tieu_de', `new_money` = '$gia_spham', `new_city` = '$tinh_thanh', `new_image` = '$file_anh',
                                                        `new_unit` = '$dvi_tien', `chotang_mphi` = '$tang_mphi', `quan_huyen` = '$quan_huyen', `phuong_xa` = '$phuong_xa',
                                                        `new_sonha`='" . $so_nha . "', `dia_chi` = '" . $dia_chi . "', `new_video` = '$file_video', `new_ctiet_dmuc` = '$ctiet_dm',
                                                        `new_update_time` = '$ngay_dang', `new_tinhtrang` = '$tinh_trang', `new_buy_sell` = '$new_buy_sell',`new_phone` = '$sdt_lhe',
                                                        `new_email` = '$email_lhe', `new_address` = '" . $dia_chi . "',`thoigian_kmbd` = '$ngay_bat_dau',`thoigian_kmkt` = '$ngay_ket_thuc',
                    ...
                    //     'FromWeb' => $domain
                    // ];
                    // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa chỉnh sửa tin đăng hoa, quà tặng, handmade: " . $id_nd . " trên " . $domain;
                    // $data_send_to_group = [
                    //     'LiveChat' => $LiveChat,
