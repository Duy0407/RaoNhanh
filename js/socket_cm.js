function like_url(type, cl, cl2) {
    ...
    $.ajax({
        ...
        success: function (data) {
            $('.show_ic').hide();
            $('.' + cl + '_txt').removeClass('red').removeClass('liked').removeClass('yll').removeClass('orange');
            // gắn active và thêm số
            if (!$('.' + cl).hasClass('active')) {
                $('.' + cl).addClass('active').attr('data', type);
                $('.' + cl2 + ' .count_ic').text(data);
                if (data != '') { $('.cm_view_ic').hide(); } else { $('.cm_view_ic').show(); }
            }
            else {
                // xóa icon trước đó
                $('.' + cl2 + ' .icon_new').removeClass('show').removeClass('icon_new');
            }
            // check có icon chưa để thêm vào danh sách
            if (type > 0 && !$('.' + cl2 + ' .ic' + type).hasClass('show')) {
                $('.' + cl2 + ' .ic' + type).addClass('icon_new').addClass('show');
            }

            switch (type) {
                case 1:
                    $('.' + cl + '_txt').addClass("liked").text('Thích');
                    $('.' + cl + '_img').attr('src', '/images/img_comment/Ic_color_1.png');
                    break;
                case 2:
                    $('.' + cl + '_txt').addClass("red").text('Yêu thích');
                    $('.' + cl + '_img').attr('src', '/images/img_comment/Ic_' + type + '.png')
                    break;
                case 3:
                    $('.' + cl + '_txt').addClass("yll").text('Wow');
                    $('.' + cl + '_img').attr('src', '/images/img_comment/Ic_' + type + '.png')
                    break;
                case 4:
                    $('.' + cl + '_txt').addClass("yll").text('Thương thương');
                    $('.' + cl + '_img').attr('src', '/images/img_comment/Ic_' + type + '.png')
                    break;
                case 5:
                    $('.' + cl + '_txt').addClass("orange").text('Phẫn nộ');
                    $('.' + cl + '_img').attr('src', '/images/img_comment/Ic_' + type + '.png')
                    break;
                case 6:
                    $('.' + cl + '_txt').addClass("yll").text('Buồn');
                    $('.' + cl + '_img').attr('src', '/images/img_comment/Ic_' + type + '.png')
                    break;
                case 7:
                    $('.' + cl + '_txt').addClass("yll").text('Ha ha');
                    $('.' + cl + '_img').attr('src', '/images/img_comment/Ic_' + type + '.png')
                    break;
                default:
                    // if ($('.'+cl).hasClass('active')) {
                    //     var dem = Number($('.'+cl2+' .count_ic').text())-1;
                    //     var txt_dem="";
                    //     if (dem>0){txt_dem=dem;}
                    //     $('.'+cl2+' .count_ic').text(txt_dem);
                    // }
                    $('.' + cl2 + ' .count_ic').text(data);
                    if (data != '') { $('.cm_view_ic').hide(); } else { $('.cm_view_ic').show(); }
                    ...
                    $('.' + cl + '_txt').removeClass('red').removeClass('liked').removeClass('yll').removeClass('orange');
                    $('.' + cl + '_txt').text('Thích');
                    $('.' + cl + '_img').attr('src', '/images/img_comment/Ic_color_2.png');
            }
        }
    });
}
...
function reply_cm(t) {
    ...
    var input = '<div class="cm_input input_reply active">\n' +
        ...
        '                                <img class="ava_cm" src="' + img_u + '" alt="bình luận">\n' +
        '                                <textarea class="ct_cm" id="ct_cm_rl" maxlength="250" oninput="this.style.height = (this.scrollHeight) + \'px\'" onkeydown="send_cm_rl(this,event)" placeholder="Viết bình luận"></textarea>\n' +
        ...
        '                                </svg>\n' +
        '                                <svg class="cm_img_ct" onclick="document.getElementById(`secleimg_rl`).click();" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
        ...
        '                           </svg>\n' +
        '                        <input style="display: none;" id="secleimg_rl" name="listimg" onchange="preview_image(event, this);" class="fileupload" type="file">\n' +
        '                       <div id="tag_friend_rl"></div>\n' +
        '               </div>';
    ...
    autotag(document.getElementById("ct_cm_rl"), arr_friend, 'tag_friend_rl', []);
};
...
$('.ct_cm').keydown(function (e) {
    ...
    if (e.keyCode === 13 && !e.shiftKey) {
        ...
        if (comment != "") {
            ...
            var name = uid_name;
            var img = uid_ava;
            send_cm(url, id, name, img, comment);
        }
...
$('.ic_send_cm').click(function () {
    var comment = $('.input_comment .ct_cm').val().trim();
    var file = $('#secleimg').prop('files')[0];
    if (comment != "" || file != undefined) {
        ...
        var img = uid_ava;
        send_cm(url, id, name, img, comment);
        $('.input_comment .ct_cm').val("").css('height', '48px').focus();
        return false;
    }
});
...
function send_cm_rl(t, e) {
    ...
    var file = $('#secleimg_rl').prop('files')[0];
    ...
    if (e.keyCode === 13 && !e.shiftKey) {
        ...
        if (comment != "" || file != undefined) {
            ...
            send_cm(url, id, name, img, comment, cm_id);
...
function send_cm(url, id, name, img, comment, cm_id = 0) {
    ...
    if (cm_id == 0) {
        file = $('#secleimg').prop('files')[0];
    } else {
        file = $('#secleimg_rl').prop('files')[0];
    }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data != '' && data != 'error') {
                if (cm_id == 0) {
                    $('.cm_list .box_cm_list').prepend(data);
                    var x = parseInt($('.cm_cm_ic span').text()) + 1;
                    $('.cm_cm_ic span').text(x);
                    $('.box_load_img').remove();
                    $('#secleimg').val('');
                }
                else if (cm_id >= 0) {
...
function autotag(inp, arr, append, tag) {
    if (arr.length == 0) {
        return false;
    }
    var currentFocus;
    inp.addEventListener("keyup", function (e) {
        ...
        closeAllLists();
        if (!str.startsWith("@") && !str.endsWith(" @") && str != '@' && val.trim() == '') { $('#' + append).hide(); return false; }
            ...
            for (j = 0; j < arr[i][0].length; j++) {
                if (arr[i][0].substr(j, val.length).toUpperCase() == val.toUpperCase() || (e.keyCode === 50 && e.shiftKey)) {
                    ...
                    b.addEventListener("click", function (e) {
                        inp.value = inp.value.substr(0, inp.value.lastIndexOf(val)) + this.getElementsByTagName("input")[0].value;
...
$('#cm_img_ct').click(function () {
    $('#secleimg').click();
})


function preview_image(e, element, t) {
    ...
    preview_before_upload(e, element, t);
}
