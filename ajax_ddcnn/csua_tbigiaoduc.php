...
if (isset($_COOKIE['UID'])) {
    ...
    if ($user_id != 0 && $id_cs != 0 && $tieu_de != '' && $id_dm != 0) {
            ...
            if (mysql_num_rows($check_tkhoan->result) > 0) {
                ...
                if (mysql_num_rows($check_tde->result) > 0) {
                    ...
                    $updat_td = new db_query("UPDATE `new` SET `new_title` = '$tieu_de', `new_money` = '$gia_spham', `new_city` = '$tinh_thanh', `new_image` = '$file_anh',
                                                ...
                                                `new_update_time` = '$ngay_dang', `new_tinhtrang` = '$tinh_trang'
                                                WHERE `new_user_id` = $user_id AND `new_type` = $user_type AND `new_id` = $id_cs ");

                    $update_des = new db_query("UPDATE `new_description` SET `new_description` = '$mo_ta' WHERE `new_id` = $id_cs ");
                    ...
                    $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa chỉnh sửa tin đăng thiết bị giao dục: " . $id_cs . " trên " . $domain;
                    $data_send_to_group = [
                        'LiveChat' => $LiveChat,
