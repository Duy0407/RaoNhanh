            ...
            if (mysql_num_rows($check_tkhoan->result) > 0) {
                ...
                if (mysql_num_rows($check_tde->result) > 0) {
                    ...
                    $updat_td = new db_query("UPDATE `new` SET `new_title` = '$tieu_de', `new_money` = '$gia_spham', `new_city` = '$tinh_thanh', `new_image` = '$file_anh',
                                            ...
                                            `new_update_time` = '$ngay_dang', `new_tinhtrang` = '$tinh_trang',`new_baohanh`='$bao_hanh',`new_phone`='$sdt_lhe',`new_email`='$email_lhe',
                                            `new_address` = '" . $dia_chi . "'
                                            WHERE `new_user_id` = $user_id AND `new_type` = $user_type AND `new_id` = $id_nd ");

                    $update_des = new db_query("UPDATE `new_description` SET `new_description` = '$mo_ta', `loai_thiet_bi` = '$loai', `hang` = '$hang',
                                            `cong_suat` = '$congsuat', `dung_tich` = '$dungtich', `khoiluong` = '$khoiluong',`loai_chung`='$loai_may_giat', 
