            ...
            if (mysql_num_rows($check_tkhoan->result) > 0) {
                ...
                if (mysql_num_rows($check_tde->result) > 0) {
                    ...
                    $updat_td = new db_query("UPDATE `new` SET `new_title` = '$tieu_de', `new_money` = '$gia_spham', `new_city` = '$tinh_thanh', `new_image` = '$file_anh',
                                            ...
                                            `new_update_time` = '$ngay_dang', `new_tinhtrang` = '$tinh_trang',`new_address` = '" . $dia_chi . "',
                                            `new_phone`='$sdt_lhe',`new_email`='$email_lhe'
                                            WHERE `new_user_id` = $user_id AND `new_type` = $user_type AND `new_id` = $id_nd ");

                    $update_des = new db_query("UPDATE `new_description` SET `new_description` = '$mo_ta', `loai_thiet_bi` = '$loai_tbi',
                                            `loai_sanpham` = '$loai_spham',`canhan_moigioi` = '$canhan_moigioi' WHERE `new_id` = $id_nd ");
                    ...
                    //     'FromWeb' => $domain
                    // ];
                    // $MessageInforSupport = "Xin chào, " . $usc_company . "" . $phone . "" . $email . "tôi vừa chỉnh sửa tin đăng thiết bị nhà bếp: " . $id_nd . " trên " . $domain;
                    // $data_send_to_group = [
                    //     'LiveChat' => $LiveChat,
