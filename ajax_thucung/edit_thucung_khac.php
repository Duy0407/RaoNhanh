...
if (isset($_COOKIE['UID'])) {
    ...
    $do_tuoi = getValue('do_tuoi', 'str', 'POST', 0);
    $kichco = getValue('kichco', 'str', 'POST', 0);
    ...
    if ($user_id != 0 && $id_tc != 0 && $tieu_de != "") {
        if ($cou_file > 0 || $cou_dang > 0) {
            ...
            if (mysql_num_rows($query_check->result) > 0) {
                ...
                `new_address` = '" . $dia_chi . "',`new_phone`='$sdt_lhe',`new_email`='$email_lhe',`new_sonha`='" . $so_nha . "', `dia_chi`='" . $dia_chi . "',
                `new_video`='$file_video', `new_ctiet_dmuc`='$ctiet_dmuc' WHERE new_id = $id_tc ");

                $query_des = new db_query("UPDATE `new_description` SET `new_description`='$mo_ta',`giong_thu_cung`='$giong_thucung',`do_tuoi`='$do_tuoi',
                                        `kich_co`='$kichco',`gioi_tinh`='$gioitinh',`canhan_moigioi` = '$canhan_moigioi' WHERE new_id = $id_tc ");
            }
        } else {
