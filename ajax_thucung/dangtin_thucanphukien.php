...
if (isset($_COOKIE['UID'])) {
    ...
    if ($user_id != 0 && $user_type != 0 && $tieu_de != "") {
        if ($cou_file > 0 || $cou_dang > 0) {
            $query_check = new db_query("SELECT `new_id` FROM `new` WHERE `new_user_id` = $user_id AND `new_type` = $user_type AND
            ...
            if (mysql_num_rows($query_check->result) > 0) {
                ...
                $query_dt = new db_query("INSERT INTO `new`(`new_title`,`link_title`,`new_money`, `new_cate_id`, `new_city`, `new_user_id`, `new_image`, `new_create_time`,
                                    ...
                                    VALUES ('$tieu_de','$tieu_de','$td_gia_spham','$cate_id','$tinh_thanh','$user_id','$file_anh','$ngay_dang',
                                    '$user_type','$new_buy_sell',1,'$dvi_tien','$user_name','$sdt_lhe','$email_lhe','" . $dia_chi . "',0,'$free_gift',
                                    '$quan_huyen','$phuong_xa','" . $so_nha . "','" . $dia_chi . "','$file_video','$ctiet_dmuc','$ngay_dang'
                ...
                $query_decription = new db_query("INSERT INTO `new_description`(`new_id`, `new_description`,`nhom_sanpham`, `giong_thu_cung`,`han_su_dung`,
                                            `khoiluong`,`the_tich`,`canhan_moigioi`) VALUES ('$id_dt','$mo_ta','$nhomsanpham','$giong_thucung','" . $han_sd . "',
                                            '" . $trong_luong . "','" . $thetich . "','$canhan_moigioi')");
